...
function changeAtt(t,goods_id) {
	...
	for (var i = 0; i<t.parentNode.childNodes.length;i++) {
		if (t.parentNode.childNodes[i].className == 'click') {
...
function show_attr_status(theid, goods_id, attr_id_www_yshop100_com)
{
	...
	var www_yshop100_com= theid.parentNode.id.replace('catt_', '');
	document.getElementById('spec_attr_type_'+www_yshop100_com).value=selected_first[0];

	//if(attr_selected()){
	//	return true;
	//}
	if(change_classname(theid,www_yshop100_com)){
		//var formBuy = document.forms['ECS_FORMBUY'];
		//spec_arr = getSelectedAttributes(formBuy);
		//Ajax.call('goods.php?act=get_products_info', 'id=' + spec_arr+ '&goods_id=' + goods_id, shows_number, 'GET', 'JSON');
		changePriceAll();
	...
	for(zzz=0; zzz<mylist.length; zzz++)
	{
		if(mylist[zzz].className!='wuxiao')
		{
				mylist[zzz].onclick=function(){
					show_attr_status(this, goods_id, attr_id_www_yshop100_com);
				}
		}
	...
	document.getElementById(the_input_id).checked="checked";
	 ...
	 {
	     selid_www_yshop100_com=0;
	     if (spec_attr_type[iii].value != www_yshop100_com)
	     {
		...
		{	
			...
			{					
				if (selid_www_yshop100_com)
				{
					if (s1_list[jjj].className == 'click')
					{
						selid_www_yshop100_com = s1_a_id;
					}
				}
				else
				{
			...
			$('#'+theid.id).siblings().removeClass('click');
			$('#'+theid.id).addClass('click');
				
			document.getElementById('spec_value_' + s1_a_id).checked = false;
		}
		document.getElementById('spec_value_' + selid_www_yshop100_com).checked = "checked";
		selected_first.push(selid_www_yshop100_com);
		document.getElementById('xuan_a_'+selid_www_yshop100_com).className='click';
		//document.getElementById('xuan_a_'+selid_www_yshop100_com).onclick=function(){}		
	     }
	}
	changeAtt(theid, goods_id);
	if (theid.parentNode.id.replace('catt_','')==attr_id_www_yshop100_com)
	{
 ...
 function is_exist_prod(selected_first, id, prod_exist_arr)
 {
	...
	var www_yshop100_com_selected = selected_first.slice(0);
	www_yshop100_com_selected.push(id);	
	var all_valid =0;
	for (var i in prod_exist_arr)
	{
		var first_exist=1;
		for (var j in www_yshop100_com_selected)
		{
			if ( prod_exist_arr[i].indexOf("|" + www_yshop100_com_selected[j] + "|") =='-1')
			{
				first_exist=0;
				break;
			}
