...
/* 代码增加_start   */
$sql_attr_www_yshop100_com="SELECT a.attr_id, ga.goods_attr_id FROM ". $GLOBALS['ecs']->table('attribute') . " AS a left join ". $GLOBALS['ecs']->table('goods_attr') . "  AS ga on a.attr_id=ga.attr_id  WHERE a.is_attr_gallery=1 and ga.goods_id='" . $goods_id. "' order by ga.goods_attr_id ";
$goods_attr=$GLOBALS['db']->getRow($sql_attr_www_yshop100_com);
if($goods_attr){
...
if (!empty($_REQUEST['act']) && $_REQUEST['act'] == 'get_gallery_attr')
{
	...
	$gallery_list_www_yshop100_com=get_goods_gallery_attr_www_yshop100_com($goods_id, $goods_attr_id);
	$gallery_content=array();
	$gallery_content['thumblist'] ='<ul>';
	foreach($gallery_list_www_yshop100_com as $gkey=>$gval)
	{
...
if (!$smarty->is_cached('goods-back.dwt', $cache_id))
{
...
/* 代码增加_end     */
 ...
 WHERE s.supplier_id=".$goods[supplier_id]." AND s.status=1";
             ...
             $smarty->assign('shopuserinfo', $shopuserinfo);
		 }
		/* 代码增加_end  By  www.yshop100.com */
		...
		$smarty->assign('pictures',            get_goods_gallery_attr_www_yshop100_com($goods_id, $goods_attr_id)); // 商品相册_修改  
		$smarty->assign('new_goods',           get_recommend_goods('new'));     // 最新商品  改  
//        $smarty->assign('bought_goods',        get_also_bought($goods_id));                      // 购买了该商品的用户还购买了哪些商品
        $smarty->assign('bought_goods',       get_recommend_goods('hot'));     // 热点文章
//        print_r(get_recommend_goods('hot'));
...
$smarty->display('goods-back.dwt',      $cache_id);
 ...
 * @return  array
 */
function get_goods_gallery_attr_www_yshop100_com($goods_id, $goods_attr_id)
{
