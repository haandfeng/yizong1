...
//代码增加 	
    ...
    //商品列表
    $count = get_all_supplier_goods_count($suppid, $brand, $price_min, $price_max);
    $max_page = ($count> 0) ? ceil($count / $size) : 1;
    if ($page > $max_page)
    {
        $page = $max_page;
    }
    $goodslist = get_all_supplier_goods($suppid, $brand, $price_min, $price_max, $size, $page, $sort, $order);
    if($display == 'grid')
    {
    ...
    assign_pager('supplier',            $cat_id, $count, $size, $sort, $order, $page, '', $brands, $price_min, $price_max, $display, $suppid); // 分页
...
function get_all_supplier_goods_count($supplier_id, $brand, $min, $max)
{
    $where = " g.is_on_sale = 1 AND g.is_alone_sale = 1 AND ".
            "g.is_delete = 0 AND g.supplier_id = $supplier_id and ispchide=0 and g.goods_id < 1529 ";
...
function get_all_supplier_goods($supplier_id, $brand, $min, $max,  $size, $page, $sort, $order)
{
    ...
    if($supplier_id == 0){
        $sql = 'SELECT distinct g.goods_id, g.goods_name, g.goods_name_style, g.market_price, g.is_new, g.is_best, g.is_hot, g.shop_price AS org_price, ' .
                ...
                'g.promote_start_date, g.promote_end_date, g.goods_brief, g.goods_thumb ,g.original_img, g.goods_img, g.supplier_id, sup.supplier_name ' .
