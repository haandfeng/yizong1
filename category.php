...
$display  = (isset($_REQUEST['display']) && in_array(trim(strtolower($_REQUEST['display'])), array('list', 'grid', 'text'))) ? trim($_REQUEST['display'])  : (isset($_COOKIE['ECS']['display']) ? $_COOKIE['ECS']['display'] : $default_display_type);
	...
	if($ishide=='1')
	{
		$where1=' and ((ishide=0 and g.supplier_id=0) or (ishide=1 and g.supplier_id>0)) and ispchide=0 ';
	}
	
			$sql = "SELECT min(g.shop_price) AS min, max(g.shop_price) as max ".
				   " FROM " . $ecs->table('goods'). " AS g ".
				   " WHERE ($children OR " . get_extension_goods($children) . ') AND g.is_delete = 0 AND g.is_on_sale = 1 AND g.is_alone_sale = 1  '.$where1;
				   //获得当前分类下商品价格的最大值、最小值
		...
		$sql = "SELECT b.brand_id, b.brand_name, b.brand_logo, COUNT(*) AS goods_num ".
				...
				" AND g.is_on_sale = 1 AND g.is_alone_sale = 1 AND g.is_delete = 0 ".$where1.
				"GROUP BY b.brand_id HAVING goods_num > 0 ORDER BY b.sort_order, b.brand_id ASC"; //此SQL语句增加字段 b.brand_logo,  By  www.yshop100.com
		...
		$brand_have_logo = 0;  //代码增加   By    www.yshop100.com
		...
		foreach ($brands AS $key => $val)
		{
			...
			$brands[$temp_key]['brand_id_yshop100'] = $val['brand_id'];
		...
		$smarty->assign('brand_zimu_yshop100', $brand_zimu);
		...
		{
		$brands_wwwyshop100Com=array();
		$brands_wwwyshop100Com[0] = $brands[0];
		$brands_qq=explode(',' , $cat['brand_qq']);
		foreach($brands_qq as $key_qq => $brand_qq)
		{
			foreach($brands as $key_wwwyshop100com=>$brand_yshop100com)
			{
				if($brand_yshop100com['brand_name']==$brand_qq)
				{
					$brands_wwwyshop100Com[]=$brand_yshop100com;
					break;
				}
			}
		}
		$brands=$brands_wwwyshop100Com;
		$smarty->assign('brands', $brands);
		}
		/* 代码增加_end      */
		...
		if ($cat['filter_attr'] > 0)
		{
			...
			foreach ($filter_attr AS $fkey=>$fval)
			{
				if ($fval)
				{
					...
					$temp_arrt_url_yshop100 =implode(".", $filter_attr_temp);
					if (strstr($fval, "_"))
					{
						$fval_array = explode("_", $fval);
						foreach ($fval_array AS $fval_key=> $fval_yshop100)
						{
							$cond_name .= ($fval_key ? "，" : "") . $filter_attr_name[$fkey][$fval_yshop100];
						}
					}
					else
					{
					...
					$condition[]=array(
							...
							'cond_name' =>$cond_name,
							'cond_url' =>build_uri('category', array('cid'=>$cat_id, 'bid'=>$brand, 'price_min'=>$price_min, 'price_max'=>$price_max, 'filter_attr'=>$temp_arrt_url_yshop100, 'filter'=>$filter), $cat['cat_name'])
						);
				}
			}
			$attr_group_more_txt = "";
			$attr_group_more_id =0;
			foreach ($all_attr_list AS $k_yshop100=>$k_value)
			{
				if ($k_yshop100 >1)
				{
		 ...
		 if($cat['attr_wwwyshop100com'])
		 {
			 $attr_wwwyshop100com=explode("\r\n", $cat['attr_wwwyshop100com']);
			 $attr_def_wwwyshop100com=array();
			 foreach($attr_wwwyshop100com as $attr_yshop100com)
			 {
				if ($attr_yshop100com)
				 {
						$attr_qq = explode(":", $attr_yshop100com);
						$attr_def_wwwyshop100com[$attr_qq[0]]=explode(",", $attr_qq[1]);
				}
			 }

			foreach ($all_attr_list as $key_attr_qq => $attr_old_yshop100com)
			{
				foreach($attr_def_wwwyshop100com as $key_def_yshop100com => $attr_def_yshop100com)
				{
					if ( $attr_old_yshop100com['filter_attr_name'] == $key_def_yshop100com)
					{
						 $attr_list_qq = array();
						 $attr_list_qq[0]= $attr_old_yshop100com['attr_list'][0];
						 foreach($attr_def_yshop100com as  $attr_new_yshop100com)
						 {
								foreach($attr_old_yshop100com['attr_list'] as $attr_temp_qq)
								{
									if($attr_temp_qq['attr_value'] == $attr_new_yshop100com )
									{
...
function category_get_goods($children, $brand, $min, $max, $ext, $size, $page, $sort, $order, $is_stock = 0)
{
    ...
    $where = "g.is_on_sale = 1 AND g.is_alone_sale = 1 ".$where1."AND ".
    ...
    $sql = "SELECT g.goods_id, g.goods_name, g.goods_name_style, g.click_count, g.goods_number, g.market_price, " .
		   ...
		   " g.promote_start_date, g.promote_end_date, g.goods_brief, g.goods_thumb, g.goods_img, g.supplier_id, sup.supplier_name " .
		   ...
		   " AS sup " .
           " ON sup.supplier_id = g.supplier_id " .
...
{
	$filter = (isset($_REQUEST['filter'])) ? intval($_REQUEST['filter']) : 0;
    $where  = "g.is_on_sale = 1 AND g.is_alone_sale = 1 AND g.is_delete = 0 AND ($children OR " . get_extension_goods($children) . ') and ispchide=0 ';
