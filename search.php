...
define('IN_ECS', true);
/* 代码添加_START  By   www.yshop100.com */
...
if (empty($_GET['encode']))
{
	...
	if($keywords!='')
	{
		...
		$sql = "select count(id) from ecs_keyword_log where word='".$keywords."'";
		$res = $GLOBALS['db']->getOne($sql);  
		if($res)
		{
			if(intval($res)>0)
			{
				echo '<script>alert("检测到屏蔽词,请重新输入！");history.go(-1);</script>';
				exit;
			}
		}  
	}
...
else
{
    ...
    if (!empty($_REQUEST['keywords']))
    {
		...
		include_once('includes/lib_splitword_www_yshop100_com.php');
		$Recordkw = str_replace(array("\'"), array(''), trim($_REQUEST['keywords']));

		$cfg_soft_lang_www_yshop100_com = 'utf-8';

        $sp_www_yshop100_com = new SplitWord($cfg_soft_lang_www_yshop100_com, $cfg_soft_lang_www_yshop100_com);
        $sp_www_yshop100_com->SetSource($Recordkw, $cfg_soft_lang_www_yshop100_com, $cfg_soft_lang_www_yshop100_com);

        $sp_www_yshop100_com->SetResultType(1);
        $sp_www_yshop100_com->StartAnalysis(TRUE);

        //$word_www_yshop100_com = $sp_www_yshop100_com->GetFinallyResult(' ');
        $word_www_yshop100_com = $sp_www_yshop100_com->GetFinallyResult('');

		//echo  $word_www_yshop100_com;
        $word_www_yshop100_com = preg_replace("/[ ]{1,}/", " ", trim($word_www_yshop100_com));

        $replacef_www_yshop100_com = explode(' ', $word_www_yshop100_com);
	...
	if($ishide=='1')
	{
		 //$where1=' and ((ishide=0 and g.supplier_id=0) or (ishide=1 and g.supplier_id>0)) and ispchide=0 and g.goods_id < 1529 ';
	}
        ...
        foreach ($replacef_www_yshop100_com AS $key => $val)
        {
            if ($key > 0 && $key < count($replacef_www_yshop100_com) && count($replacef_www_yshop100_com) > 1)
            {
                $keywords .= " OR ";
            }
            ...
            $keywords  .= "(goods_name LIKE '%$val%' OR goods_sn LIKE '%$val%' OR keywords LIKE '%$val%' $sc_dsad)".$where1;
   ...
   /* fulltext_search_add_START_www.yshop100.com */
    if($_CFG['fulltext_search'] == '0'){
   /* fulltext_search_add_END_www.yshop100.com */
    /* 获得符合条件的商品总数 */
    ...
    $count = $db->getOne($sql);
	/* 代码添加_START   By  www.yshop100.com  */	
    if ($page == 1 && $Recordkw && $count && !$category) Recordkeyword($Recordkw, $count, 'ecshop');  // 代码修改  By   www.yshop100.com
	if($count==0)
	{
		if (preg_match('/^[a-zA-z]+$/i', $_REQUEST['keywords']))
		{
			$sql_www_yshop100_com = "select keyword from ". $ecs->table('keyword') ." where letter='". $_REQUEST['keywords'] ."' order by items desc limit 0,1";
			$keyword_www_yshop100_com = $db->getOne($sql_www_yshop100_com);
			if($keyword_www_yshop100_com && $keyword_www_yshop100_com!=$_REQUEST['keywords'])
			{
				header('Location: search.php?keywords='.$keyword_www_yshop100_com.'&keyword_zm='.$_REQUEST['keywords']);
				exit;
			}
		}
	}
	...
	{
		$smarty->assign('beizhuxinxi_www_yshop100_com', '关键词<font color=#cc0000>'.$_REQUEST['keyword_zm'].'</font>搜索结果为零，<br>但是我们为您匹配到了相关关键词<font color=#cc0000>'.$_REQUEST['keywords'].'</font>，下面是它的查询结果！');
	}
	/* 代码添加_END   By  www.yshop100.com  */
...
//	$ts_string_array = array("*","/");
    ...
    $sql = "SELECT g.goods_id, g.goods_name, g.market_price, g.click_count, g.goods_number, g.is_new, g.is_best, g.is_hot, g.shop_price AS org_price, ".
                ...
                "g.promote_price, g.promote_start_date, g.promote_end_date, g.goods_thumb, g.goods_img, g.goods_brief, g.goods_type, g.supplier_id, sup.supplier_name ".
            "FROM " .$ecs->table('goods'). " AS g ".
            "LEFT JOIN " . $GLOBALS['ecs']->table('member_price') . " AS mp ".
                    "ON mp.goods_id = g.goods_id AND mp.user_rank = '$_SESSION[user_rank]' ".
            "LEFT JOIN " . $GLOBALS['ecs']->table('supplier') . " AS sup ".
    ...
    if($_CFG['fulltext_search'] == '1'){
		...
		if(empty($idarray)){
			$idarray = array();
				foreach($replacef_www_yshop100_com as $key => $value){
    ...
    while ($row = $db->FetchRow($res))
    {
		...
		$arr[$row['goods_id']]['goods_name'] = $row['goods_name'];
		/* 代码添加_START   By    www.yshop100.com */
		$arr[$row['goods_id']]['goods_name_www_yshop100_com'] =  $arr[$row['goods_id']]['goods_name'];
		foreach($replacef_www_yshop100_com as $key_www_yshop100_com =>$temp_qq)
		{
				//$replacet[$key_www_yshop100_com]=  '<strong style="color:#cc0000;">'. $temp_qq .'</strong>';;
				
				if(in_array($temp_qq,$ts_string_array)){
					$arr[$row['goods_id']]['goods_name_www_yshop100_com'] = preg_replace('/(?!<[^>]*)\\'.$temp_qq.'(?![^<]*>)/i', '<strong style="color:#cc0000;">'. $temp_qq .'</strong>', $arr[$row['goods_id']]['goods_name_www_yshop100_com']);
				}else{
					$arr[$row['goods_id']]['goods_name_www_yshop100_com'] = preg_replace('/(?!<[^>]*)'.$temp_qq.'(?![^<]*>)/i', '<strong style="color:#cc0000;">'. $temp_qq .'</strong>', $arr[$row['goods_id']]['goods_name_www_yshop100_com']);
				}
        ...
        $arr[$row['goods_id']]['supplier_name'] = $row['supplier_name'];
...
//    }
    ...
    $smarty->assign('promotion_info', get_promotion_info());
    $smarty->assign('cate_img1',get_adv('分类列表-banner图'));
