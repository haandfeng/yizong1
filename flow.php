...
elseif ($_REQUEST['act']=='saveAddress')
{
	...
	$consignee = array(
            ...
            'country'       => empty($address_yshop100->country)    ? '1' :   intval($address_yshop100->country),
...
function get_consignee_list_yshop100()
{
	...
	foreach ($consignee_list_yshop100  as $cons_key => $cons_val)
	{
				...
				$consignee_list_yshop100[$cons_key]['address_short_name'] .=  "<br>".(empty($cons_val['mobile']) ? $cons_val['tel'] : $cons_val['mobile']);
...
if ($_REQUEST['step'] == 'add_to_cart')
{
	...
	//www.yshop100.com start add 2015-3-26
	$time_xg_now=gmtime();
	...
	//www.yshop100.com end add 2015-3-26
...
/* 代码增加_end  By www.yshop100.com */
    ...
    else
    {
        ...
        elseif (!empty($_POST['act']) && $_POST['act'] == 'signup')
        {
            ...
            {
                /* 用户注册成功 */
				/* 四合一修改 start by www.yshop100.com */
                ecs_header("Location: flow.php?step=checkout\n");
				/* 四合一修改 end by www.yshop100.com */
                exit;
            }
            else
            {
...
elseif ($_REQUEST['step'] == 'checkout')
{
   ...
   // else if($_SESSION['extension_code'] == 'virtual_good')
    ...
    {
        /* 如果不完整则转向到收货人信息填写界面 */
		/* 代码修改_start  By  www.yshop100.com  注释或删掉  */
        //ecs_header("Location: flow.php?step=consignee\n");   
        //exit;
   ...
   $smarty->assign('shop_province_list', get_regions(1, $_CFG['shop_country']));
    ...
    $payment_list = available_payment_list(1, $cod_fee, false, $_SESSION['extension_code']=='virtual_good'?1:0, $suppliers_id);
    ...
    if(isset($payment_list))
    {
        foreach ($payment_list as $key => $payment)
        {
            ...
            /* 不显示旧版微信支付 */
            if ($payment['pay_code'] == 'weixin' || $payment['pay_code'] == 'weixin_cn')
            {
                unset($payment_list[$key]);
            }
        }
   ...
   // unset($payment_list[2]);
    /*unset($payment_list[3]);
   $payment_list[4]['pay_code'] = 'weixin';*/
    /*echo '<pre>';
...
elseif ($_REQUEST['step'] == 'select_payment')
{
    ...
    $cart_goods = cart_goods($flow_type); // 取得商品列表，计算合计
	//echo '<pre>';
	//print_r(check_consignee_info($consignee, $flow_type));exit;
    if (empty($cart_goods) )
    {
        $result['error'] = $_LANG['no_goods_in_cart'];
    }
    ...
    else
    {
        ...
        /*余额额支付密码_添加_END_www.yshop100.com*/
...
elseif ($_REQUEST['step'] == 'change_surplus')
{
    ...
    else
    {
        ...
        else
        {
            ...
            /* 代码修改 By  www.yshop100.com #1305 Start */
            //if($total['amount_formated'] <= 0){
            if($total['amount'] <= 0)
            {
            /* 代码修改 By  www.yshop100.com #1305 End */
    ...
    /* 代码修改 By  www.yshop100.com #1854 Start */
    $result['surplus'] = round($result['surplus'] ,2);
    /* 代码修改 By  www.yshop100.com #1854 End */
    $json = new JSON();
    die($json->encode($result));
}
...
{	
    ...
    foreach ($cart_goods_new as $ckey=>$cval){
    	...
    	$order['hkd_goods_amount'] = $order['goods_amount_exchange'];
	    ...
	    $order['hkd_order_amount'] = $order['exchange_amount'];
	...
	if ($order['order_amount'] > 0)
    {
        ...
        {
            $pay_config['payment_inst'] = $order['pay_id'] == 9 ? 'ALIPAYCN' : 'ALIPAYHK';
        }
		...
		RecordsFile( ROOT_PATH.'data/logs/','order-logs-paybutton-'.date('Y-m-d').'.txt',[$pay_online]);
...
{
	$sql_hot = "SELECT g.goods_id, g.goods_name, g.goods_thumb, g.shop_price, g.market_price, ss.suppliers_desc from " . $GLOBALS['ecs']->table('goods'). " AS g LEFT JOIN " . $GLOBALS['ecs']->table('suppliers'). " AS ss ON g.suppliers_id = ss.suppliers_id WHERE ". $rtype ."=1 order by  goods_id desc limit 0,10 ";
...
if ($_REQUEST['step']=='cart' || $_REQUEST['step']=='checkout')
{
	$smarty->assign('template_dir', $GLOBALS['_CFG']['template']);
        if(isset($total['real_goods_count']) && $total['real_goods_count'] == 0){
            ...
            //$smarty->display('flow_jm_test.dwt');
...
{
   /* 代码修改_start  By  www.yshop100.com  将这块替换掉*/
