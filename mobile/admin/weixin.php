	...
	case "addmenu"://添加菜单
		...
		if($_POST){
			...
			if($id){
				...
				$ret = $db->query ( "INSERT INTO " . $GLOBALS['ecs']->table('weixin_menu') . " (`pid`, `name`, `type`, `value`, `text`, `order`)
					VALUES('$pid', '$name', $type, '$value', '$text', '$order');" );
				$id = $db->insert_id();
				if($type == 4){
					$value = "text_".$id;
					$db->query ( "UPDATE " . $GLOBALS['ecs']->table('weixin_menu') . " SET value = '$value' WHERE id = '$id'");
				}
			}
			update_menu();
			if ($ret) {
