...
$exc_order   = new exchange($ecs->table("ecsmart_ad"), $db, 'ad_id', 'order');
...
elseif ($_REQUEST['act'] == 'insert')
{
    ...
    if ($_POST['media_type'] == '0')
    {
        ...
        {
            //限制图片的宽度和高度
            list($width, $height) = getimagesize($_FILES['ad_img']['tmp_name']);
...
//-- 编辑广告排序
/*------------------------------------------------------ */
elseif ($_REQUEST['act'] == 'edit_ad_order')
{
    check_authz_json('ad_manage');
    ...
    if ($exc_order->edit("`order` = '$order'", $id))
    {
        admin_log($order,'edit','ads');
...
function get_adslist()
{
    ...
    /* 获得广告数据 */
    $arr = array();
    $sql = 'SELECT ad.*, COUNT(o.order_id) AS ad_stats, p.position_name , ad.order '.
