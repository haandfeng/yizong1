...
}elseif ($_REQUEST['act'] == 'apply_list'){
    $sql = "SELECT rank_id, rank_name, min_points FROM ".$ecs->table('user_rank')." ORDER BY min_points ASC ";
    $rs = $db->query($sql);
...
elseif ($_REQUEST['act'] == 'update')
{
    ...
    if($status != 1){
     ...
     $sql = "update ".$ecs->table('users')." set `real_name`='$real_name',`card`='$card',`country`='$country',`province`='$province',`city`='$city',`district`='$district',`address`='$address',`is_fenxiao` = '$is_fenxiao' ,`status`='$status' ".$con."  where user_name = '".$username."'";
    $db->query($sql);
...
function user_list()
{
    ...
    if ($result === false)
    {
        ...
        $ex_where = ' WHERE 1 ';
        $ex_where .= " AND is_fenxiao = 1 AND status = 1";
        ...
        $sql = "SELECT user_id, user_name, email, real_name, mobile_phone, is_validated, validated,status,user_money, frozen_money, rank_points, pay_points, reg_time,is_fenxiao,parent_id,invite_code ".
    ...
    for ($i=0; $i<$count; $i++)
    {
        ...
        $user_list[$i]['apply_mobile'] = $user_list[$i]['mobile_phone'];
        $user_list[$i]['apply_name'] = $user_list[$i]['real_name'];
        $user_list[$i]['remark'] = get_fenxiao_remark($user_list[$i]['user_id']);
        //已消费订单总额
        $sql = "SELECT SUM(money_paid) AS total_amount FROM " . $GLOBALS['ecs']->table('order_info') . " WHERE (order_status = '1' OR order_status = '5') AND user_id =" .$user_list[$i]['user_id'];
        $row = $GLOBALS['db']->getRow($sql);
        $user_list[$i]['total_amount'] = $row['total_amount'] > 0 ? $row['total_amount'] : '0.00';
    }
...
function fx_user_list()
{
    ...
    if ($result === false)
    {
        ...
        $ex_where .= " AND is_fenxiao = 1 AND status = 2";
        ...
        $sql = "SELECT user_id, user_name, email, is_validated, validated,status,user_money, frozen_money, rank_points, pay_points, reg_time,is_fenxiao,parent_id,invite_code ".
            " FROM " . $GLOBALS['ecs']->table('users') . $ex_where .
            " ORDER by " . $filter['sort_by'] . ' ' . $filter['sort_order'] .
            " LIMIT " . $filter['start'] . ',' . $filter['page_size'];
    ...
    for ($i=0; $i<$count; $i++)
    {
        ...
        $row = $GLOBALS['db']->getRow($sql);
        $user_list[$i]['invite_qrcode'] = $row['qcode'];
        $user_list[$i]['apply_mobile'] = !empty($user_list[$i]['mobile_phone']) ? $user_list[$i]['mobile_phone'] : get_fenxiao_apply_mobile($user_list[$i]['user_id']);
        $user_list[$i]['apply_name'] = !empty($user_list[$i]['real_name']) ? $user_list[$i]['real_name'] : get_fenxiao_apply_name($user_list[$i]['user_id']);
        $user_list[$i]['remark'] = get_fenxiao_apply_remark($user_list[$i]['user_id']);
        $user_list[$i]['add_time'] = local_date($GLOBALS['_CFG']['date_format'],get_fenxiao_apply_addtime($user_list[$i]['user_id']));
        //已消费订单总额
        $sql = "SELECT SUM(money_paid) AS total_amount FROM " . $GLOBALS['ecs']->table('order_info') . " WHERE (order_status = '1' OR order_status = '5') AND user_id =" .$user_list[$i]['user_id'];
        $row = $GLOBALS['db']->getRow($sql);
        $user_list[$i]['total_amount'] = $row['total_amount'] > 0 ? $row['total_amount'] : '0.00';
...
function get_fenxiao_apply_name($user_id)
{
    $sql = "SELECT real_name FROM " . $GLOBALS['ecs']->table('fenxiao_apply') . " WHERE user_id = '$user_id'";
    return $GLOBALS['db']->getOne($sql);
}

function get_fenxiao_apply_mobile($user_id)
{
    $sql = "SELECT mobile FROM " . $GLOBALS['ecs']->table('fenxiao_apply') . " WHERE user_id = '$user_id'";
    return $GLOBALS['db']->getOne($sql);
}

function get_fenxiao_apply_remark($user_id)
{
    $sql = "SELECT remark FROM " . $GLOBALS['ecs']->table('fenxiao_apply') . " WHERE user_id = '$user_id'";
    return $GLOBALS['db']->getOne($sql);
}

function get_fenxiao_apply_addtime($user_id)
{
    $sql = "SELECT add_time FROM " . $GLOBALS['ecs']->table('fenxiao_apply') . " WHERE user_id = '$user_id'";
    return $GLOBALS['db']->getOne($sql);
}

function get_fenxiao_remark($user_id)
{
    $sql = "SELECT isagree, agreetime, real_name, bank_card, bank_name, card, mobile_phone FROM " . $GLOBALS['ecs']->table('user_fenxiao_period') . " WHERE user_id = '$user_id'";
    $row = $GLOBALS['db']->getRow($sql);

    if($row){
        $result .= !empty($row['real_name']) ? ' 收款人姓名：'.$row['real_name'] : '';
        $result .= !empty($row['card']) ? ' 身份证：'.$row['card'] : '';
        $result .= !empty($row['bank_name']) ? ' 银行编号：'.$row['bank_name'] : '';
        $result .= !empty($row['bank_card']) ? ' 银行卡账号：'.$row['bank_card'] : '';
        $result .= !empty($row['agreetime']) ? ' 协议签订日期：'.date("Y-m-d", strtotime($row['agreetime'])) : '';
    }
    return $result;
}
