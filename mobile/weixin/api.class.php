...
class weixinapi{
	...
	function sendBonus($wxid,$type){
		...
		if($uid){
			...
			//$GLOBALS['db']->query($sql);file_put_contents('bonus0209.txt', 'a-'.$sql,FILE_APPEND);
			$flag=false;
			//if($wxid=='o9buKwpBHSG9geH4DfkhcfKzujkk')
			//{
				//file_put_contents('bonus0209.txt', '0-'.$uid.'-'.$wxid,FILE_APPEND);
				$now = gmtime();
				$sql_b = "SELECT type_id,send_maxcount,sentcount_0 FROM " . $GLOBALS['ecs']->table("bonus_type") . " WHERE send_type='5'  AND use_start_date<=" . $now . " AND use_end_date>=" . $now;
				$res_bonus = $GLOBALS['db']->query($sql_b);
				while($row_bonus = $GLOBALS['db']->fetchRow($res_bonus))
				{
					if(intval($row_bonus['send_maxcount'])>0&&intval($row_bonus['sentcount_0'])<intval($row_bonus['send_maxcount']))
					{ 
						$sql = "INSERT INTO " . $GLOBALS['ecs']->table('user_bonus') . "(bonus_type_id, bonus_sn, user_id, used_time, order_id, emailed)" . " VALUES('" . $row_bonus['type_id'] . "', 0, '" . $uid . "', 0, 0, 0)";
						$GLOBALS['db']->query($sql);
						$flag=true;
					}
					...
					$sql_c = "SELECT count(user_id) as c FROM " . $GLOBALS['ecs']->table("user_bonus") . " WHERE `bonus_type_id`=" . $row_bonus['type_id'] . "" ;
					$res_bonuscount = $GLOBALS['db']->getOne($sql_c);
					if(intval($res_bonuscount)<>intval($row_bonus['sentcount_0']))
					{
						$sql="update " . $GLOBALS['ecs']->table("bonus_type") . " set sentcount_0=".$res_bonuscount." WHERE type_id=".$row_bonus['type_id']."";
						$GLOBALS['db']->query($sql);
					}
				}
			//}
			...
			//关注红包
			$bonus_sn = $GLOBALS['db']->getOne("SELECT bonus_sn FROM " . $GLOBALS['ecs']->table('user_bonus') . " where bonus_type_id={$type} and used_time=0 and emailed=0");file_put_contents('bonus0209.txt', 'b-'.$bonus_sn,FILE_APPEND);
			if($bonus_sn){
				$GLOBALS['db']->query("update ".$GLOBALS['ecs']->table('user_bonus')." set emailed=1 where bonus_sn='{$bonus_sn}'");file_put_contents('bonus0209.txt', "c-"."update ".$GLOBALS['ecs']->table('user_bonus')." set emailed=1 where bonus_sn='{$bonus_sn}'",FILE_APPEND);
				return $bonus_sn;
			}
		}
		return false;
