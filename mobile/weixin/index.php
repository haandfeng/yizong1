...
//file_put_contents("test.txt", print_r($_GET.'\n',true), FILE_APPEND);
...
//file_put_contents("test.txt", print_r($data.'\n',true), FILE_APPEND);
//上报地理信息
...
//file_put_contents("test.txt", print_r($content.'\n',true), FILE_APPEND);
...
if ($event['event'] == "subscribe") { //用户关注	
	if(intval($followInfo['ecuid']) == 0 && $weixinconfig['reg_type'] == 1){
		...
		//未分销
			...
			$uid = $GLOBALS['db']->getOne($sql);
			$GLOBALS['db']->query("UPDATE ".$GLOBALS['ecs']->table('users')." SET is_fenxiao = 0 WHERE user_id = '" . $uid . "'");
	}
	//取消关注再重新关注不送红包
...
if ($event['event'] == "CLICK"){
	...
	if(strpos($content,"text_") !== false){
		$id = str_replace('text_','',$content);
		$sql = "SELECT `text` FROM " . $GLOBALS['ecs']->table('weixin_menu') . "WHERE id = '$id'";
		$text = $db->getOne($sql);
		echo $weixin->text($text)->reply();exit;
	}
