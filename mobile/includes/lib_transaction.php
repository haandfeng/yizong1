...
function edit_profile($profile)
{
    ...
    if (isset($profile['shipscancode']))
    { 
		$GLOBALS['db']->query("update " . $GLOBALS['ecs']->table('users') . " set shipscancode='".$profile['shipscancode']."' WHERE user_id='" . $profile['user_id'] . "'");
    }
    ...
    /* 过滤非法的键值 */
    $other_key_array = array('user_name','msn', 'qq', 'office_phone', 'home_phone', 'mobile_phone','shipscancode');
...
function get_profile($user_id)
{
    ...
    $info['bonus']            = $bonus;
    $info['shipscancode']               = $infos['shipscancode'];
    $info['qq']               = $infos['qq'];
    $info['msn']              = $infos['msn'];
...
function add_onuser_bonus($user_id, $type_id)
{
    if (empty($user_id))
    {
        $GLOBALS['err']->add($GLOBALS['_LANG']['not_login']);
    ...
    if ($row)
    {
		$max=$row['user_bonus_max'];
		$sql = "SELECT count(*) FROM " .$GLOBALS['ecs']->table('user_bonus') .
			   " WHERE bonus_type_id = '$type_id' and user_id='$user_id' ";
		$getcount = $GLOBALS['db']->getOne($sql);
		if($max>$getcount || $max==0) 
        { 
            $sql = "INSERT INTO " .$GLOBALS['ecs']->table('user_bonus') . "( bonus_type_id, bonus_sn, user_id, used_time, order_id, emailed, supplier_id)select type_id,0,'$user_id',0,0,0,supplier_id  from ecs_bonus_type where type_id='$type_id' ";
            $result = $GLOBALS['db'] ->query($sql);
            if ($result)
            {
                 return true;
...
function get_user_orders_ajax($user_id, $limit,$where='')
{
    ...
    $sql = "SELECT o.*, ifnull(ssc.value,'网站自营') as shopname, " .
           "(goods_amount + shipping_fee + insure_fee + pay_fee + pack_fee + card_fee + tax - discount) AS total_fee , ss.is_exchange".
...
//        print_r($goods_list_r);
		...
		$arr[$row['order_id']] = array('order_id'       => $row['order_id'],
                        ...
                        'exchange_total_fee'      => price_exchange_format($row['total_fee']),
						'goods_list'     => $goods_list_r,
						'pay_online'       => $row['pay_online'],
						'is_suborder' => $row['parent_order_id'] ? "(子订单)" : "",  //代码增加   By  www.yshop100.com
...
function get_order_goods($order)
{
    ...
    $goods_list = array();
    $goods_attr = array();
    $sql = "SELECT o.*, IF(o.product_id > 0, p.product_number, g.goods_number) AS storage, o.goods_attr, o.goods_attr_id, g.suppliers_id, IFNULL(b.brand_name, '') AS brand_name, p.product_sn, a.attr_value,g.goods_thumb,g.goods_id,ss.is_exchange 
    ...
    while ($row = $GLOBALS['db']->fetchRow($res))
    {
        ...
        $row['exchange_formated_subtotal']       = price_exchange_format($row['goods_price'] * $row['goods_number']);
        $row['exchange_formated_goods_price']    = price_exchange_format($row['goods_price']);
...
function get_order_detail($order_id, $user_id = 0)
{
        ...
        $order['order_status'] == OS_CONFIRMED))
    {
        ...
        else
        {
            ...
            {
                $payment['payment_inst'] = $order['pay_id'] == 9 ? 'ALIPAYCN' : 'ALIPAYHK';
            }
            ...
            $order['is_exchange_rate'] = $payment_info['is_exchange_rate'];
            ...
            /* 修改支付按钮 针对微信支付 支付宝*/
            if($order['pay_name'] == '支付宝' || $order['pay_name'] == '支付宝CN' || $order['pay_name'] == '支付宝HK'){
                $order['pay_online'] = '<div style="text-align:center"><input type="button" onclick="window.location.href=\''.$order['pay_online'].'\'"/></div>';
            }
            if($order['pay_name'] == '微信支付' || $order['pay_name'] == '微信支付CN' || $order['pay_name'] == '微信支付HK'){
                $order['pay_online'] = '<div style="text-align:center"><input type="button" onclick="window.location.href=\'/mobile/weixinpay.php?out_trade_no='.$order['log_id'].'\'"/></div>';
//                $order['pay_online'] = $order['pay_online'];
            }
        }
    }
    else
...
function get_new_bouns_list($user_id,$type_id=0)
{
    $sql = "SELECT * ".
           " FROM ".
           $GLOBALS['ecs']->table('bonus_type'). " AS b".
           " WHERE send_type = 6 AND send_start_date<=unix_timestamp() and unix_timestamp()<=send_end_date order by type_id desc";
	if($type_id>0)
	{
		$sql = "SELECT * ".
           " FROM ".
           $GLOBALS['ecs']->table('bonus_type'). " AS b".
           " WHERE send_type = 6 AND send_start_date<=unix_timestamp() and unix_timestamp()<=send_end_date and type_id='".$type_id."'";
	}
    $res = $GLOBALS['db']->query($sql);
    $arr = array();
    ...
    { 
		$type_id=$row['type_id'];
		//file_put_contents('20211221.txt',"type_id:".$type_id."\r\n" , FILE_APPEND);  
		$sqlone = "SELECT count(*) as c FROM ecs_user_bonus WHERE bonus_type_id='".$type_id."' and user_id='".$user_id."'";
  
		$hbcount = $GLOBALS['db']->getOne($sqlone);
		if($row['user_bonus_max']==0)
		{
			$row['status'] = '待领取';
		}
		else if($row['user_bonus_max']>$hbcount)
		{
			$row['status'] = '待领取';
		}
		else
