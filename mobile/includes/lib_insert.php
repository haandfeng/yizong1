...
function insert_ads($arr)
{
	...
	$arr['num'] = (isset($arr['num'])) ? intval($arr['num']) : $arr['num'];
    $arr['id'] = (isset($arr['id'])) ? intval($arr['id']) : $arr['id'];
    $time = gmtime();
    if (!empty($arr['num']) && $arr['num'] != 1)
    {
        $sql  = 'SELECT a.ad_id, a.position_id, a.media_type, a.ad_link, a.ad_code, a.ad_name, p.ad_width, ' .
                ...
                "WHERE enabled = 1 AND start_time <= '" . $time . "' AND end_time >= '" . $time . "' ".
                    "AND a.position_id = '" . intval($arr['id']) . "' " .
                'ORDER BY rnd LIMIT ' . intval($arr['num']);
    ...
    else
    {
        if ($static_res[$arr['id']] === NULL)
        {
            $sql  = 'SELECT a.ad_id, a.position_id, a.media_type, a.ad_link, a.ad_code, a.ad_name, p.ad_width, '.
                    ...
                    'FROM ' . $GLOBALS['ecs']->table('ecsmart_ad') . ' AS a '.
                    'LEFT JOIN ' . $GLOBALS['ecs']->table('ecsmart_ad_position') . ' AS p ON a.position_id = p.position_id ' .
                    "WHERE enabled = 1 AND a.position_id = '" . intval($arr['id']) .
...
$arr['type']=addslashes($arr['type']);
    ...
    $cmt = assign_comment(intval($arr['id']),          $arr['type']);
...
$arr['id'] = (isset($arr['id'])) ? intval($arr['id']) : $arr['id'];
    ...
    $sql = 'SELECT u.user_name, og.goods_number, oi.add_time, IF(oi.order_status IN (2, 3, 4), 0, 1) AS order_status ' .
           ...
           'WHERE oi.order_id = og.order_id AND ' . time() . ' - oi.add_time < 2592000 AND og.goods_id = ' . intval($arr['id']) . ' ORDER BY oi.add_time DESC LIMIT 5';
    ...
    $sql = 'SELECT count(*) ' .
           'FROM ' . $GLOBALS['ecs']->table('order_info') . ' AS oi LEFT JOIN ' . $GLOBALS['ecs']->table('users') . ' AS u ON oi.user_id = u.user_id, ' . $GLOBALS['ecs']->table('order_goods') . ' AS og ' .
           'WHERE oi.order_id = og.order_id AND ' . time() . ' - oi.add_time < 2592000 AND og.goods_id = ' . intval($arr['id']);
...
function insert_question($arr)
{
    ...
    $GLOBALS['smarty']->assign('email',        $_SESSION['email']);
    $GLOBALS['smarty']->assign('id',           intval($arr['id']));
    $cmt = assign_question(intval($arr['id']));
...
function insert_get_shop_shipping($arr){
    ...
    $suppliers_id = $arr['suppliers_id'];
    $suppliers_shipping_str = $db->getOne('SELECT shipping_ids FROM '. $ecs->table('suppliers') . ' WHERE suppliers_id = "'.$suppliers_id.'"');
    if($suppliers_shipping_str){
        $suppliers_shipping_ids = explode(",", trim($suppliers_shipping_str));
    }

	$shipping_list = available_shipping_list($region,$suppid,1,null,$suppliers_shipping_ids);
	...
	if(count($shipping_list)>0){
		...
		foreach($shipping_list as $key=>$val){
			...
			if($shipping_code == 'pups')
			{
				...
				if(empty($pickinfo) || $pickinfo == false || $suppliers_id == '2')//保税仓不能自提
				{
...
function insert_add_url_uid(){
    ...
    $url = str_replace('//', '/', $url);
    if($user_id && strpos($url,'/mobile/brand.php') === FALSE)
    {
        ...
        //{
        //    if(strstr($url, '?')){
        //        $url = 'https://'.str_replace('?', '?u='.$user_id.'&', $url);
        //    }
        //    else
        //    {
        //        $url = 'https://'.$url.'?u='.$user_id;
        //    }
