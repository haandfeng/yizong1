...
function get_cart_goods($other='')
{
    ...
    /* 循环、统计 */
	$sql_where = $_SESSION['user_id']>0 ? "c.user_id='". $_SESSION['user_id'] ."' " : "c.session_id = '" . SESS_ID . "' AND c.user_id=0 ";
	$sql = "SELECT c.*, g.cat_id, g.brand_id, c.extension_code, IF(ga.act_id, ga.supplier_id, g.supplier_id) as supplier_id, IF(c.parent_id, c.parent_id, c.goods_id) AS pid , IF(ga.act_id, ga.suppliers_id, g.suppliers_id) AS suppliers_id" .
...
function get_pickup_info($cityid,$suppid=0){
	global $db,$ecs;
	$sql = "select pp.*,r.region_name from ".$ecs->table('pickup_point')." as pp left join ".$ecs->table('region')." as r on pp.district_id=r.region_id where pp.supplier_id=".$suppid." order by pp.id";
	$info = $db->getAll($sql);
	return $info;
}
/**
