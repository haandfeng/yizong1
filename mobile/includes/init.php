...
if(!empty($_COOKIE['accesstoken'])){
	...
	$sql = "select user_id,user_name from " . $ecs->table('users') . " where accesstoken='$accesstoken' ";
...
if (!defined('INIT_NO_SMARTY'))
{
	...
	{
	$smarty->template_dir   = ROOT_PATH . 'themesmobile/yshop100com_mobile';
...
}else
{
	...
	$comefrom= isset($_REQUEST['comefrom'])?trim($_REQUEST['comefrom']):'';
	...
	$is_h5_app = 0;
	$is_h5_login = 0;
	if(!empty($_SESSION['user_id']) && !empty($_SESSION['comefrom'])){
		$is_h5_login = 1;
	}
	if(!empty($_SESSION['comefrom'])){
		$is_h5_app = 1;
	...
	echo '<br>';
	print_r($_SESSION);*/
	$smarty->assign('is_h5_app',$is_h5_app);
	$smarty->assign('is_h5_login',$is_h5_login);	
}
