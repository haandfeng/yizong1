   ...
   foreach ($res AS $row)
    {
        switch ($row['type'])
        {
            ...
            case 3:
                ...
                $sql = 'SELECT article_id as id,cat_id,author,title as short_title,author,add_time FROM ' . $GLOBALS['ecs']->table('article');
                $res = $GLOBALS['db']->getAll($sql);
                foreach($res as $k => &$v){
                    if($v['author'] == ""){
                        $v['author'] = "漢方杏林堂";
                    }
                    $v['url'] = 'article.php?id='.$v['id'];
                    if($v['cat_id'] == -1){
                        unset($res[$k]);
                    }
                }
                $cat_articles['arr'] = $res;
