 ...
 */
function available_shipping_list($region_id_list,$suppid=0,$is_default_show=1,$shipping_code=null,$suppliers_shipping_ids=0)
{
	$where = ' AND s.supplier_id='.$suppid.' and r.region_id ' . db_create_in($region_id_list);
	
    if($suppliers_shipping_ids == 0){
    ...
    if($suppliers_shipping_ids != 0){
        foreach ($suppliers_shipping_ids as $suppliers_shipping_id) {
            $where .= ' AND s.shipping_id = "'.$suppliers_shipping_id.'" ';
        }
    }
...
function getzhe()
{
	$sql="select value from ecs_shop_config where id=1070";
	$res= $GLOBALS['db']->getOne($sql);  
	if($res=='')
		$res='1';
	return $res;
}
function getzhemaxamount()
{
	$sql="select value from ecs_shop_config where id=1071";
	$res= $GLOBALS['db']->getOne($sql);    
	if($res=='')
		$res='1000';
	return $res;
}
function iszheconfig()
{
	$sql="select value from ecs_shop_config where id=1072";
	$res= $GLOBALS['db']->getOne($sql);    
	if($res=='1')
		return true;
	else 
...
function isbonded()
{
	$sql="select count(user_id) from ecs_users where user_id='".$_SESSION['user_id']."' and isBonded=-1";
	$isbonded=$GLOBALS['db']->getOne($sql); 
	if($isbonded>0)
	{
		return true;
	}
...
function checkbonded()
{
	if(iszheconfig())
	{
		$sql="SELECT card FROM `ecs_users` WHERE user_id='".$_SESSION['user_id']."' ";
		$card=$GLOBALS['db']->getOne($sql); 
		if($card<>'')
		{
			$sql="SELECT count(user_id) FROM `ecs_users` WHERE card='".$card."' and isBonded=1 ";
			$res=$GLOBALS['db']->getOne($sql); 
			if($res>0)
			{
				$sql="update ecs_users set isBonded=1 where card='".$card."' ";
				$GLOBALS['db']->query($sql); 
			}
			else
			{
				$sql="SELECT count(order_id) FROM `ecs_order_info` inner join ecs_users on  ecs_users.user_id=ecs_order_info.user_id WHERE suppliers_id=2 and order_status<>2 and order_status<>0 and card='".$card."' ";
				$res=$GLOBALS['db']->getOne($sql); 
				if($res>0)
				{
					$sql="update ecs_users set isBonded=1 where card='".$card."' ";
				...
				{
					$sql="SELECT count(order_id) FROM `ecs_order_info` inner join ecs_users on  ecs_users.user_id=ecs_order_info.user_id WHERE suppliers_id=2 and order_status=0 and card='".$card."' ";
					$res=$GLOBALS['db']->getOne($sql); 
					if($res>0)
					{
						$sql="update ecs_users set isBonded=0 where card='".$card."' ";
						$GLOBALS['db']->query($sql); 
					}
					else
					{
		...
		//$sql="SELECT count(user_id) FROM `ecs_users` WHERE isBonded=1 and user_id='".$_SESSION['user_id']."'";
		//$res=$GLOBALS['db']->getOne($sql); 
		//if($res==0)
		//{
		//	$sql="SELECT count(order_id) FROM `ecs_order_info` WHERE suppliers_id=2 and order_status<>2 and order_status<>0 and user_id='".$_SESSION['user_id']."'";
		//	$res=$GLOBALS['db']->getOne($sql); 
		//	if($res>0)
		//	{
		//		$sql="update ecs_users set isBonded=1 where user_id='".$_SESSION['user_id']."' ";
		//		$GLOBALS['db']->query($sql); 
		//	}else
		//	{
		//		$sql="SELECT count(order_id) FROM `ecs_order_info` WHERE suppliers_id=2 and order_status=0 and user_id='".$_SESSION['user_id']."'";
		//		$res=$GLOBALS['db']->getOne($sql); 
		//		if($res>0)
		//		{
		//			$sql="update ecs_users set isBonded=0 where user_id='".$_SESSION['user_id']."' ";
		//			$GLOBALS['db']->query($sql); 
		//		}
		//		else{
		//			$sql="update ecs_users set isBonded=-1 where user_id='".$_SESSION['user_id']."' ";
...
function shipping_fee($shipping_code, $shipping_config, $goods_weight, $goods_amount, $goods_number='')
{
    ...
    if (file_exists($filename))
    {
		...
		//file_put_contents('20191127fee.txt','\r\n'.date('H:i:s',time()).':'.$obj->configure['free_money'], FILE_APPEND); 
		
        $fee=0.00;
        //是否按新计费
         if(isnewexpressfeeconfig())
        {
            //file_put_contents('20190515.txt',"123" , FILE_APPEND); 
            $cartcodes="0";
            if($_SESSION['sel_cartgoods']!="")
                $cartcodes="0,".$_SESSION['sel_cartgoods'].",0";
            $sql="SELECT * FROM `zs_expressfeeconfig` WHERE name<>'开关' and isuse=1"; 
            $expressconfig = $GLOBALS['db']->getAll($sql); 
            foreach ($expressconfig AS $row)
            {
                $rid=$row['id'];
                $rname=$row['name'];
                $rgoodsamount=$row['goodsamount'];  
				if($rid==1)
				{
					if($obj->configure['free_money']!='')
					{
						$rgoodsamount=$obj->configure['free_money'];  
					}
				}
				//file_put_contents('20191127fee.txt','\r\n'.date('H:i:s',time()).'shipping_fee1:rname'.strval($rname), FILE_APPEND); 
                 $rsql="SELECT rec_id,g.goods_id, goods_price,c.goods_number, goods_weight FROM `ecs_cart` c inner join ecs_goods g on g.goods_id=c.goods_id where 1=1 and c.user_id='". $_SESSION['user_id'] ."'";
                 if($cartcodes<>"0")
                 {
                    $rsql=$rsql." and rec_id in(".$cartcodes.") ";
                 }
                if($rid!="")
                {
                    $rsql=$rsql." and g.expressfee_id=".$rid."  ";
                }
                //file_put_contents('20190515.txt','rsql:'.$rsql, FILE_APPEND); 
                $rdata = $GLOBALS['db']->getAll($rsql);
                $groupmoney=0.00;
                $groupweight=0.000;
                $groupnumber=0;  
                foreach ($rdata AS $da)
                {
                    $cartcodes=str_replace(",".$da['rec_id'].",",",",$cartcodes); 
                    $groupmoney=$groupmoney+floatval($da['goods_price'])*intval($da['goods_number']);
                    $groupweight=$groupweight+floatval($da['goods_weight'])*intval($da['goods_number']);
                    $groupnumber=$groupnumber+intval($da['goods_number']);
                }
                //file_put_contents('20190515.txt','groupmoney:'.strval($groupmoney), FILE_APPEND); 
				//file_put_contents('20191127fee.txt','\r\n'.date('H:i:s',time()).'shipping_fee1:groupmoney'.strval($groupmoney).'--rgoodsamount'.strval($rgoodsamount), FILE_APPEND); 
                if($groupmoney>0)
                {
                    $nowfee=0.00; 
                    if($rgoodsamount=="")//满200包邮
                    {
                        $rgoodsamount="0";
                    }
                    if(floatval($rgoodsamount)>0)
                    { 
                        $rname=str_replace('{money}',$rgoodsamount,$rname);
                        $rname=str_replace('{weight}',$groupweight,$rname);
                        if(floatval($rgoodsamount)>floatval($groupmoney))
                        {
                            $nowfee=$obj->calculate($groupweight,0, $groupnumber);   
                            $rname='还需凑'.strval($rgoodsamount-$groupmoney).'免运费'.strval($nowfee);  
							//file_put_contents('20191127fee.txt','\r\n'.date('H:i:s',time()).'shipping_fee1:a1', FILE_APPEND); 
                        }
                        else
                        {
                            $nowfee=0; 
							//file_put_contents('20191127fee.txt','\r\n'.date('H:i:s',time()).'shipping_fee1:a2', FILE_APPEND); 
                        } 
                        $shippingremark=$shippingremark.$rname."<br/>";
                    ...
                    else
                    {
                        ...
                        $rname=str_replace('{weight}',$groupweight,$rname); 
                        if($nowfee>0)
                            $rname=$rname."".strval($nowfee);
                         $shippingremark=$shippingremark.$rname."<br/>";
						 //file_put_contents('20191127fee.txt','\r\n'.date('H:i:s',time()).'shipping_fee1:a3', FILE_APPEND); 
                    } 
					     file_put_contents('20211028.txt','\r\n'.date('Y-m-d H:i:s',time()).' -----------shipping_fee----------', FILE_APPEND); 
                         file_put_contents('20211028.txt','\r\n'.date('Y-m-d H:i:s',time()).' oldfee:'.strval($fee).' 0-shipping_code:'.strval($shipping_code).' 1-groupweight:'.strval($groupweight).' 2-nowfee:'.strval($nowfee).' 3-groupnumber:'.strval($groupnumber), FILE_APPEND); 
						 file_put_contents('20211028.txt','\r\n'.date('Y-m-d H:i:s',time()).' shippingremark:'.$shippingremark, FILE_APPEND); 
	...
	//file_put_contents('20191127fee.txt','\r\n'.date('H:i:s',time()).'shipping_fee11:'.strval($goods_amount).'--'.strval($fee).'--'.strval($shipping_code), FILE_APPEND); 
        return $fee;
    }
    else
    {
	//file_put_contents('20191127fee.txt','\r\n'.date('H:i:s',time()).'shipping_fee10:'.strval($goods_amount).'--'.strval($filename), FILE_APPEND); 
        return 0;
    }
}
...
$arr=array("fee"=>"0.00","remark"=>"");
     ...
     if (file_exists($filename))
    {
        ...
        if(isnewexpressfeeconfig())
        {
            //file_put_contents('20190515.txt',$_SESSION['sel_cartgoods'] ); 
            $cartcodes="0";
            if($_SESSION['sel_cartgoods']!="")
                $cartcodes="0,".$_SESSION['sel_cartgoods'].",0";
            $sql="SELECT * FROM `zs_expressfeeconfig` WHERE id>0 and isuse=1"; 
            $expressconfig = $GLOBALS['db']->getAll($sql);
            foreach ($expressconfig AS $row)
            {
                $rid=$row['id'];
				...
				//file_put_contents('20191127fee.txt','\r\n'.date('H:i:s',time()).'shipping_fee2:rname'.strval($rname), FILE_APPEND); 
                $rsql="SELECT rec_id,g.goods_id, goods_price,c.goods_number, goods_weight FROM `ecs_cart` c inner join ecs_goods g on g.goods_id=c.goods_id where 1=1 and c.user_id='". $_SESSION['user_id'] ."'";
                 if($cartcodes<>"0")
                 {
                    $rsql=$rsql." and rec_id in(".$cartcodes.") ";
                ...
                //file_put_contents('20190515.txt','groupmoney:'.strval($groupmoney), FILE_APPEND); 
				//file_put_contents('20191127fee.txt','\r\n'.date('H:i:s',time()).'shipping_fee2:groupmoney'.strval($groupmoney).'--rgoodsamount'.strval($rgoodsamount), FILE_APPEND); 
                if($groupmoney>0)
                {
                    $nowfee=0.00; 
                    if($rgoodsamount=="")//满200包邮
                    ...
                    { 
                       ...
                       if(floatval($rgoodsamount)>floatval($groupmoney))
                        {
                            $nowfee=$obj->calculate($groupweight,0, $groupnumber);   
                            $rname=$rname.'还需凑'.price_format($rgoodsamount-$groupmoney,false).'免运费'.price_format($nowfee,false);  
							//file_put_contents('20191127fee.txt','\r\n'.date('H:i:s',time()).'shipping_fee2:a1', FILE_APPEND); 
                        }
                        else
                        {
                            $nowfee=0; 
							//file_put_contents('20191127fee.txt','\r\n'.date('H:i:s',time()).'shipping_fee2:a2', FILE_APPEND); 
                        } 
                        $shippingremark=$shippingremark.$rname."<br/>";
                    ...
                    else
                    {
                        ...
                        $rname=str_replace('{weight}',$groupweight,$rname); 
                        if($nowfee>0)
                            $rname=$rname."".price_format($nowfee,false);
                         $shippingremark=$shippingremark.$rname."<br/>";
						 //file_put_contents('20191127fee.txt','\r\n'.date('H:i:s',time()).'shipping_fee2:a3', FILE_APPEND); 
                    }
                         //file_put_contents('20190515.txt',$shippingremark); 
                         
					     file_put_contents('20211028.txt','\r\n'.date('Y-m-d H:i:s',time()).' -----------shipping_tee----------', FILE_APPEND); 
                         file_put_contents('20211028.txt','\r\n'.date('Y-m-d H:i:s',time()).' oldfee:'.strval($fee).' 0-shipping_code:'.strval($shipping_code).' 1-groupweight:'.strval($groupweight).' 2-nowfee:'.strval($nowfee).' 3-groupnumber:'.strval($groupnumber), FILE_APPEND); 
						 file_put_contents('20211028.txt','\r\n'.date('Y-m-d H:i:s',time()).' shippingremark:'.$shippingremark, FILE_APPEND); 
                        $fee=$fee+$nowfee;
                }
    ...
    $arr["fee"]=$fee;
    $arr["remark"]=$shippingremark;
	//file_put_contents('20191127fee.txt','\r\n'.date('H:i:s',time()).'shipping_fee2:'.strval($goods_amount).'--'.strval($fee).strval($shippingremark), FILE_APPEND); 
    return $arr;
}
 
function isnewexpressfeeconfig()
{
    $sql="SELECT isuse FROM `zs_expressfeeconfig` WHERE id='0'"; 
    $res = $GLOBALS['db']->getOne($sql); 
    if($res=="1")
    {
        return true;
 ...
 * @return  float   运费
 */
function shipping_fee_old($shipping_code, $shipping_config, $goods_weight, $goods_amount, $goods_number='')
{
    if (!is_array($shipping_config))
    {
        $shipping_config = unserialize($shipping_config);
 ...
 * @return  array   配送方式数组
 */
//mantsui 添加suppliers_id选择
function available_payment_list($support_cod, $cod_fee = 0, $is_online = false, $suppliers_id = false)
{
    ...
    if ($suppliers_id)
    {
        $sql_pay_ids = "SELECT pay_ids" .
            " FROM " . $GLOBALS['ecs']->table('suppliers') .
            " WHERE suppliers_id = '" . $suppliers_id . "'  ";
        $pay_ids = $GLOBALS['db']->getOne($sql_pay_ids);
        $pay_ids_arr = explode(",", trim($pay_ids));

        $i = 0;
        foreach ($pay_ids_arr as $val) {
            if($i == 0){
                $sql .= "AND ( pay_id = '" . $val . "' ";
            }else{
                $sql .= " OR pay_id = '" . $val . "' ";
...
function order_info($order_id, $order_sn = '')
{
    ...
    if ($order)
    {
        ...
        $order['formated_exchange_goods_amount']   = price_exchange_format($order['goods_amount'], false);
        $order['formated_exchange_discount']       = price_exchange_format($order['discount'], false);
        $order['formated_exchange_tax']            = price_exchange_format($order['tax'], false);
        $order['formated_exchange_shipping_fee']   = price_exchange_format($order['shipping_fee'], false);
        $order['formated_exchange_insure_fee']     = price_exchange_format($order['insure_fee'], false);
        $order['formated_exchange_pay_fee']        = price_exchange_format($order['pay_fee'], false);
        $order['formated_exchange_pack_fee']       = price_exchange_format($order['pack_fee'], false);
        $order['formated_exchange_card_fee']       = price_exchange_format($order['card_fee'], false);
        $order['formated_exchange_total_fee']      = price_exchange_format($order['total_fee'], false);
        $order['formated_exchange_money_paid']     = price_exchange_format($order['money_paid'], false);
        $order['formated_exchange_bonus']          = price_exchange_format($order['bonus'], false);
        $order['formated_exchange_integral_money'] = price_exchange_format($order['integral_money'], false);
        $order['formated_exchange_surplus']        = price_exchange_format($order['surplus'], false);
        $order['formated_exchange_order_amount']   = price_exchange_format(abs($order['order_amount']), false);
...
function order_goods($order_id)
{
    ...
    while ($row = $GLOBALS['db']->fetchRow($res))
    {
       ...
       // $row['goods_thumb'] = $GLOBALS['db']->getOne("select goods_thumb from ". $GLOBALS['ecs']->table('goods') ." where goods_id = ".$row['goods_id']); 
        $goods_row = $GLOBALS['db']->getRow("select goods_thumb, suppliers_id from ". $GLOBALS['ecs']->table('goods') ." where goods_id = ".$row['goods_id']);
        
        $row['goods_thumb'] = get_pc_url().'/'.get_image_path($row['goods_id'], $goods_row['goods_thumb'] );
        $row['suppliers_id'] = $goods_row['suppliers_id'];
        $goods_list[] = $row;
    }
...
function order_fee($order, $goods, $consignee)
{
    ...
    foreach ($goods AS $val)
    {
        ...
        if($total['real_goods_count'] == 1){
            $total['is_exchange'] = $GLOBALS['db']->getOne("SELECT ss.is_exchange FROM " . $GLOBALS['ecs']->table('suppliers') . " AS ss LEFT JOIN  " . $GLOBALS['ecs']->table('goods') . " AS g ON ss.suppliers_id = g.suppliers_id WHERE g.goods_id = '".$val['goods_id']."'");
        } 

        $total['goods_price']  += $val['goods_price'] * $val['goods_number'];
        $total['market_price'] += $val['market_price'] * $val['goods_number'];

        $total['goods_price_exchange']  += number_format($val['goods_price']*$GLOBALS['_CFG']['exchange_rate'], 2, '.', '') * $val['goods_number'];
        $total['market_price_exchange']  += number_format($val['market_price']*$GLOBALS['_CFG']['exchange_rate'], 2, '.', '') * $val['goods_number'];
    }
    ...
    //供应商为人民币结算
    $total['goods_price_exchange_formated']  = price_exchange_format($total['goods_price']);
    $total['market_price_exchange_formated'] = price_exchange_format($total['market_price']);
    $total['saving_exchange_formated']       = price_exchange_format($total['saving']);

    $total['goods_price_formated']  = price_format($total['goods_price'], false);
    $total['market_price_formated'] = price_format($total['market_price'], false);
    $total['saving_formated']       = price_format($total['saving'], false);
    $total['saving_exchange']  = number_format($total['saving']*$GLOBALS['_CFG']['exchange_rate'], 2, '.', '');
    ...
    $total['discount_exchange_formated'] = price_exchange_format($total['discount']);
    $total['discount_formated'] = price_format($total['discount'], false);
    $total['discount_exchange'] = number_format($total['discount']*$GLOBALS['_CFG']['exchange_rate'], 2, '.', '');
    ...
    $total['pack_fee_exchange_formated'] = price_exchange_format($total['pack_fee']);
    $total['pack_fee_formated'] = price_format($total['pack_fee'], false);
    $total['pack_fee_exchange'] = number_format($total['pack_fee']*$GLOBALS['_CFG']['exchange_rate'], 2, '.', '');
    ...
    $total['card_fee_exchange_formated'] = price_exchange_format($total['card_fee']);
    $total['card_fee_formated'] = price_format($total['card_fee'], false);
    $total['card_fee_exchange'] = number_format($total['card_fee']*$GLOBALS['_CFG']['exchange_rate'], 2, '.', '');
    ...
    $total['bonus_exchange_formated'] = price_exchange_format($total['bonus']);
    $total['bonus_formated'] = price_format($total['bonus'], false);
    $total['bonus_exchange'] = number_format($total['bonus']*$GLOBALS['_CFG']['exchange_rate'], 2, '.', '');
...
$feeremark=shipping_fee2($shipping_info['shipping_code'],$shipping_info['configure'], $weight_price2['weight'], $total['goods_price_supplier'][$row_supp['supplier_id']], $weight_price2['number']);
           $total['supplier_shipping'][$row_supp['supplier_id']]['shipping_fee'] = ($shipping_count_supp == 0 AND $weight_price2['free_shipping'] == 1) ?0 :  $feeremark["fee"];
           $total['supplier_shipping'][$row_supp['supplier_id']]['formated_exchange_shipping_fee'] = price_exchange_format($total['supplier_shipping'][$row_supp['supplier_id']]['shipping_fee']);
           $total['supplier_shipping'][$row_supp['supplier_id']]['formated_shipping_fee'] = price_format($total['supplier_shipping'][$row_supp['supplier_id']]['shipping_fee'], false);
            ...
            //税费新增
            $sql = "SELECT g.is_intax, t.tax_rate, t.tax_discount FROM ".$GLOBALS['ecs']->table('goods')." g LEFT JOIN zs_tax t ON ( g.tax_id = t.tax_id)  WHERE goods_id = '". $val['goods_id'] ."'";
            $row_tax = $GLOBALS['db']->getRow($sql);

            if($row_tax && $row_tax['is_intax'] == 0 ){
                if($row_tax['tax_rate'] > 0){
                    if($row_tax['tax_discount'] > 0){
                        $total['tax'] += $val['goods_price'] * $row_tax['tax_rate'] * $row_tax['tax_discount'];
                    }else{
                        $total['tax'] += $val['goods_price'] * $row_tax['tax_rate'];
                    }
                }
            }
        }
        $total['tax_exchange_formated'] = price_exchange_format($total['tax']);
        $total['tax_formated'] = price_format($total['tax'], false);
        $total['tax_exchange'] = number_format($total['tax']*$GLOBALS['_CFG']['exchange_rate'], 2, '.', '');

        krsort($total['supplier_shipping']);
        
		$total['shipping_fee']    = shipping_fee($shipping_info['shipping_code'],$shipping_info['configure'], $weight_price2['weight'], $total['goods_price_supplier'][$row_supp['supplier_id']], $weight_price2['number']);
		$total['remark'] =$feeremark["remark"];
        $total['shipping_fee_exchange_formated']    = price_exchange_format($total['shipping_fee']);
		$total['shipping_fee_formated']    = price_format($total['shipping_fee'], false);
		$total['shipping_fee_exchange'] = number_format($total['shipping_fee']*$GLOBALS['_CFG']['exchange_rate'], 2, '.', '');
    ...
    if ($order['extension_code'] == GROUP_BUY_CODE && $group_buy['deposit'] > 0)
    {
        ...
        $total['amount_exchange'] = $total['goods_price_exchange'];
    }
    ...
    {
        $total['amount'] = $total['goods_price'] - $total['discount'] + $total['pack_fee'] + $total['card_fee'] + $total['cod_fee'];
        $total['amount_exchange'] = $total['goods_price_exchange'] - $total['discount_exchange'] + $total['pack_fee_exchange'] + $total['card_fee_exchange'] + $total['cod_fee_exchange'];
        ...
        $total['amount_exchange'] -= $total['bonus_exchange'];
        $max_amount      -= $use_bonus; // 积分最多还能支付的金额
        
        $total['tax_exchange_formated'] = price_exchange_format($total['tax']);
        $total['tax_formated'] = price_format($total['tax'], false);
        $total['amount_exchange'] += $total['tax_exchange'];
        //分开计算运费，运费不计税
        $total['amount'] += $total['shipping_fee'] + $total['shipping_insure'];
        $total['amount_exchange'] += $total['shipping_fee_exchange'] + $total['shipping_insure_exchange'];
    }

    /* 余额 */
    $order['surplus'] = $order['surplus'] > 0 ? $order['surplus'] : 0;
    $order['surplus_exchange'] = number_format($order['surplus']*$GLOBALS['_CFG']['exchange_rate'], 2, '.', '');
    if ($total['amount'] > 0)
    {
        if (isset($order['surplus']) && $order['surplus'] > $total['amount'])
        {
            $order['surplus'] = $total['amount'];
            $order['surplus_exchange'] = $total['amount_exchange'];
            $total['amount']  = 0;
            $total['amount_exchange']  = 0;
        }
        else
        {
            $total['amount'] -= floatval($order['surplus']);
            $total['amount_exchange'] -= floatval($order['surplus_exchange']);
        }
    }
    else
    {
    ...
    $total['surplus_exchange_formated'] = price_exchange_format($order['surplus']);
    $total['surplus_formated'] = price_format($order['surplus'], false);
    $total['surplus_exchange'] = number_format($total['surplus']*$GLOBALS['_CFG']['exchange_rate'], 2, '.', '');
    ...
    if ($total['amount'] > 0 && $max_amount > 0 && $order['integral'] > 0)
    {
        ...
        $total['amount_exchange'] = number_format($total['amount']*$GLOBALS['_CFG']['exchange_rate'], 2, '.', '');
    ...
    $total['integral_exchange_formated'] = price_exchange_format($total['integral_money']);
    $total['integral_formated'] = price_format($total['integral_money'], false);
    $total['integral_exchange'] = number_format($total['integral_money']*$GLOBALS['_CFG']['exchange_rate'], 2, '.', '');
    ...
    $total['pay_fee_exchange_formated'] = price_exchange_format($total['pay_fee']);
    $total['pay_fee_formated'] = price_format($total['pay_fee'], false);
    $total['pay_fee_exchange']  = number_format($total['pay_fee']*$GLOBALS['_CFG']['exchange_rate'], 2, '.', '');

    $total['amount']           += $total['pay_fee']; // 订单总额累加上支付费用
	$total['zhe']=10;
	if($order['suppliers_id']==2)
	{
		if(isbonded())
		{
			$maxamount=floatval(getzhemaxamount());
			$zhe=floatval(getzhe()); 
			if($total['amount']*$zhe<=$maxamount)
			{
				$total['amount']=$total['amount']*$zhe;
				$total['zhe']=$zhe*10;
			}
		}
	}
    $total['amount_exchange_formated']  = price_exchange_format($total['amount']);
    $total['amount_formated']  = price_format($total['amount'], false);
    $total['amount_exchange']  += $total['pay_fee_exchange'];
    ...
    //供应商为人民币结算
    $total['formated_exchange_goods_price']  = price_exchange_format($total['goods_price']);
    $total['formated_exchange_market_price'] = price_exchange_format($total['market_price']);
    $total['formated_exchange_saving']       = price_exchange_format($total['saving']);
...
{ 
  if(date('h')<6)
  {
    return date('Ymdh') . str_pad(mt_rand(1, 9), 1, '0', STR_PAD_LEFT) . str_pad(mt_rand(0, 99), 2, '0', STR_PAD_LEFT);
  }
  else if(date('h')<12)
  {
    return date('Ymd') . str_pad(mt_rand(1, 99), 2, '0', STR_PAD_LEFT) . str_pad(mt_rand(0, 999), 3, '0', STR_PAD_LEFT);
  }
  else
  {
    return date('Ymd') . str_pad(mt_rand(1, 999), 3, '0', STR_PAD_LEFT) . str_pad(mt_rand(0, 99), 2, '0', STR_PAD_LEFT);
  }
}
...
$sql_where = $_SESSION['user_id']>0 ? "c.user_id='". $_SESSION['user_id'] ."' " : "c.session_id = '" . SESS_ID . "' AND c.user_id=0 ";
    $sql = "SELECT c.rec_id, c.user_id, c.goods_id, c.goods_name, c.goods_sn, c.goods_number, c.market_price, " .
			...
			" IF(ga.act_id, IFNULL(s.supplier_name, '网站自营'), IFNULL(ss.suppliers_name, '网站自营')) as seller ,g.suppliers_id" .
			...
			" WHERE $sql_where " .
            " AND c.rec_type = '$type' $id_ext ";  //代码修改 By  www.yshop100.com  增加一个 $id_ext , package_attr_id	
...
function addto_cart($goods_id, $num = 1, $spec = array(), $parent = 0)
{
    ...
    foreach ($basic_list as $parent_id => $fitting_price)
    {
        ...
        $GLOBALS['db']->autoExecute($GLOBALS['ecs']->table('cart_log'), $parent, 'INSERT');
    ...
    if ($num > 0)
    {
        ...
        if($row) //如果购物车已经有此物品，则更新
        {
            ...
            if ($GLOBALS['_CFG']['use_storage'] == 0 || $num <= $goods_storage)
            {
                ...
                //修改购物车日志
                $sql = "UPDATE " . $GLOBALS['ecs']->table('cart_log') . " SET goods_number = '$num'" .
                    " , goods_price = '$goods_price'".
                    " WHERE session_id = '" .SESS_ID. "' AND goods_id = '$goods_id' ".
                    " AND parent_id = 0 AND goods_attr = '" .get_goods_attr_info($spec). "' " .
                    " AND extension_code <> 'package_buy' " .
        ...
        else //购物车没有此物品，则插入
        {
            ...
            //购物车添加日志
            $GLOBALS['db']->autoExecute($GLOBALS['ecs']->table('cart_log'), $parent, 'INSERT');
			$recid=$GLOBALS['db']->getOne("SELECT max(rec_id) from ".$GLOBALS['ecs']->table('cart')." where session_id = '" . SESS_ID . "'");
        }
    }
    ...
    $GLOBALS['db']->query($sql);
file_put_contents('20191127.txt',"a1:".$sql."\r\n" , FILE_APPEND); 
...
function clear_cart($type = CART_GENERAL_GOODS,$other='')
{
    ...
    $GLOBALS['db']->query($sql);
file_put_contents('20191127.txt',"a2:".$sql."\r\n" , FILE_APPEND); 
}
 ...
 * @return  array   红包数组
 */
function user_bonus($user_id, $goods_amount = 0, $cart_goods)
    {
        $day    = getdate();
    ...
    {
        $sql = "SELECT t.type_id, t.type_name, t.type_money, b.bonus_id, t.supplier_id, t.is_use_link " .
        ...
        foreach ($rows as $k => $v) {
            if($v['is_use_link'] == '1'){
                foreach ($cart_goods as $vv) {
					//file_put_contents('20210302.txt',"cart_goods:".$vv['goods_id']."\r\n" , FILE_APPEND); 
					if(!$can_use)
					{
						$sql = "SELECT 1 FROM " . $GLOBALS['ecs']->table('link_bonus') . " WHERE link_goods_id = '".$vv['goods_id']."' AND bonus_type_id = '".$v['type_id']."'";
						$can_use=$GLOBALS['db']->getOne($sql);
						if($can_use){
							$can_use=true;
						}
					}
...
function bonus_info($bonus_id, $bonus_sn = '')
{
    $sql = "SELECT t.*, sum(t.type_money) as type_money, b.* " .
            "FROM " . $GLOBALS['ecs']->table('bonus_type') . " AS t," .
                $GLOBALS['ecs']->table('user_bonus') . " AS b " .
            "WHERE t.type_id = b.bonus_type_id ";
	if (!empty($bonus_id)||!empty($bonus_sn))
	{
		if (!empty($bonus_id))
		{
...
function use_bonus($bonus_id, $order_id)
{
    $sql = "UPDATE " . $GLOBALS['ecs']->table('user_bonus') .
            ...
            "WHERE bonus_id = '$bonus_id' and (user_id='".$_SESSION['user_id']."' or user_id='0') LIMIT 1";
...
function get_cart_goods($other='')
{
    ...
    /* 循环、统计 */
	$sql_where = $_SESSION['user_id']>0 ? "c.user_id='". $_SESSION['user_id'] ."' " : "c.session_id = '" . SESS_ID . "' AND c.user_id=0 ";
	$sql = "SELECT c.*, g.cat_id, g.brand_id, c.extension_code, IF(ga.act_id, ga.supplier_id, g.supplier_id) as supplier_id, IF(c.parent_id, c.parent_id, c.goods_id) AS pid , IF(ga.act_id, ga.suppliers_id, g.suppliers_id) AS suppliers_id, ss.is_exchange" .
            " FROM " . $GLOBALS['ecs']->table('cart') . " AS c left join " .$GLOBALS['ecs']->table('goods')." AS g ".
			" on c.goods_id=g.goods_id ".
			" left join " . $GLOBALS['ecs']->table('goods_activity') . " as ga " .
			" on ga.act_id = c.goods_id and c.extension_code = 'package_buy'" .
            " left join " . $GLOBALS['ecs']->table('suppliers') . " as ss " .
            " on g.suppliers_id = ss.suppliers_id" .
    ...
    while ($row = $GLOBALS['db']->fetchRow($res))
    {
        ...
        //供应商为人民币结算
        $row['subtotal_exchange']     = price_exchange_format($row['goods_price'] * $row['goods_number']);
        $row['goods_exchange_price']  = price_exchange_format($row['goods_price']);
        $row['market_exchange_price'] = price_exchange_format($row['market_price']);
        ...
        if ($row['extension_code'] == 'package_buy')
        {
            $row['package_goods_list'] = get_package_goods($row['goods_id'], $row['package_attr_id'] ); //修改   增加一个变量
        }
	$row['is_cansel'] = is_cansel($row['goods_id'], $row['product_id'], $row['package_buy']);

	if($row['suppliers_id'])
	{
		$suppliers_name = $GLOBALS['db']->getOne("select suppliers_name from ". $GLOBALS['ecs']->table('suppliers') ." where suppliers_id='". $row['suppliers_id']."' ");
	...
	$keyname = $row['suppliers_id'] ? $row['suppliers_id'] : '0' ;
	$goods_list[$keyname]['goods_list'][] = $row;
	$goods_list[$keyname]['suppliers_name'] = $suppliers_name;
    $goods_list[$keyname]['suppliers_id'] = $row['suppliers_id'];
	ksort($goods_list);
    ...
    $total['saving_exchange']       = price_exchange_format($total['market_price'] - $total['goods_price']);
    ...
    $total['goods_exchange_price']  = price_exchange_format($total['goods_price']);
    $total['market_exchange_price'] = price_exchange_format($total['market_price']);
...
function compute_discount($supplierid=-1)
{
    ...
    $sql = "SELECT c.goods_id, c.goods_price * c.goods_number AS subtotal, g.cat_id, g.brand_id, g.supplier_id " .
			...
			if($_SESSION['sel_cartgoods']!="")
			{
				$sql .= " AND c.rec_id in (". $_SESSION['sel_cartgoods'] .") "; 
			}
...
function send_order_bonus($order_id,$supplier_id)
{
    ...
    if ($bonus_list)
    {
        ...
        foreach ($bonus_list AS $bonus)
        {
			...
			$sql = "";
			if(intval($bonus['send_maxcount'])>0&&intval($bonus['sentcount_0'])<intval($bonus['send_maxcount']))
			{
            /* 修改用户红包 */
		    ...
		    else if(intval($bonus['send_maxcount'])==0)
			{
             /* 修改用户红包 */
             $sql = "INSERT INTO " . $GLOBALS['ecs']->table('user_bonus') . " (bonus_type_id, user_id, supplier_id ) " ."VALUES('$bonus[type_id]', '$user[user_id]', '$supplier_id')";
			}	
            ...
            //{
				if(intval($bonus['number'])>0)
				{
					...
					{ 
						$sql_c = "SELECT count(user_id) as c FROM " . $GLOBALS['ecs']->table("user_bonus") . " WHERE `bonus_type_id`=" . $bonus['type_id'] . "" ;
						$res_bonuscount = $GLOBALS['db']->getOne($sql_c);
						if(intval($res_bonuscount)<>intval($bonus['sentcount_0']))
						{
							$sql="update " . $GLOBALS['ecs']->table("bonus_type") . " set sentcount_0=".$res_bonuscount." WHERE type_id=".$bonus['type_id']."";
							$GLOBALS['db']->query($sql);
						}
					}
				}
...
{	
    ...
    $sql = "SELECT b.type_id, b.type_money, SUM(o.goods_number) AS number,send_maxcount,sentcount_0 " .
            "FROM " . $GLOBALS['ecs']->table('order_goods') . " AS o, " .
                      $GLOBALS['ecs']->table('goods') . " AS g, " .
    ...
    /* 查询按订单发的红包 */
	 $sql = "SELECT type_id, type_money, IFNULL(FLOOR('$amount' / min_amount), 1) AS number,send_maxcount,sentcount_0 " .
            "FROM " . $GLOBALS['ecs']->table('bonus_type') .
            "WHERE send_type = '" . SEND_BY_ORDER . "' " .
...
function add_package_to_cart($package_id, $num = 1, $package_attr_id='', $package_prices='')
{
    ...
    $sql = "DELETE FROM " . $GLOBALS['ecs']->table('cart') . " WHERE session_id = '" . SESS_ID . "' AND is_gift <> 0";
    $GLOBALS['db']->query($sql);
file_put_contents('20191127.txt',"a3:".$sql."\r\n" , FILE_APPEND); 
...
function get_pickup_info($cityid,$suppid=0){
	global $db,$ecs;
	$sql = "select pp.*,r.region_name from ".$ecs->table('pickup_point')." as pp left join ".$ecs->table('region')." as r on pp.district_id=r.region_id where pp.supplier_id=".$suppid." and is_show='1' order by pp.id";
	$info = $db->getAll($sql);
	return $info;
}
/**
