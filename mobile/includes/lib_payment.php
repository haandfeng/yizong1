...
function check_money_surplus($log_id, $money)
{
    if(is_numeric($log_id))
    {
        $sql = 'SELECT order_amount FROM ' . $GLOBALS['ecs']->table('pay_log') .
    ...
    {
		$sql = 'select surplus from ecs_order_info where order_id in(SELECT order_id FROM ' . $GLOBALS['ecs']->table('pay_log') .
              " WHERE log_id = '$log_id')";
        $surplus = $GLOBALS['db']->getOne($sql);
		if($surplus+$money==$amount)
		{
			return true;
		}
		else
