...
function cat_list($cat_id = 0, $selected = 0, $re_type = true, $level = 0, $is_show_all = true)
{
    ...
    if ($re_type == true)
    {
        ...
        foreach ($options AS $var)
        {
			...
			//$sql = "SELECT COUNT(*) AS goods_num " .
            //        " FROM " . $GLOBALS['ecs']->table('goods') .
            //        " WHERE is_delete = 0 AND is_on_sale = 1 and cat_id=".$var['cat_id']." " .$where1;
            //$goodsnum = $GLOBALS['db']->getOne($sql);
			//if($goodsnum==0)
			//{
			//	$var['is_show']=0;
			//}
//file_put_contents('20200713s.txt',date('Y-m-d H:i:s')."return1-".$var['cat_id'].":".$goodsnum."\r\n" , FILE_APPEND); 
        }
        return $select;
    }
    else
    {
        foreach ($options AS $key => $value)
        {
			...
			//$sql = "SELECT COUNT(*) AS goods_num " .
            //        " FROM " . $GLOBALS['ecs']->table('goods') .
            //        " WHERE is_delete = 0 AND is_on_sale = 1 and cat_id=".$value['cat_id']." " .$where1;
            //$goodsnum = $GLOBALS['db']->getOne($sql);
			//if($goodsnum==0)
			//{
			//	$value['is_show']=0;
			//}
//file_put_contents('20200713s.txt',date('Y-m-d H:i:s')."return2-".$value['cat_id'].":".$goodsnum."\r\n" , FILE_APPEND); 
        }
        return $options;
    }
}
    ...
    else
    {
        ...
        foreach ($res as $rows)
        {
            ...
            elseif ($rows['act_range'] == FAR_CATEGORY)
            {
                ...
                /* 代码修改_BY yshop100 STAR_20151215 商品为店铺商品时优惠活动按照店铺内分类筛选 */
                if($suppid>0){
                        $flag = false;
                ...
                /* 代码修改_BY yshop100 END_20151215 商品为店铺商品时优惠活动按照店铺内分类筛选 */        
                }else{
 ...
 * @return  string
 */
function price_exchange_format($price, $change_price = true)
{
    if($price==='')
    {
     $price=0;
    ...
    return sprintf($GLOBALS['_CFG']['currency_exchange_format'],$price,$price_exchange);
}
    ...
    switch ($app)
    {
        ...
        case 'group_buy':
            ...
            else
            {
				...
				$uri = 'pintuan.php?act=view&amp;act_id=' . $gbid;
            }
	...
	foreach($supplier_ids as $val){
		...
		if ($position == 1){
			if ($info['sms_order_placed'] == '1' && $info['sms_shop_mobile'] != '')
		    {
		    	...
		    	array_filter($phones);
				$content1 = sprintf($content,$supplierinfo[$val],$_CFG['sms_sign']);
