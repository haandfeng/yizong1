...
function register_by_xcx ($username, $password, $mobile, $other = array())
{
	return _register($username, $password, $mobile, $other, 'xcx');
}
function register_by_app ($username, $password, $mobile, $other = array())
{
	return _register($username, $password, $mobile, $other, 'app');
}
...
function _register ($username, $password, $email_or_mobile, $other = array(), $register_type = 'email')
{
	...
	else if($register_type == 'xcx')
	{
		if(strpos($username,'xc_')!=0)
		{
			$GLOBALS['err']->add($GLOBALS['_LANG']['username_empty']);
		}
	}
	else if($register_type == 'app')
	{
		if(strpos($username,'ap_')!=0)
		{
			$GLOBALS['err']->add($GLOBALS['_LANG']['username_empty']);
		}
	}
	...
	{ 
		$result = $GLOBALS['user']->add_user($username, $password, $email_or_mobile);
	}
	else if($register_type == 'app')
	{ 
		$result = $GLOBALS['user']->add_user($username, $password, $email_or_mobile);
