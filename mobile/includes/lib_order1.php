...
function shipping_fee($shipping_code, $shipping_config, $goods_weight, $goods_amount, $goods_number='')
{
         ...
         if(isnewexpressfeeconfig())
        {
            ...
            foreach ($expressconfig AS $row)
            {
                ...
                $rgoodsamount=$row['goodsamount'];  
                 $rsql="SELECT rec_id,g.goods_id, goods_price,c.goods_number, goods_weight FROM `ecs_cart` c inner join ecs_goods g on g.goods_id=c.goods_id where 1=1 ";
                 if($cartcodes<>"0")
                 {
                ...
                if($groupmoney>0)
                {
					file_put_contents('20191127fee.txt','/r/n'.date('H:i:s',time()).'shipping_fee11:groupmoney:'.strval($groupmoney), FILE_APPEND); 
                    $nowfee=0.00; 
                    ...
                    { 
                        ...
                        $rname=str_replace('{weight}',$groupweight,$rname);
							file_put_contents('20191127fee.txt','/r/n'.date('H:i:s',time()).'shipping_fee11:rgoodsamount:'.strval($rgoodsamount), FILE_APPEND); 
                        if(floatval($rgoodsamount)>floatval($groupmoney))
                        {
							...
							file_put_contents('20191127fee.txt','/r/n'.date('H:i:s',time()).'shipping_fee11:a1:'.strval($nowfee), FILE_APPEND); 
                        }
                        else
                        {
                            $nowfee=0; 
							file_put_contents('20191127fee.txt','/r/n'.date('H:i:s',time()).'shipping_fee11:a2:'.strval($nowfee), FILE_APPEND); 
                        } 
                    ...
                    {
						file_put_contents('20191127fee.txt','/r/n'.date('H:i:s',time()).'shipping_fee11:rgoodsamount==0:'.strval($rgoodsamount), FILE_APPEND); 
                        $nowfee=$obj->calculate($groupweight,0, $groupnumber);   
                        $rname=str_replace('{weight}',$groupweight,$rname); 
                        if($nowfee>0)
                            $rname=$rname."".strval($nowfee);
                         $shippingremark=$shippingremark.$rname."<br/>";
						 file_put_contents('20191127fee.txt','/r/n'.date('H:i:s',time()).'shipping_fee11:0:'.strval($nowfee), FILE_APPEND); 
                    } 
                         //file_put_contents('20190515.txt',$shippingremark); 
                        
                        $fee=$fee+$nowfee;
						file_put_contents('20191127fee.txt','/r/n'.date('H:i:s',time()).'shipping_fee11:nowfee:'.strval($nowfee), FILE_APPEND); 
                }
            } 
        }
        else
	...
	file_put_contents('20191127fee.txt','/r/n'.date('H:i:s',time()).'shipping_fee11:'.strval($goods_amount).'--'.strval($fee).'--'.strval($shipping_code), FILE_APPEND); 
        return $fee;
    }
    else
    {
	file_put_contents('20191127fee.txt','/r/n'.date('H:i:s',time()).'shipping_fee10:'.strval($goods_amount).'--'.strval($filename), FILE_APPEND); 
        return 0;
    }
}
