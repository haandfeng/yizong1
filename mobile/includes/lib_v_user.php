...
{
    $sql = "SELECT u.invite_code,w.* FROM " . $GLOBALS['ecs']->table('users') . " as u LEFT JOIN " . $GLOBALS['ecs']->table('weixin_user') . " as w ON w.ecuid = u.user_id WHERE user_id = '$user_id'";
...
//获取用户
function get_user_code_by_user_id($user_id)
{
    $sql = "SELECT user_money FROM " . $GLOBALS['ecs']->table('users') . " WHERE user_id = '$user_id'";
    $user_money = $GLOBALS['db']->getOne($sql);
    if($user_money > 0)
...
function get_distrib_user_info($user_id,$level)
{
	...
	if($count)
	{
		 $sql = "SELECT user_id, user_name, email, is_validated, user_money, frozen_money, rank_points, pay_points, reg_time ".
                    " FROM " . $GLOBALS['ecs']->table('users') . " WHERE user_id IN($up_uid) ORDER BY user_id DESC";
...
function get_affiliate_count_by_user_id($user_id)
{
	...
	$sql = "SELECT COUNT(*) FROM 
			...
			$GLOBALS['ecs']->table('order_goods') . " as og where o.order_id = og.order_id AND o.user_id = '$user_id' $sql_where group by o.order_id ) as oog $where";
	return $GLOBALS['db']->getOne($sql);
}
...
function get_affiliate_info_by_user_id($user_id,$page,$size)
{
	...
	$sql = "select order_id,is_separate,supplier_id,suppliers_id,total_split_money from " . 
			"(select a.order_id,a.is_separate,supplier_id,suppliers_id," . 
		    ...
		    "group by a.order_id ) as ab " . $where . " ORDER BY order_id DESC";
    ...
    while ($row = $GLOBALS['db']->fetchRow($res))
	{
		...
		{
			$arr[$row['order_id']]['suppliers_name'] = '平台方'; 
		}
		else
		{
			$arr[$row['order_id']]['suppliers_name'] = get_suppliers_desc($row['suppliers_id']); 
		}
...
function get_count_distrib_order_by_user_id($user_id,$is_separate)
{
	...
	if(!empty($oids))
	{
		$sql = "SELECT COUNT(*) FROM 
				(SELECT SUM(split_money) as total_money FROM " . 
				$GLOBALS['ecs']->table('order_info') . " as o ," . 
				$GLOBALS['ecs']->table('order_goods') . " as og where o.order_id = og.order_id AND o.order_id in($oids) AND is_separate = '$is_separate' group by o.order_id ) as oog $where";
		return $GLOBALS['db']->getOne($sql);
	}
	return 0;
}
...
function get_all_distrib_order_by_user_id($user_id,$is_separate,$page,$size)
{
	...
	if(!empty($uids))
	{
		...
		$sql = "SELECT order_id FROM " . $GLOBALS['ecs']->table('order_info') . " WHERE user_id in($uids) AND (order_status = '5' OR order_status = '1')" . $sql_where;
		$order_list = $GLOBALS['db']->getAll($sql);
		$oids = ''; //分销商下所有下级会员的订单id
		for($i = 0; $i < count($order_list); $i++)
		{
		...
		if(!empty($oids))
		{
			$sql = "SELECT  order_id,goods_id,goods_name,user_id," . 
				"supplier_id,suppliers_id,goods_thumb,user_name,add_time, total_split_money FROM 
				(SELECT og.order_id,og.goods_id,og.goods_name, " . 
				"o.user_id,o.supplier_id,o.suppliers_id,g.goods_thumb, o.add_time,u.user_name," . 
				"SUM(split_money) as total_money," .
			...
			while ($row = $GLOBALS['db']->fetchRow($res))
			{
				...
				//供货商
				$arr[$row['order_id']]['suppliers_name'] = get_suppliers_name($row['suppliers_id']); 
				//获取订单商品中可分成商品数量
				$arr[$row['order_id']]['goods_count'] = get_goods_count($row['order_id']);
				$arr[$row['order_id']]['total_split_money'] = $row['total_split_money'];
				...
				$arr[$row['order_id']]['call_username_id'] = '会员ID：'.$row['user_id'];
				...
				$arr[$row['order_id']]['add_time'] = '下单时间：'.local_date("Y-m-d G:i:s", $row['add_time']);
			}
			if(!empty($arr))
			{
				return $arr; 
	...
	else
	{
		...
		if(!empty($uids))
		{
			if($GLOBALS['_CFG']['distrib_type'] == 0)
			{
				...
				$GLOBALS['ecs']->table('order_info') . " as a " . 
				"  where  a.user_id in($uids) and is_separate = '$is_separate'";
			}
			else
			{
				...
				$GLOBALS['ecs']->table('order_info') . " as a ," . 
				$GLOBALS['ecs']->table('order_goods') . 
				" as b where a.order_id = b.order_id and a.user_id in($uids) and is_separate = '$is_separate' " .
				" group by a.order_id";
			}
...
function is_distribor($user_id)
{
	...
	$sql = "SELECT status FROM " . $GLOBALS['ecs']->table('users') . " WHERE is_fenxiao = 1 AND user_id = '$user_id'";
	//echo $sql;die();
	return $GLOBALS['db']->getOne($sql);
}
...
function get_suppliers_desc($suppliers_id)
{
	$sql = "SELECT suppliers_desc FROM " . $GLOBALS['ecs']->table('suppliers') . " WHERE suppliers_id = '$suppliers_id'";
	return $GLOBALS['db']->getOne($sql); 
}
...
//判断会员是否是同意协议
function is_agree($user_id)
{
	$sql = "SELECT isagree FROM " . $GLOBALS['ecs']->table('user_fenxiao_period') . " WHERE user_id = '$user_id'";
	return $GLOBALS['db']->getOne($sql);
}

function get_order_desc($suppliers_id)
{
	$sql = "SELECT suppliers_desc FROM " . $GLOBALS['ecs']->table('suppliers') . " WHERE suppliers_id = '$suppliers_id'";
	return $GLOBALS['db']->getOne($sql); 
}
