...
function get_recommend_goods($type = '', $cats = '',$rate='')
{
    ...
    if (!in_array($type, array('best', 'new', 'hot', 'bs')))
    {
        return array();
    }
    ...
    if (empty($type_goods[$type]))
    {
        ...
        if ($data === false)
        {
            $sql = 'SELECT g.goods_id, g.is_best, g.is_new, g.is_hot, g.is_bs, g.is_promote, b.brand_name,g.sort_order, g.supplier_id, g.suppliers_id ' .
               ' FROM ' . $GLOBALS['ecs']->table('goods') . ' AS g ' .
               ' LEFT JOIN ' . $GLOBALS['ecs']->table('brand') . ' AS b ON b.brand_id = g.brand_id ' .
               ' WHERE g.is_on_sale = 1 AND g.is_alone_sale = 1 AND g.is_delete = 0 AND (g.is_best = 1 OR g.is_new =1 OR g.is_hot = 1 OR g.is_bs = 1)'.$where1.
               ' ORDER BY g.sort_order ASC, g.second_order ASC, g.last_update DESC';
            ...
            if (!empty($goods_res))
            {
                foreach($goods_res as $data)
                {
                    ...
                    if ($data['is_bs'] == 1)
                    {
                        $goods_data['bs'][] = array('goods_id' => $data['goods_id'], 'sort_order' => $data['sort_order']);
                    }
                    if ($data['brand_name'] != '')
                    {
        ...
        $type2lib = array('best'=>'recommend_best', 'new'=>'recommend_new', 'hot'=>'recommend_hot', 'bs'=>'recommend_bs');
        if (empty($type_array))
        {
        ...
        $sql = 'SELECT cat.cat_name,g.goods_id, g.goods_name,g.click_count, g.goods_name_style, g.market_price, g.shop_price AS org_price, g.promote_price, ' .
                "IFNULL(mp.user_price, g.shop_price * '$_SESSION[discount]') AS shop_price, g.exclusive, ".
                "promote_start_date, promote_end_date, g.goods_brief, g.goods_thumb, g.goods_img, RAND() AS rnd, g.supplier_id, g.suppliers_id, g.is_intax, ss.suppliers_desc, ss.is_exchange " .
                'FROM ' . $GLOBALS['ecs']->table('goods') . ' AS g ' .
        ...
        $type_merge = array_merge($type_array['new'], $type_array['best'], $type_array['hot'], $type_array['bs']);
        $type_merge = array_unique($type_merge);
        $sql .= ' WHERE g.goods_id ' . db_create_in($type_merge).$where1;
        $sql .= ' ORDER BY g.sort_order ASC, g.second_order ASC, g.add_time DESC';
        ...
        foreach ($result AS $idx => $row)
        {
            ...
            $goods[$idx]['final_exchange_price'] = price_exchange_format(get_final_price($row['goods_id'], 1, false));
        ...
        $goods[$idx]['supplier_id'] = $row['supplier_id'];
        $goods[$idx]['suppliers_id'] = $row['suppliers_id'];
        $goods[$idx]['suppliers_desc'] = $row['suppliers_desc'];
        $goods[$idx]['is_exchange'] = $row['is_exchange'];
        $goods[$idx]['is_intax'] = $row['is_intax'];
            ...
            if (in_array($row['goods_id'], $type_array['bs']))
            {
                $type_goods['bs'][] = $goods[$idx];
            }
        }
    }
...
function get_promote_goods($cats = '')
{
    ...
    $sql = 'SELECT g.goods_id, g.goods_name, g.goods_name_style, g.market_price, g.shop_price AS org_price, g.promote_price, ' .
            ...
            " AND g.is_promote = 1 AND promote_start_date <= '$time' AND promote_end_date >= '$time' ".$where1;
    $sql .= $order_type == 0 ? ' ORDER BY g.sort_order, g.second_order ASC, g.last_update DESC' : ' ORDER BY rnd';
    $sql .= " LIMIT $num ";
    $result = $GLOBALS['db']->getAll($sql);
...
function get_category_recommend_goods($type = '', $cats = '', $brand = 0, $min =0,  $max = 0, $ext='')
{
    ...
    $sql =  'SELECT g.goods_id, g.goods_name, g.goods_name_style, g.market_price, g.shop_price AS org_price, g.promote_price, ' .
            ...
            "LEFT JOIN " . $GLOBALS['ecs']->table('member_price') . " AS mp ".
                    "ON mp.goods_id = g.goods_id AND mp.user_rank = '$_SESSION[user_rank]' ".
            'WHERE g.is_on_sale = 1 AND g.is_alone_sale = 1 AND g.is_delete = 0 ' .$where1. $brand_where . $price_where . $ext;
    $num = 0;
...
function get_goods_info($goods_id)
{
    ...
    $sql = 'SELECT g.*, c.measure_unit, b.brand_id, b.brand_name AS goods_brand, m.type_money AS bonus_money, ' .
                ...
                "IFNULL(mp.user_price, g.shop_price * '$_SESSION[discount]') AS rank_price , ss.suppliers_desc, ss.is_exchange " .
            'FROM ' . $GLOBALS['ecs']->table('goods') . ' AS g ' .
    ...
    if ($row !== false)
    {
        ...
        $row['shop_price_exchange_formated'] = price_exchange_format($row['shop_price']);
        ...
        $row['promote_exchange_price'] = price_exchange_format($promote_price);
...
function get_goods_price($goods_id)
{
    ...
    if ($row !== false)
    {
		...
		{
			return $row['promote_exchange_price'];
...
function get_goods_gallery($goods_id)
{
    ...
    $sql = 'SELECT img_id, img_url, thumb_url, img_desc' .
        ...
        " WHERE goods_id = '$goods_id' ORDER BY img_sort ASC LIMIT " . $goods_gallery_number;
...
function selled_wap_count($goods_id)
{
    ...
    $goods_name = $GLOBALS['db']->getOne($sql);
	 ...
	 $sql= "select sum(goods_number) as count from ".$GLOBALS['ecs']->table('order_goods')."where goods_name ='".str_replace("'","''",$goods_name)."'";
