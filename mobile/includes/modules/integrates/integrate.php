...
class integrate
{
	...
	{//file_put_contents('20191029.txt',"login2".$username."-3:".$password."\r\n" , FILE_APPEND); 
		...
		{//file_put_contents('20191029.txt',"login50\r\n" , FILE_APPEND); 
			if($this->need_sync)
			{
		...
		{//file_put_contents('20191029.txt',"login51\r\n" , FILE_APPEND); 
			return false;
		}
	}
	 ...
	 * @return void
	 */
	function loginmsg ($username, $password, $remember = null)
	{//file_put_contents('20191013.txt',"loginmsg\r\n" , FILE_APPEND); 
		if(is_email($username))
		{
			$sql = "select " . $this->field_name . " from " . $this->table($this->user_table) . " where " . $this->field_email . "='" . $username . "'";
			$username = $this->db->getOne($sql);
		...
		$uid=$this->check_user($username, $password);
		//file_put_contents('20191013.txt',"uid:".$uid."\r\n" , FILE_APPEND); 
		if($uid > 0)
		{ 
			return $uid;
		}
	...
	function loginmsgunion ($username)
	{ 
			$sql = "select user_id from " . $this->table($this->user_table) . " where wx_unionid='" . $username . "'";
	...
	function add_user ($username, $password, $email, $gender = -1, $bday = 0, $reg_date = 0, $md5password = '')
	{
		...
		{ 
			$arr=explode(',',$email);
			return $this->add_user_by_unionid($username, $password, $arr[0], $arr[1], $md5password);
		}
		else
		{
...
// 		return true;
	...
	function add_user_by_unionid ($username, $password, $unionid, $xcx_id, $md5password = '')
	{
		/* 将用户添加到整合方 */
		if($this->check_user($username) > 0)
		{
		...
		/* 检查mobile是否重复 */
		$sql = "SELECT " . $this->field_id . " FROM " . $this->table($this->user_table) . " WHERE wx_unionid = '$unionid'";
		if($this->db->getOne($sql, true) > 0)
		{
			$this->error = ERR_MOBILE_PHONE_EXISTS;
		...
		/* 补充手机已验证 */
		$fields = array(
			$this->field_name, 'wx_unionid', $this->field_pass, 'xcx_id'
		);
		$values = array(
			$post_username, $unionid, $post_password, $xcx_id
		);
