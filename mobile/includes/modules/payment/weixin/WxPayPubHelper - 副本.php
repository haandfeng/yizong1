	...
	{		
		try 
		{
			if($this->parameters["bill_date"] == null ) 
			{
		   	...
		   	$this->parameters["appid"] = APPID;//公众账号ID
		   	$this->parameters["mch_id"] = MCHID;//商户号
            $this->parameters['bill_type'] = 'SUCCESS';
		    $this->parameters["nonce_str"] = $this->createNoncestr();//随机字符串
		    $this->parameters["sign"] = $this->getSign($this->parameters);//签名
		    return  $this->arrayToXml($this->parameters);
		}catch (SDKRuntimeException $e)
 ...
 * 查询结算资金的明细 
 */
class Gettlement_pub extends Wxpay_client_pub
{

	function __construct() 
	{
		//设置接口链接
		$this->url = "https://api.mch.weixin.qq.com/pay/settlementquery";
		//设置curl超时时间
		$this->curl_timeout = 10;		
	}
	...
	{		
		try 
		{
			if($this->parameters["usetag"] == null ) 
			{
				throw new SDKRuntimeException("结算资金明细接口中，缺少必填参数usetag！"."<br>");
			}
			if($this->parameters["date_start"] == null ) 
			{
				throw new SDKRuntimeException("结算资金明细接口中，缺少必填参数date_start！"."<br>");
			}
			if($this->parameters["date_end"] == null ) 
			{
				throw new SDKRuntimeException("结算资金明细接口中，缺少必填参数date_end！"."<br>");
			}
		   	$this->parameters["appid"] = APPID;//公众账号ID
		   	$this->parameters["mch_id"] = MCHID;//商户号
            $this->parameters['offset'] = '0';
            $this->parameters['limit'] = '31'; 
		    $this->parameters["nonce_str"] = $this->createNoncestr();//随机字符串
		    $this->parameters["sign"] = $this->getSign($this->parameters);//签名
		    return  $this->arrayToXml($this->parameters);
		}catch (SDKRuntimeException $e)
 ...
 * 查询单个订单交易接口
 */
class OrderQueryPay_pub extends Wxpay_client_pub
{
	...
	{		
		try 
		{
			if($this->parameters["out_trade_no"] == null ) 
			{
				throw new SDKRuntimeException("订单查询接口中，缺少必填参数out_trade_no！"."<br>");
			}
		   	$this->parameters["appid"] = APPID;//公众账号ID
		   	$this->parameters["mch_id"] = MCHID;//商户号
		    $this->parameters["nonce_str"] = $this->createNoncestr();//随机字符串
