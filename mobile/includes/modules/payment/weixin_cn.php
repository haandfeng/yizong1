...
//		return $jsApi->getParameters();
	...
	function jsApiPay($params){
		...
		//file_put_contents('20211123.txt',date('Y-m-d H:i:s').$sql."\r\n" , FILE_APPEND); 
        $GLOBALS['db']->query($sql);
        return $jsApiParameters;
    }

	function jsApiRefund($params){
        define(APPID,$params['appId']);
        define(MCHID,$params['partnerId']);
        define(KEY,$params['partnerKey']);
        include_once ("weixin/WxPayPubHelper.php");
        ...
        $order_amount = $order_amount*100;
        $notify_url = "https://www.quickact.net/mobile/respond_back.php";
        ...
        $jsApi->setParameter("out_refund_no","$out_refund_no");//商品描述
        $jsApi->setParameter("total_fee","$order_amount");//总金额
        $jsApi->setParameter("refund_fee","$refund_fee");   
        $jsApi->setParameter("notify_url",$notify_url);//通知地址  
        $jsApiParameters = $jsApi->getResult(); 
        return $jsApiParameters;
    }
	...
	function respond() {
		...
		$notify->saveData ( $xml );
		//file_put_contents('20210628.txt',date('Y-m-d H:i:s')."xml:".$xml."\r\n" , FILE_APPEND); 
		$payment = get_payment ( 'weixin_cn' );
		define ( KEY, $payment ['partnerKey'] ); // 通加密串
		if ($notify->checkSign () == TRUE) {	
//file_put_contents('20210628.txt',date('Y-m-d H:i:s')."checkSign:true\r\n" , FILE_APPEND); 		
			if ($notify->data ["return_code"] == "FAIL") {
				...
				if (! check_money_surplus ( $pay_info['log_id'], $notify->data ['total_fee']/100 )) {
			...
			//file_put_contents('20210628.txt',date('Y-m-d H:i:s')."checkSign:false\r\n" , FILE_APPEND); 		
