...
class weixin {
	...
	function get_code($order, $payment) {
        ...
        if(empty($final_data) || ($final_data['qrcode_endtime'] < time() && $final_data['pay_type'] == 800201) || empty($final_data['qrcode']) || ($final_data['txamt'] != $order['exchange_amount'] * 100)){
            ...
            $fields = array(
                ...
                'txamt' => urlencode($order['exchange_amount'] * 100),
                'txdtm' => $now_time,
...
//		return $jsApi->getParameters();
	...
	function jsApiPay($params){
        ...
        $body = '支付订单：'.$params['order_sn'].'，共：HKD'.$params['order_amount'];
	...
	function respond() {
		...
		if ($notify->checkSign () == TRUE) {				
			if ($notify->data ["return_code"] == "FAIL") {
				...
				if ($order_info['exchange_amount'] != $notify->data ['total_fee']/100 ) {
    ...
    function downloadbill($bill_date){
        $call_info = array();
        define(APPID,'wx62c6a5493ee94f4c');
        define(MCHID,'1493763372');
        define(KEY,'forEaseSummitEaseButechHkShFjiLI');
        include_once ("weixin/WxPayPubHelper.php");
	...
	function natpayHtml($order) {
		if (! strpos ( $_SERVER ['HTTP_USER_AGENT'], 'MicroMessenger' )) {
			...
			$order['order_id'] = $order['log_id'].'-'.$order['exchange_amount']*100; 

			// 设置统一支付接口参数
			$return_url = 'http://' . $_SERVER ['HTTP_HOST'].'/respond.php';
			$unifiedOrder->setParameter ( "body", $order ['order_sn'] );
			$unifiedOrder->setParameter ( "out_trade_no", $order ['order_id'] ); // 商户订单号
			$unifiedOrder->setParameter ( "total_fee", $order ['exchange_amount'] * 100 ); // 总金额
