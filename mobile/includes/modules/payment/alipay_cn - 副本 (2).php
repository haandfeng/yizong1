...
class alipay_cn
{
    ...
    function get_code($order, $payment)
    {
		...
		$parameter['notify_url'] = 'https://www.quickact.net/mobile/respond-test.php?code=alipay_cn';//return_url(basename(__FILE__, '.php'));
		$parameter['return_url'] = 'https://www.quickact.net/mobile/respond-test.php?code=alipay_cn';
        ...
        if($real_method == 3){
            ...
            $parameter['price'] = $order['exchange_amount'];
        }
		...
		file_put_contents('20200504.txt',date('Y-m-d H:i:s')."==2:\r\n" , FILE_APPEND);
        return $button;
    }
...
//        print_r(md5($sign));echo '-----'.$_GET['sign'];die;
        ...
        /* 取得订单信息 */ //jx
        $sql = 'SELECT pl.log_id, oi.exchange_amount FROM ' . $GLOBALS['ecs']->table('pay_log') .
        ...
        /* 检查支付的金额是否相符 */
        if ($orderinfo['exchange_amount'] != $_GET['total_fee']){
            return false;
        }
