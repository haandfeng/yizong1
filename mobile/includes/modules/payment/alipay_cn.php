...
class alipay_cn
{
    ...
    function get_code($order, $payment)
    {
		...
		//file_put_contents('20200504.txt',date('Y-m-d H:i:s')."==:".$_SERVER['HTTP_USER_AGENT']."\r\n" , FILE_APPEND);
		//file_put_contents('20200504.txt',date('Y-m-d H:i:s')."==a:".$real_method."order_amount:".$order['order_amount']."\r\n" , FILE_APPEND);
		if(preg_match('/Windows/', $_SERVER['HTTP_USER_AGENT']))
		{
				...
				$parameter['notify_url'] = 'https://www.quickact.net/mobile/respond.php?code=alipay_cn';//return_url(basename(__FILE__, '.php'));
				$parameter['return_url'] = 'https://www.quickact.net/mobile/respond.php?code=alipay_cn';
				$parameter['subject'] = $order['order_sn'];
				$parameter['out_trade_no'] = $order['order_sn'];
				$parameter['body'] = 'test';
				$parameter['product_code'] = 'NEW_WAP_OVERSEAS_SELLER';
				$parameter['payment_inst'] = $payment['payment_inst'];
				$parameter['secondary_merchant_id'] = $payment['alipay_account'];
				$parameter['secondary_merchant_name'] = $payment['alipay_account'];
				$parameter['secondary_merchant_industry'] = '7011';
				
				$parameter['seller_email'] = $payment['alipay_account'];

				if($real_method == 3){
					$parameter['currency'] = $payment['alipay_pay_currency'];
					$parameter['total_fee'] = $order['exchange_amount'];
				}else{
					$parameter['price'] = $order['order_amount'];//$order['exchange_amount'];
				...
				//file_put_contents('20200504.txt',date('Y-m-d H:i:s')."==2:".$button."\r\n" , FILE_APPEND);
				return $button;
		}
		else
		{
				...
				$parameter['service'] = $service;
				$parameter['partner'] = $payment['alipay_partner'];
				$parameter['seller_id'] = $payment['alipay_partner'];
				$parameter['_input_charset'] = $charset;
				$parameter['notify_url'] = 'https://www.quickact.net/mobile/respond.php?code=alipay_cn';//return_url(basename(__FILE__, '.php'));
				$parameter['return_url'] = 'https://www.quickact.net/mobile/respond.php?code=alipay_cn';
				$parameter['subject'] = $order['order_sn'];
				...
				$parameter['show_url'] = 'https://www.quickact.net/mobile/respond.php?code=alipay_cn';
				if($real_method == 3){
					$parameter['currency'] = $payment['alipay_pay_currency'];
				...
				//$button = '<div style="text-align:center"><input type="button" onclick="window.open(\'https://mapi.alipay.com/gateway.do?'.$param. '&sign='.md5($sign).'&sign_type=MD5\')" value="' .$GLOBALS['_LANG']['pay_button']. '" class="main-btn main-btn-large"/></div>';
				$button = 'https://mapi.alipay.com/gateway.do?'.$param. '&sign='.md5($sign).'&sign_type=MD5';
				//file_put_contents('20200504.txt',date('Y-m-d H:i:s')."==b:".$button."\r\n" , FILE_APPEND);
				return $button;
...
//        print_r(md5($sign));echo '-----'.$_GET['sign'];die;
        ...
        /* 取得订单信息 */ //jx
        $sql = 'SELECT pl.log_id, oi.order_amount FROM ' . $GLOBALS['ecs']->table('pay_log') .
        ...
        /* 检查支付的金额是否相符 */
        if ($orderinfo['order_amount'] != $_GET['total_fee']){
            return false;
        }
        ...
        elseif ($_GET['trade_status'] == 'TRADE_SUCCESS')
        {
            ...
            /* 改变订单状态 */
            order_paid($order_sn, 2);
			$sql = 'update ' .$GLOBALS['ecs']->table('order_info') ." set pay_id=9,pay_name='支付宝' WHERE order_sn = '$order_sn' ";
