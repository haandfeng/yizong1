...
elseif ($_REQUEST['act'] == 'view')
{
    ...
    if (!$smarty->is_cached('exchange_goods.dwt', $cache_id))
    {
        ...
        else
        {
            ...
            $smarty->assign('description',        $goods['goods_brief']);
...
function get_exchange_goods_list($limit){
    ...
    setcookie('ECS[display]', $display, gmtime() + 86400 * 7);
        ...
        $goodslist = ajax_exchange_get_goods($children, $integral_min, $integral_max, $keyword, $ext,$limit, $sort, $order);
        if($display == 'grid')
        {
            if(count($goodslist) % 2 != 0)
            {
...
function ajax_exchange_get_goods($children, $min, $max, $keyword, $ext, $limit, $sort, $order)
{
    $display = $GLOBALS['display'];
    $where = "eg.is_exchange = 1 AND g.is_delete = 0 AND ".
             "($children OR " . get_extension_goods($children) . ')';
    ...
    if (!empty($keyword))
    {
        $where .= " AND g.keywords LIKE '%".$keyword."%' ";
    }
...
function exchange_get_keywords()
{
    /* 获得类别列表 */
    $sql = "SELECT DISTINCT g.keywords FROM " . $GLOBALS['ecs']->table('goods') ." AS g LEFT JOIN ". $GLOBALS['ecs']->table('exchange_goods') ." AS eg ON (g.goods_id = eg.goods_id)  WHERE eg.is_exchange = 1 AND g.is_delete = 0 AND g.keywords != ''";
