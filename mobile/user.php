	...
	{//file_put_contents('20191014.txt',"33\r\n" , FILE_APPEND); 
		...
		{ 
			if(! empty($_SERVER['QUERY_STRING']))
			{
				...
				{ 
					//$url = $GLOBALS['ecs']->url().urlencode($_REQUEST['href']); 
				}
...
//$allstr = file_get_contents("php://input");file_put_contents('20201108.txt',"allstr:".$allstr."\r\n" , FILE_APPEND); 
...
$ui_arr = array(
	'login','profile','order_list','order_detail','address_list','collection_list','message_list','tag_list',
       ...
       'supplier_reg','comment_order','address','set_address','follow_shop','account_manage','my_comment','shaidan_send','back_order',
		'add_onuser_bonus','back_order_act','back_list', 'center', 'binding_mobile', 'fenxiao'
);
...
{//file_put_contents('20201108.txt',"action3:".$action."\r\n" , FILE_APPEND); 
...
function action_default()
{
    ...
    $is_shiming = $db->getOne("select sm_status from " . $ecs->table('users') . " where user_id = $user_id");
    $smarty->assign('is_shiming', $is_shiming);
...
function action_login()
{
	...
	if(!empty($_REQUEST['href']))
	{
		$back_act=urlencode($_REQUEST['href']);
	} 
...
{ 
	...
	$back_act2 = isset($_REQUEST['back_act']) ? trim($_REQUEST['back_act']) : '';	
	$back_act2=urldecode($back_act2);
	$_wxt = isset($_REQUEST['_wxt']) ? trim($_REQUEST['_wxt']) : '';
	$captcha = intval($_CFG['captcha']);
//file_put_contents('20201108.txt',"abcd\r\nnocapt=".$nocapt." topatuin=".$topatuin."\r\n" , FILE_APPEND);
	if($nocapt!='abcd')
	{
		if(($captcha & CAPTCHA_LOGIN) && (! ($captcha & CAPTCHA_LOGIN_FAIL) || (($captcha & CAPTCHA_LOGIN_FAIL) && $_SESSION['login_fail'] > 2)) && gd_version() > 0)
		{
	...
	if($_wxt=='unionid')
	{
		$sql = "select user_name from " . $ecs->table('users') . " where wx_unionid='" . $username . "' and wx_unionid not in('err:40003','err:nosub')";
		$username_res = $db->query($sql);
		$kkk = 0;
		while($username_row = $db->fetchRow($username_res))
		{
	...
	}//file_put_contents('20201108.txt',"abcd\r\nnocapt=".$nocapt." topatuin=".$topatuin." username=".$username." password=".$password."\r\n" , FILE_APPEND);
	if($_wxt=='autoaccount')
	{
		$sql = "select user_name from " . $ecs->table('users') . " where user_id='" . $topatuin . "'";
		$username_res = $db->query($sql);
		$kkk = 0;
		while($username_row = $db->fetchRow($username_res))
		{
	...
	{ //file_put_contents('20201201.txt',"abcd0\r\nusername=".$username." password=".$password."\r\n" , FILE_APPEND);
	...
	else if($nocapt=='abcd'&&$topatuin==6)
	{ //file_put_contents('20210324.txt',"abcd0\r\nusername=".$username." \r\n" , FILE_APPEND);
		$cuid=$user->loginmsgunion($username);
		if($cuid>0)
		{
			update_user_info();
			recalculate_price(); 		
			$xcxsql="update ecs_users set xcx_id='".$_REQUEST['xcxopenid']."' where user_id=".$cuid;
			$GLOBALS['db']->query($xcxsql);
			echo $cuid;
		}
		else
		{
...
function action_act_edit_profile()
{
   ...
   if($_POST['password'])
   {
	...
	$profile = array(
		'user_id' => $user_id, 'user_name'=> $user_name,'shipscancode' => isset($_POST['shipscancode']) ? intval($_POST['shipscancode']) : 0,'sex' => isset($_POST['sex']) ? intval($_POST['sex']) : 0,'birthday' => $birthday,'other' => isset($other_only) ? $other_only : array()
	);
...
function action_add_onuser_bonus()
{
	$user = $GLOBALS['user'];
	$_CFG = $GLOBALS['_CFG'];
	$_LANG = $GLOBALS['_LANG'];
	...
	if(add_onuser_bonus($user_id, $type_id))
	{
		show_message($_LANG['add_bonus_sucess'], $_LANG['back_up_page'], 'user.php?act=add_bonus', 'info');
	}
	else
	{
		$GLOBALS['err']->show($_LANG['back_up_page'], 'user.php?act=add_bonus');
	}
}
...
//	}
	...
	foreach($goods_list as $key => $value)
	{
		...
		$goods_list[$key]['exchange_market_price'] = price_exchange_format($value['market_price']);
		$goods_list[$key]['exchange_goods_price'] = price_exchange_format($value['goods_price']);
		$goods_list[$key]['exchange_subtotal'] = price_exchange_format($value['subtotal']);
	}
	
	$suppliers_id = $goods_list[0]['suppliers_id'];
	$is_exchange = $GLOBALS['db']->getOne("SELECT is_exchange FROM `ecs_suppliers` WHERE suppliers_id = '".$suppliers_id."'");
		...
		if(is_array($payment_list))
		{
			foreach($payment_list as $key => $payment)
			{
                            ...
                            if($payment['pay_code'] == 'balance')
				{
					...
					$smarty->assign('pay_balance_id', $payment['pay_code']);
				}
			}
		}
...
//	}
	...
	$user = user_info($order['user_id']);
	$smarty->assign('rank_points', $user['rank_points']); 
	$smarty->assign('minpoints', $minpoints); 
	$smarty->assign('isksf', $isksf); 
	$smarty->assign('iszhigou', $iszhigou); 
...
function action_add_bonus()
{
	...
	if ($userid == 0)
    { 
		$back_act = 'user.php?act=add_bonus'; 
        ecs_header("Location: user.php?href=".urlencode($back_act)."\n");
        exit;
    }
	...
	$user_id = $GLOBALS['user_id'];
    $action = empty($_REQUEST['act'])?'':trim($_REQUEST['act']);
    $typeid = empty($_REQUEST['typeid'])?'0':trim($_REQUEST['typeid']);

	include_once (ROOT_PATH . 'includes/lib_transaction.php');
	 
	$bonus = get_new_bouns_list($user_id,$typeid);
...
function action_center ()
{
	...
	$sql = "SELECT headimg, rank_points, pay_points, card, dl_status FROM " . $ecs->table('users') . " WHERE user_id = $user_id";
	$row = $db->getRow($sql);
	...
	$card = $row['card'];
	$birday = $card > 0 ? date('Y-m-d',strtotime(substr($card, 6, 8))) : '';
	...
	$smarty->assign('birday',$birday);
	$smarty->assign('card',$card);
	$smarty->assign('dl_status',$dl_status);
...
function action_fenxiao()
{
	...
	if($_POST['real_name']){
		...
		if($row['status'] == '1'){
			...
			$row = $db->getRow($sql);
			if($row){
				show_message("您已经提交申请，审核中，请等待代理专员联系您！");
			}
		}
