...
function changeSurplusResponse(obj)
{
	if (obj.error) {
        ...
        {
          if(document.getElementById("ksf")!=null)
          {
            document.getElementById("ksf").style.display = "none"; 
          }
          if(document.getElementById("ckksf")!=null)
          { 
            document.getElementById("ckksf").style.display = "none";
          } 
        }
        else
        {
          if(document.getElementById("ksf")!=null)
          {
            document.getElementById("ksf").style.display = ""; 
          }
          if(document.getElementById("ckksf")!=null)
          { 
            document.getElementById("ckksf").style.display = "";
          } 
        }
				$('#pay_div').show();
...
function check_before_submit(){
  ...
  $('.tijiao_butn').attr("onclick","");

	if($("input[name='payment']:checked").val()=="1"||$("input[name='payment']:checked").val()=="9"||$("input[name='payment']:checked").val()=="10")
	{
		if(document.getElementById('iswei').value=="1")
		{
			alert("因合作限制，微信内用支付宝需要复制链接到浏览器才能完成支付。如有疑问请先联系客服，谢谢!");
		}
	}

  if (document.getElementById('mobile') && (Utils.isEmpty(document.getElementById('mobile').value)))
  {
    alert("手机号码不能为空");
    $('.tijiao_butn').attr("onclick","return check_before_submit()");
    return false;
   }
  else
  {
    if (document.getElementById('mobile') && !Utils.isMobile(document.getElementById('mobile').value))
    ...
    if($("input[name='payment']:checked").val()=="130")
    {
      open_quick_window();
    }
    else{
...
function open_quick_window(){
    document.getElementById("popup_winquick").style.display="";
}
...
function end_input_quick(){
    var surplus_password = document.getElementById("pwd").value;
    Ajax.call('flow.php?step=verify_quick_password','pwd='+surplus_password,check_quick_response,'GET','TEXT',true,true);
}
function check_quick_response(result){
    if(result != ''){
    document.getElementById('quickpwd').value=result;
        submit_the_form();
    }
    else{
        alert('密码错误！');
...
function cancel_input_quick(){
    document.getElementById("pwd").value="";
    document.getElementById("popup_winquick").style.display="none";
}
...
function checkOrderForm(frm)
{
  ...
  have_alipay=false;
  if(frm.elements['www_yshop100_com_bank']){
	  for(var iii=0;iii<frm.elements['www_yshop100_com_bank'].length;iii++)
	  {    
			if(frm.elements['www_yshop100_com_bank'][iii].checked)
			{
					have_alipay=true;
			}
	  }
