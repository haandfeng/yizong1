...
/*返回上一页*/
function goback(is_app) {
    if (parseInt(is_app) == 1) {
        if (isAndroid) {
            window.AndroidWebView.AppGoBack();
        } else if (isiOS) {
            AppGoBack();
        }
    } else {
...
function go_app_or_login(is_app) {
    if (parseInt(is_app) == 1) {
        if (isAndroid) {
            window.AndroidWebView.AppFinish();
        } else if (isiOS) {
            AppFinish();
        }
    } else {
...
/*返回上一页*/
function goService(is_app) {
    if (parseInt(is_app) == 1) {
        if (isAndroid) {
            window.AndroidWebView.AppToCustomerService();
        } else if (isiOS) {
            AppToCustomerService();
        }
    } else {
        window.location.href="http://q.url.cn/CDDunP?_type=wpa&qidian=true";
        // if ((navigator.userAgent.indexOf('MSIE') >= 0) && (navigator.userAgent.indexOf('Opera') < 0)) { // IE
        //     if (history.length > 0) {
        //         indow.location.href="login.jsp?backurl="+window.location.href;
        //         //window.history.go(-1);
        //     } else {
        //         window.opener = null;
        //         window.close();
...
/*JS调用APP首页*/
function H5ToAppIndex() {
    if (isAndroid) {
        window.AndroidWebView.AppIndex();
    } else if (isiOS) {
        AppIndex();
    }
}
...
/*JS调用APP分类页*/
function H5ToAppCatelog() {
    if (isAndroid) {
        window.AndroidWebView.AppCatelog();
    } else if (isiOS) {
        AppCatelog();
    }
}
...
/*JS调用APP会员中心页*/
function H5ToAppMember() {
    if (isAndroid) {
        window.AndroidWebView.AppMember();
    } else if (isiOS) {
        AppMember();
    }
}
...
/*JS调用APP购物车页*/
function H5ToAppCart() {
    if (isAndroid) {
        window.AndroidWebView.AppCart();
    } else if (isiOS) {
        AppCart();
    }
}
...
/*JS调用APP登录*/
function H5ToAppLogin(jumpurl) {
    if (isAndroid) {
        window.AndroidWebView.AppLogin(jumpurl);
    } else if (isiOS) {
        AppLogin(jumpurl);
    }
}
...
function addToCartResponse(result) {
    if (result.error > 0) {
        ...
        var cart_nums = result.goods_number;
        if (isAndroid) {
            window.AndroidWebView.AppToCart(cart_nums);
        } else if (isiOS) {
            AppToCart(cart_nums);
        }
    }
...
Ajax.call('flow.php?step=add_package_to_cart', 'packageId=' + packageId+'&number='+number, addPackageToCartResponse, 'POST', 'JSON');
}
