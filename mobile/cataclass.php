 ...
 * $Author: derek $
 * $Id: cataclass.php 17217 2011-01-19 06:29:08Z derek $
*/
...
if (!$smarty->is_cached('cataclass.dwt'))
{
    /* 取出所有分类 */
	...
	if($pid==0) {
        ...
        $key_list=get_categories();

        foreach ($key_list as $value)
         {
				$item_list[$value['name']]=array(
				'tid'       => $value['id'],
				'tb'       => get_categorieschild($value['id'])
				);   
         }    
...
function get_categorieschild($parentid)
{
		
	/* 获取当前分类及其子分类 */
	$sql = 'SELECT cat_id,cat_name ,parent_id,type_img,is_show ' .
			'FROM ' . $GLOBALS['ecs']->table('category') .
			" where parent_id=".$parentid." ORDER BY sort_order ASC, cat_id ASC"; 
	...
	{ 
			...
			$cat_arr[$row['cat_id']]['is_show'] = $row['is_show'];   
