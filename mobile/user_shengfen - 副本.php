...
function action_shiming ()
{
	...
	$sql = "select mobile_phone,status from " . $GLOBALS['ecs']->table('users') . " where user_id = '" . $_SESSION['user_id'] . "'";
...
file_put_contents("log/shiming_".date('Y-m-d',time()).".txt", date('Y-m-d H:i:s',time())."\r\n返回:".$out_put."\r\n", FILE_APPEND);
				...
				{
					$sql = 'update ' . $GLOBALS['ecs']->table('users') . " set status ='1' where user_id = '" . $_SESSION['user_id'] . "'";
				...
				{
					$sql = 'update ' . $GLOBALS['ecs']->table('users') . " set status ='3' where user_id = '" . $_SESSION['user_id'] . "'";
					$GLOBALS['db']->query($sql);
