...
//file_put_contents('20210617.txt',date('Y-m-d H:i:s')."order_id-"."SELECT order_id FROM ".$GLOBALS['ecs']->table('pay_log')." WHERE log_id = '$out_trade_no'"."\r\n" , FILE_APPEND); 
//获取订单信息
...
if($order)
{
	if ($order['order_amount'] > 0){
        ...
        $weixinconfig = $GLOBALS['db']->getRow ( "SELECT * FROM " . $GLOBALS['ecs']->table('weixin_config') . " WHERE `token` = '$pay_code'" );
        if(!$weixinconfig){
        	echo 1;exit; 
        }
		$weixin = new core_lib_wechat($weixinconfig);
		if(!$_GET['code']){
		    $url = $GLOBALS['ecs']->url()."weixinpayy.php?out_trade_no={$_GET['out_trade_no']}";
        ...
        //file_put_contents('20191023.txt',"jsApiParameters::".$jsApiParameters."\r\n" , FILE_APPEND);
			...
			WeixinJSBridge.invoke(
				...
				function(res){
					...
					if(res.err_msg == "get_brand_wcpay_request:ok" ) {
						window.location.href='respond_wx.php?act=ok&oid=<?php echo $order_id; ?>';
