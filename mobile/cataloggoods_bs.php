...
if ($_REQUEST['act'] == 'ajax') {
	...
	$size =10;
    $size = empty($_GET['size'])?$size:$_GET['size'];
    $page = empty($_GET['page'])?0:$_GET['page'];
	$last=$size*$page;
	
    $cat_id = empty($_GET['catid'])?0:$_GET['catid'];
	//if(!empty($_POST['last']))
	//{
	//	$size = empty($_POST['amount'])?$size:$_POST['amount'];
	//	$last = empty($_POST['last'])?0:$_POST['last'];
	//}
    $json = new JSON;

    $goodslist = category_getgoods($cat_id, $size, $last);

    foreach ($goodslist as $val) {
        $smarty->assign('goods', $val);
        $res[]['info'] = $GLOBALS['smarty']->fetch('library/goods_list_catalog_ajax.lbi');
    }
