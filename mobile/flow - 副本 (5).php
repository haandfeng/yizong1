...
}elseif ($_REQUEST['step'] == 'save_point')
{
	...
	$result['picktxt'] = "<input type='hidden' id='point".$suppid."' name='pickup_point[".$suppid."]' value='".$info['id']."'><span class='ziti'>指定配送地址：</span><span>".$info['shop_name']."</span><a href='javascript:void(0);' onclick='show(\"pop\",".$suppid.")' class='revise'>修改</a>";
	echo $json->encode($result);
    exit;
}
...
//	}
		...
		if(is_pups($recid)){
			// if(isset($consignee['city']) && intval($consignee['city'])>0){
				...
				if($pickinfo){
					$result['picktxt'] = "<input type='hidden' id='point".$suppid."' name='pickup_point[".$suppid."]' value='0'><a href='javascript:void(0);' onclick='show(\"pop\",".$suppid.");' class='pickup_point_btn'>请选择指定配送地址</a></script>";
				}
				foreach($pickinfo as $pkey=>$pval){
					if($consignee['district'] == $pval['district_id']){
						$result['picktxt'] = "<input type='hidden' id='point".$suppid."' name='pickup_point[".$suppid."]' value='".$pval['id']."'><span class='ziti'>指定配送地址：</span><span>".$pval['shop_name']."</span><a href='javascript:void(0);' onclick='show(\"pop\",".$suppid.")' class='revise'>修改</a>";
					}
				}
			// }
		}
