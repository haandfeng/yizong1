...
if(!empty($_REQUEST['back_act'])) 
{
	setcookie('back_act', urlencode($_REQUEST['back_act']), time()+3600);
file_put_contents('20211220.txt',date('Y-m-d H:i:s')."hasback_act:".$back_act."\r\n" , FILE_APPEND);
}
$back_act = isset($_COOKIE['back_act']) ? urldecode(trim($_COOKIE['back_act'])) : '';   
if($back_act == 'user.php') $back_act = 'index.php'; 
...
file_put_contents('20211220.txt',date('Y-m-d H:i:s')."code3:".$back_act."\r\n" , FILE_APPEND);
                if(!empty($back_act)){
...
file_put_contents('20211220.txt',date('Y-m-d H:i:s')."code4:".$back_act."\r\n" , FILE_APPEND);
    if(!empty($back_act)){
        header("Location:".$back_act);exit;
    }else{
        header("Location:index.php");exit;
