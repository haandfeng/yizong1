	    ...
	    if ($order['shipping_id'] > 0)
	    {
	        ...
	        if($shipping['shipping_code'] == 'pups'){
				...
				if($flagisadd){
					...
					$sqladd = $sqladd." SELECT '".$_SESSION['user_id']."', '门店', `email`, `country`, `province`, `city`, `district`, `address`, '".$pmobile."' FROM `ecs_user_address` WHERE user_id ='0' and consignee='" . $order['is_pickup'] . "'";
					...
					if($nadid>0)
					{
						$sqladd = "update ecs_users set address_id='".$nadid."' from ecs_users where user_id ='".$_SESSION['user_id']."'";
