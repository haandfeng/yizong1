...
$default_sort_order_type   = $_CFG['sort_order_type'] == '0' ? 'sort_order' : ($_CFG['sort_order_type'] == '1' ? 'shop_price' : 'goods_id');

$sort  = (isset($_REQUEST['sort'])  && in_array(trim(strtolower($_REQUEST['sort'])), array('goods_id', 'shop_price', 'last_update','salenum', 'sort_order'))) ? trim($_REQUEST['sort'])  : $default_sort_order_type;
...
function brand_get_goods($brand_id, $cate, $size, $page, $sort, $order, $limit='')
{
	...
	if($ishide=='1')
	{
		$where1=' and g.ishide=0 ';
	}

    $cate_where = ($cate > 0) ? 'AND ' . get_children($cate) : '';

    $cate_where = $cate_where . $where1;
	...
	if ($sort=='salenum')
	{
        ...
        $sql = 'SELECT g.goods_id, g.goods_name, g.market_price, g.shop_price AS org_price, ' .
            ...
            'LEFT JOIN ' . $GLOBALS['ecs']->table('member_price') . ' AS mp ' .
                "ON mp.goods_id = g.goods_id AND mp.user_rank = '$_SESSION[user_rank]' " .
            "WHERE g.is_on_sale = 1 AND g.is_alone_sale = 1 AND g.is_delete = 0 AND g.brand_id = '$brand_id' $cate_where".
            "ORDER BY $sort $order, second_order ASC, suppliers_id DESC $limit";
    }
