...
$smarty->assign('shipscancode',      $shipscancode); 
	...
	$smarty->assign('wap_index_ad_ad4',get_wap_advlist('首页4张广告位', 4));
    ...
    $smarty->assign('wap_bs_adv',get_wap_advlist('保税专区广告位', 10));
    $smarty->assign('wap_zy_adv',get_wap_advlist('直邮专区广告位', 10));
    $smarty->assign('wap_bsth_adv',get_wap_advlist('保税特惠广告位', 1));
    $smarty->assign('wap_pintuan_adv',get_wap_advlist('拼团广告位', 1));
    ...
    $smarty->assign('wap_all_title',get_wap_advlist('所有商品标题', 10));
...
{ 
	...
	//file_put_contents('20211220.txt',date('Y-m-d H:i:s')."index_get_group_buy:".$limit."\r\n" , FILE_APPEND);
    ...
    if ($limit > 0)
    {
        $sql = 'SELECT gb.*,g.*,gb.act_id AS group_buy_id, gb.goods_id, gb.ext_info, gb.goods_name, g.goods_thumb, g.goods_img ' .
                ...
                "WHERE 1=1 " . // gb.act_type = '" . GAT_GROUP_BUY . "'
...
//file_put_contents('20211220.txt',date('Y-m-d H:i:s')."index_get_group_buy:".$sql."\r\n" , FILE_APPEND);
