...
if($order_id > 0)
{
	$sql = "SELECT COUNT(*) FROM " . $GLOBALS['ecs']->table('order_info') . " WHERE order_id = '$order_id' ";
...
//               $smarty->assign('goods_list',$goods_list);
		...
		$wuliu = $GLOBALS['db']->getAll($sql);
                ...
                foreach($wuliu as $key=>$value){
                    ...
                    $kuaidi_list[$value['delivery_id']]['data'] = $result['Traces']; 
		 ...
		 $smarty->assign('Traceslength',count($result['Traces']));
...
$smarty->display('kuaidi_list_admin.dwt');
