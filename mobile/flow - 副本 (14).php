...
if ($_REQUEST['step'] == 'add_to_cart')
{
   include_once('includes/cls_json.php');
	...
	if ( $row_xg['is_buy'] == 1 && $row_xg['buymax'] >0 && $row_xg['buymax_start_date'] < $time_xg_now  && $row_xg['buymax_end_date'] > $time_xg_now  )
	{
		...
		else
		{
			...
			$num_cart_old_2=$GLOBALS['db']->getOne("select sum(og.goods_number) from ". $GLOBALS['ecs']->table('order_goods') ." AS og , ". $GLOBALS['ecs']->table('order_info') ." AS o where o.user_id='$_SESSION[user_id]' and  o.order_id = og.order_id and (o.order_status = '0' OR o.order_status = '1' OR o.order_status = '5') and add_time > ". $row_xg['buymax_start_date'] ." and add_time < ". $row_xg['buymax_end_date'] ."  and og.goods_id = " . $goods->goods_id );
   ...
   /* 判断是否为秒杀的商品，非促销时间内无法销售 */
   $off_seckill = $GLOBALS['db']->getOne("SELECT 1 FROM ".$GLOBALS['ecs']->table('goods')." WHERE goods_id = '".$goods->goods_id."' AND is_seckill = '1' AND (promote_start_date > '$time_xg_now' OR promote_end_date < '$time_xg_now')" );
   if($off_seckill){
        $result['error']   = 888;
...
elseif ($_REQUEST['step'] == 'consignee')
{
    ...
    else
    {
        ...
        ecs_header("Location: flow.php?step=checkout&zx=1\n");
        exit;
    }
}
...
elseif ($_REQUEST['step'] == 'checkout')
{
    ...
    if ($db->getOne($sql) == 0)
    {
		...
		foreach($goods_list as $k => $v)
		{
			...
			{
				$num_cart_old=$GLOBALS['db']->getOne("select sum(og.goods_number) from ". $GLOBALS['ecs']->table('order_goods') ." AS og , ". $GLOBALS['ecs']->table('order_info') ." AS o where o.user_id='$_SESSION[user_id]' and o.order_id = og.order_id and (o.order_status = '0' OR o.order_status = '1' OR o.order_status = '5') and add_time > ". $v['buymax_start_date'] ." and add_time < ". $v['buymax_end_date'] ."  and og.goods_id = " . $v['goods_id'] ); 
    ...
    if(empty($consignee['address'])){
        ...
        //如果有购买记录，获取上一次的手机号码
        $sql = "SELECT mobile FROM ".$ecs->table('order_info'). " WHERE user_id='". $_SESSION['user_id'] ."' ORDER BY order_id DESC LIMIT 1";
        $pre_mobile = $db->getOne($sql); 
        $smarty->assign('pre_mobile', $pre_mobile);
    }
...
$consignee['region'] = $db->getOne($sql); 
   ...
   $smarty->assign('shop_province_list', get_regions(1, $_CFG['shop_country']));
    ...
    if(count($cart_goods)>0){
    	...
    	foreach($cart_goods as $key => $val){
            ...
            if($i == 0){
                ...
                //人民币结算供应商
                $is_exchange =$GLOBALS['db']->getOne("SELECT is_exchange FROM " . $ecs->table('suppliers') . " WHERE suppliers_id = '".$suppliers_id."'");
                $smarty->assign('is_exchange', $is_exchange);
    ...
    //实名验证
    $sql = "select sm_status as status  from " . $GLOBALS['ecs']->table('users') . " where user_id = '" . $_SESSION['user_id'] . "'";
...
CART_EXCHANGE_GOODS))
    {
        // 取得用户可用红包
		$user_bonus = user_bonus($_SESSION['user_id'], $total['goods_price_supplier'], $cart_goods);//把参数由总金额改为每个店铺的订单金额
...
$user_bonus[$key];
	...
	if(isbonded())
	{
		$smarty->assign('allow_use_bonus', 0);
	}
...
}elseif ($_REQUEST['step'] == 'save_point')
{
	...
	$result['picktxt'] = "<input type='hidden' id='point".$suppid."' name='pickup_point[".$suppid."]' value='".$info['id']."'><span class='ziti'>指定配送地址：</span><span>".$info['shop_name']."</span><br/><a href='javascript:void(0);' onclick='show(\"pop\",".$suppid.")' class='pickup_point_btn'>更换体验店</a><div class='p-info1'><div class='p-price1'>地址：".$info['address']."</div><div class='p-price1'>联系人：".$info['contact']."</div><div class='p-price1'>电话：".$info['phone']."</div></div>";
	echo $json->encode($result);
    exit;
}
...
//	}
		...
		if(is_pups($recid)){
			// if(isset($consignee['city']) && intval($consignee['city'])>0){
				...
				if($pickinfo){
					$result['picktxt'] = "<input type='hidden' id='point".$suppid."' name='pickup_point[".$suppid."]' value='0'><a href='javascript:void(0);' onclick='show(\"pop\",".$suppid.")' class='pickup_point_btn'>更换体验店</a></script>";
				}
				$kn_pid=get_pickup_shopid();
				$o_pid=getorder_pickup_shopid();
				foreach($pickinfo as $pkey=>$pval){
					if($o_pid>0&&$o_pid== $pval['id']){
						$inf = get_pickup_one_info($pval['id']);
						$result['picktxt'] = "<input type='hidden' id='point".$suppid."' name='pickup_point[".$suppid."]' value='".$pval['id']."'><span class='ziti'>指定配送地址：</span><span>".$pval['shop_name']."</span><br/><a href='javascript:void(0);' onclick='show(\"pop\",".$suppid.")' class='pickup_point_btn'>更换体验店</a><div class='p-info1'><div class='p-price1'>地址：".$inf['address']."</div><div class='p-price1'>联系人：".$inf['contact']."</div><div class='p-price1'>电话：".$inf['phone']."</div></div>";
						break;
					}
					else if($consignee['district'] == $pval['district_id']){
						$inf = get_pickup_one_info($pval['id']);
						$result['picktxt'] = "<input type='hidden' id='point".$suppid."' name='pickup_point[".$suppid."]' value='".$pval['id']."'><span class='ziti'>指定配送地址：</span><span>".$pval['shop_name']."</span><br/><a href='javascript:void(0);' onclick='show(\"pop\",".$suppid.")' class='pickup_point_btn'>更换体验店</a><div class='p-info1'><div class='p-price1'>地址：".$inf['address']."</div><div class='p-price1'>联系人：".$inf['contact']."</div><div class='p-price1'>电话：".$inf['phone']."</div></div>";
						break;
					}
					else if($kn_pid>0&&$kn_pid==$pval['id']){
						$inf = get_pickup_one_info($pval['id']);
						$result['picktxt'] = "<input type='hidden' id='point".$suppid."' name='pickup_point[".$suppid."]' value='".$pval['id']."'><span class='ziti'>指定配送地址：</span><span>".$pval['shop_name']."</span><br/><a href='javascript:void(0);' onclick='show(\"pop\",".$suppid.")' class='pickup_point_btn'>更换体验店</a><div class='p-info1'><div class='p-price1'>地址：".$inf['address']."</div><div class='p-price1'>联系人：".$inf['contact']."</div><div class='p-price1'>电话：".$inf['phone']."</div></div>";
						break;
					}
		...
		if($recid=='33')
		{
			$result['picktxt'] = "<a href='flow.php?step=consignee'  class='pickup_point_btn'>更改其他指定地址</a>";
		}
    }
    echo $json->encode($result);
    exit;
...
{	
    ...
    if (empty($_SESSION['pintuan_level'])==false && empty($_SESSION['extension_id'])==false && empty($_SESSION['extension_code'])==false )
    {  
        $pintuansql="INSERT INTO ecs_pintuan(user_id, user_nickname, user_head, act_id, act_type, need_people,available_people,  ";
		$pintuansql=$pintuansql."price, create_time, end_time, create_succeed, status) VALUES ('".$_SESSION['user_id']."', '".$_SESSION['pintuan_nickname']."', '".$_SESSION['pintuan_headimgurl']."', ";
		$pintuansql=$pintuansql."'".$_SESSION['extension_id']."', '".$_SESSION['act_type']."', '".$_SESSION['need_people']."',0,";
		$pintuansql=$pintuansql."'".$_SESSION['pintuan_price']."', NOW(), DATE_ADD(now(),INTERVAL ".$_SESSION['pintuan_time_limit']." HOUR), 0, 0)";
		$GLOBALS['db']->query($pintuansql); 
		$ptid = $GLOBALS['db']->insert_id();
		unset($_SESSION['pintuan_level']);
		unset($_SESSION['extension_id']);
		unset($_SESSION['extension_code']);
		$_SESSION['pintuan_pt_id']=$ptid;
		//file_put_contents('20211210.txt',"pintuansql:".$pintuansql."\r\n" , FILE_APPEND); 
		//file_put_contents('20211210.txt',"ptid:".$ptid."\r\n" , FILE_APPEND); 
    }
	...
	$order_zhe = isset($_POST['zhe']) ? $_POST['zhe'] : 10;
    ...
    foreach ($cart_goods_new as $ckey=>$cval){
        ...
        $order['goods_amount_exchange'] = $order['exchange_goods_amount'] = $total['goods_price_exchange'];
        $order['exchange_discount'] = $total['discount_exchange'];
        $order['exchange_surplus'] = $order['surplus_exchange'] = $total['surplus_exchange'];
        
		if($order['surplus'] > 0){
			$order_surplus_exchange = number_format($order_surplus, 2, '.', '');
			//前台的总余额减去每一个平台方或入驻商家使用的余额,结果做为下一个商家使用的余额
			$order_surplus = $order_surplus - $order['surplus'];
			$order_surplus_exchange = $order_surplus_exchange - $order['surplus_exchange'];
	    ...
	    if ($order['shipping_id'] > 0)
	    {
	        ...
	        if($shipping['shipping_code'] == 'pups'){
				...
				//自提订单且没有收货人信息，记录手机号码，保存默认地址
				if($flagisadd){
					$pickup_point = isset($_POST['pickup_point'][$ckey]) ? $_POST['pickup_point'][$ckey] : 0;
					$pmobile = $_POST['mobile'];
					$sqladd = "insert into ecs_user_address (`user_id`, `consignee`, `email`, `country`, `province`, `city`, `district`, `address`, `mobile`) ";
					$sqladd = $sqladd." SELECT '".$_SESSION['user_id']."', '门店', `email`, `country`, `province`, `city`, `district`, `address`, '".$pmobile."' FROM `ecs_user_address` WHERE user_id ='0' and consignee='" . $pickup_point . "'";
					//file_put_contents('20191206.txt',"sqladd1:".$sqladd."\r\n" , FILE_APPEND); 
					$GLOBALS['db']->query($sqladd); 
					$sqladd = "select address_id from ecs_user_address where user_id ='".$_SESSION['user_id']."' and consignee='门店' order by address_id desc LIMIT 1";
					//file_put_contents('20191206.txt',"sqladd2:".$sqladd."\r\n" , FILE_APPEND); 
					$nadid=$GLOBALS['db']->getOne($sqladd); 
					//file_put_contents('20191206.txt',"sqladd3:".$nadid."\r\n" , FILE_APPEND); 
					if($nadid>0)
					{
						$sqladd = "update ecs_users set address_id='".$nadid."' where user_id ='".$_SESSION['user_id']."'";
						//file_put_contents('20191206.txt',"sqladd4:".$sqladd."\r\n" , FILE_APPEND); 
						$GLOBALS['db']->query($sqladd); 
					}
				}
	        }
		...
		if($order_zhe<10)
		{
			if($order_zhe>0)
			{
				$order['bonus']        = $order['goods_amount']+$order['shipping_fee']-$order['order_amount'];
                
    			$order['exchange_amount'] =$order['exchange_amount']*floatval($order_zhe)/10;
			}
		}
    	...
    	/*发票金额*/
        if($is_exchange == 1){
            $order['inv_money'] =  $order['exchange_amount'] ;
        }else{
		...
		//if ($order['order_amount'] > 0)
	    {
	        if($order['surplus'] >0) //余额支付里如果输入了一个金额
	        {
                ...
                $order['exchange_amount'] = $order['exchange_amount'] + $order['surplus_exchange'];
	        ...
	        else
	        {
	            ...
	            $order['surplus_exchange'] = $order['exchange_amount'];
				//是否开启余额变动给客户发短信-用户消费
...
//                    }
    	...
    	$all_order_amount_exchange += $order['exchange_amount'];
    	user_uc_call('add_feed', array($order['order_id'], BUY_GOODS)); //推送feed到uc
    }
	if (empty($_SESSION['pintuan_pt_id'])==false)
    {  
        $pintuansql="INSERT INTO ecs_pintuan_orders(pt_id,act_id,act_user,follow_user,follow_user_nickname,follow_user_head,order_id,follow_time) ";
		$pintuansql=$pintuansql." select pt_id,act_id,user_id,'".$_SESSION['user_id']."',(select user_name from ecs_users where user_id='".$_SESSION['user_id']."'),(select headimg from ecs_users where user_id='".$_SESSION['user_id']."'),'".$order['order_id']."',now() from ecs_pintuan where pt_id='".$_SESSION['pintuan_pt_id']."'";
		 //file_put_contents('20211210.txt',"pintuansql2:".$pintuansql."\r\n" , FILE_APPEND);
		$GLOBALS['db']->query($pintuansql); 
		$pintuansql="select count(1) from ecs_pintuan where user_id=".$_SESSION['user_id']." and pt_id=".$_SESSION['pintuan_pt_id'];
		$ismy=$GLOBALS['db']->getOne($pintuansql); 
		//file_put_contents('20211210.txt',"pintuansql3:".$pintuansql."\r\n" , FILE_APPEND);
		if($ismy==1)
		{ 
			setcookie("pintuan_flagnew",$_SESSION['pintuan_pt_id'], time()+3600);
			//file_put_contents('20211210.txt',"pintuan_flagnew:".$_COOKIE["pintuan_flagnew"]."\r\n" , FILE_APPEND);
		}
		unset($_SESSION['pintuan_pt_id']);
    }
	...
	$order['exchange_amount'] = $all_order_amount_exchange;
	...
	if ($order['order_amount'] > 0)
    {
		...
		RecordsFile( ROOT_PATH.'data/logs/','test-'.date('Y-m-d').'.txt',['botton' => $pay_online,'pay_code'=>$payment['pay_code'],'order_amount'=>$order['order_amount']]);
	...
	if ($is_package == 0)
	{
		...
		if ( $row_xg['is_buy'] == 1 && $row_xg['buymax'] >0 && $row_xg['buymax_start_date'] < $time_xg_now  && $row_xg['buymax_end_date'] > $time_xg_now  )
		{
			...
			else
			{
				...
				$num_cart_old_2=$GLOBALS['db']->getOne("select sum(og.goods_number) from ". $GLOBALS['ecs']->table('order_goods') ." AS og , ". $GLOBALS['ecs']->table('order_info') ." AS o where o.user_id='$_SESSION[user_id]' and o.order_id = og.order_id and (o.order_status = '0' OR o.order_status = '1' OR o.order_status = '5') and add_time > ". $row_xg['buymax_start_date'] ." and add_time < ". $row_xg['buymax_end_date'] ."  and og.goods_id = " . $goods_id );
				$num_cart_old = $num_cart_old_1 + $num_cart_old_2 ;
...
{
	$smarty->assign('isappli',0);
}
else
{
	$smarty->assign('isappli',1);
}
assign_dynamic('shopping_flow');
$smarty->assign('zx', $_REQUEST['zx']);
...
function getorder_pickup_shopid()
{ 
	$pickup_shopid=0;
	$res=$GLOBALS['db']->getOne("select pickup_point from " . $GLOBALS['ecs']->table('order_info') . " WHERE pickup_point>0 and user_id='" . $_SESSION['user_id'] . "' order by order_id desc limit 1");
	if($res=='')
	{}
	else
	{$pickup_shopid=$res;}
	return $pickup_shopid;
}
function get_pickup_shopid()
{
	$pickup_shopid=0;
	$usercity=$GLOBALS['db']->getOne("select city from " . $GLOBALS['ecs']->table('weixin_user') . " WHERE ecuid='" . $_SESSION['user_id'] . "'");
	if($usercity!="")
	{
	$pickup_shopid=$GLOBALS['db']->getOne("SELECT id FROM `ecs_pickup_point` p inner join ecs_region on ecs_region.region_id=p.city_id where region_name='".$usercity."'");
	}
	return $pickup_shopid;
}
