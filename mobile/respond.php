...
else
{
    ...
    else
    {
        ...
        if (file_exists($plugin_file))
        {
			...
			else if($_GET['code'] == 'weixin_cn')
            {
				$orderid= @$payment->get_out_trade_no();
			}
        }
        else
        {
	...
	function getshiming($order_sn)
	{
		$msg='';
		$sql = "select sm_status from ecs_users where exists (select 1 from ecs_order_info where order_sn='$order_sn' and ecs_order_info.user_id=ecs_users.user_id)";
		...
		{
			$msg="<br/>为了方便购物，需要补充实名信息姓名+身份证号码 <a href='/mobile/user_shengfen.php?act=shiming' style='color:red'>去填写</a>";
		}
		return $msg;
	}	
	if($orderid!="")
	{
		$msg=$msg.getshiming($orderid);
	}
assign_template();
