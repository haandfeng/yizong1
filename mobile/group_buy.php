...
require_once(ROOT_PATH . 'includes/prince/lib_common.php');
...
if ($_REQUEST['act'] == 'asynclist')
{
    ...
    if (is_array($goodslist)) {
        ...
        <div class="proInfo-wrap"> <a href="' . $vo['url'] . '" >
          ...
          <div class="ptPrice">
             <div class="ptPrice1">
			 <em >' . $vo['lowest_amount'] ."人起团".$s1."&nbsp;&nbsp;&nbsp;&nbsp;".$s2."￥".$vo['lowest_price'].'</em>
			 <em style="float:right;font-size:15px;" >去开团></em>
			 </div> 
          </div>
          <br /><div  class="mkPrice" >
            <em>原价：</em> 
            <del >' . "￥".$vo['single_buy_price'] . '</del> 
            <em>销量：'.$vo['sold'].'</em> 
          </div></a> 
        </div>'
            );
   ...
   //  print_r( $goodslist  );
    ...
    if (!$smarty->is_cached('pintuan_list.dwt', $cache_id))
    {
        ...
        $smarty->assign('page_title', $position['title']);    // 页面标题
        $smarty->assign('ur_here',    $position['ur_here']);  // 当前位置
        $smarty->assign('categories', get_categories_tree()); // 分类排行
...
elseif ($_REQUEST['act'] == 'view')
{
    ...
    if ($userid == 0)
    {
		...
		{
			$back_act = 'pintuan.php?' . strip_tags($_SERVER['QUERY_STRING']);
		}
	...
	if(empty($_SESSION['user_id']) || $_SESSION['user_id']==0){
		...
		$sql = "UPDATE ".$GLOBALS['ecs']->table('weixin_user')."  SET continue_url = '". $continue_url . "'".
			   " WHERE fake_id = '" . $_SESSION['wechat_id'] . "'";
		$db->query($sql);
	}
    ...
    if (empty($pintuan))
    {
	    ...
	    $qcode = $GLOBALS['db']->getOne("SELECT qcode FROM ".$GLOBALS['ecs']->table('weixin_qcode')."  WHERE type=4 and content = '$affiliate_u'");

		$smarty->assign('qcode',    $qcode);  // 图片二维码
    ...
    $smarty->display('pintuan_goods.dwt');
}
...
elseif ($_REQUEST['act'] == 'pt_view')
{
	...
	//PRINCE 120029121
	if(strpos($pintuan['goods_thumb'],'ttp')>0){
		$pintuan['goods_thumb']=$pintuan['goods_thumb'];
	}else{
		$pintuan['goods_thumb']=$config['site_url'] .'../'. $pintuan['goods_thumb'];
	}

    /* 缓存id：语言，拼团活动id，状态，（如果是进行中）当前数量和是否登陆*/
    //$cache_id = $_CFG['lang'] . '-' . $pintuan_id . '-' . $pintuan['status'];
    //$cache_id = sprintf('%X', crc32($cache_id));
//file_put_contents('20211210.txt',"pt_view:".$cache_id."\r\n" , FILE_APPEND);
    /* 如果没有缓存，生成缓存 */
...
//file_put_contents('20211210.txt',"my_pintuan:".$sql."\r\n" , FILE_APPEND);
       ...
       $sql = "SELECT user_id " .
            "FROM  " . $GLOBALS['ecs']->table('pintuan') . " AS p  " . 
            "WHERE p.pt_id=".$pintuan_id."";
       $tuanuserid = $GLOBALS['db']->getOne($sql);
	   if($tuanuserid==$_SESSION['user_id'])//自己的团
	   {
		     if($my_pintuan['pay_status']==0 && $my_pintuan['order_status']<=1){
				 $right_action="立即付款";
    ...
    $smarty->assign('now_time',  gmtime());           // 当前系统时间
    $smarty->display('pintuan_view.dwt');
}
...
elseif ($_REQUEST['act'] == 'buy')
{
    ...
    if (empty($pintuan))
    {
	 ...
	 if($pt_id ==0 && $pintuan_level!=0){
			...
			if ($total>=$pintuan['open_limit']  && $pintuan['open_limit']!=0){    
				$_SESSION['pt_tips']="抱歉！您已经有 ".$total." 个进行中的拼团 暂时不能继续发起拼团，快快点击左下方\"我的拼团\"您的拼团分享给好友吧";
	...
	file_put_contents('20211210.txt',"buy:".$pt_id."\r\n" , FILE_APPEND);
	...
	if($pintuan_level>1){
		if($pt_id==0)
		{
			...
			$_SESSION['need_people'] = get_lowest_amount( $pintuan['price_ladder']);  
			$_SESSION['act_type'] = $pintuan['act_type'];
