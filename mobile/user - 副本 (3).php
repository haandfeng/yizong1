...
function action_fenxiao()
{
	...
	if($_POST['real_name']){
		...
		$sql = "UPDATE " . $ecs->table('users') . " SET is_fenxiao = '1' , sm_status = '2' WHERE user_id = '".$user_id."'";
		$db->query($sql);
		...
		$sql ="SELECT sm_status FROM " . $ecs->table('users') . " WHERE is_fenxiao = '1' AND user_id = '".$user_id."'";
		$row = $db->getRow($sql);
		if($row['status'] == '1'){
