...
function action_do_card_binding ()
{
	// 获取全局变量
	$user = $GLOBALS['user'];
	$_CFG = $GLOBALS['_CFG'];
	...
	$card = $_POST['card'];file_put_contents('test001.txt', $_FILES['face_card']['tmp_name'],FILE_APPEND);
	...
	if($rows['face_card'] == '')
	{
		if($face_card == '')
		{
			show_message('请上传身份证正面照！', '返回上一页', 'security.php?act=card_binding', 'info');
		}
	}
	...
	if($rows['back_card'] == '')
	{
		if($back_card == '')
		{
			show_message('请上传身份证背面照！', '返回上一页', 'security.php?act=card_binding', 'info');
		}
	}
	...
	if($num > 0)
	{
		show_message('您已申请实名认证，请等待管理员的审核！', '返回上一页', 'security.php?act=card_binding');
	}
	else
	{
		show_message('实名认证失败！', '返回上一页', 'security.php?act=card_binding');
	}
}
/**
 * 绑定邮箱
 ...
 * 绑定邮箱
 */
function action_to_card_binding ()
{
	// 获取全局变量
	$_LANG = $GLOBALS['_LANG'];
	$smarty = $GLOBALS['smarty'];
	...
	// 获取验证方式
	$smarty->assign('step', 'step_2');
	$smarty->assign('action', 'card_binding');
...
function get_validate_types ($user_id)
{
	...
	if(count($validate_types) == 0)
	{
		...
		else if($action == 'card_binding' && empty($card))
		{
			$smarty->assign('first_binding', '1');
			$_SESSION['security_validate'] = true;
			action_to_card_binding();
			exit;
		}
	}
