...
function action_register ()
{
    ...
    $db = $GLOBALS['db'];
    $ecs = $GLOBALS['ecs'];
//file_put_contents('20201108.txt',"register\r\n\r\n" , FILE_APPEND);
    /* 增加是否关闭注册 */
...
//file_put_contents('20201108.txt',"register000\r\n\r\n" , FILE_APPEND);

        if(strlen($_POST['invite_code'])>0 && !preg_match('/^[a-zA-Z0-9]{6}$/',$_POST['invite_code'])){
            show_message('邀请码错误', $_LANG['sign_up'], 'register.php', 'error');
        }
        $invite = !empty($_POST['invite_code']) ? trim($_POST['invite_code']) : '0';
...
// 		}
//file_put_contents('20201108.txt',"registeraaa\r\n\r\n" , FILE_APPEND);
...
//file_put_contents('20201108.txt',"register111\r\n\r\n" , FILE_APPEND);
...
//file_put_contents('20201108.txt',"register222\r\n\r\n" , FILE_APPEND);
        ...
        else if($register_type == "mobilexcx")
        {
            require_once (ROOT_PATH . 'includes/lib_validate_record.php');
            $db = $GLOBALS['db'];
			$mobile_phone = ! empty($_POST['mobile_phone']) ? trim($_POST['mobile_phone']) : '';
            $wx_unionid = ! empty($_POST['wx_unionid']) ? trim($_POST['wx_unionid']) : '';
			...
			if($cc==0)
			{
				...
				if($result)
				{
					...
					//unionid
					$sql = "update " . $ecs->table('users') . " set wx_unionid='".$wx_unionid."',froms='xcx',parent_id=".$parentid." where user_id='".$userid."'";
					$db->query($sql);
					echo $userid;exit;
				}
				else{
					echo $GLOBALS['err'];exit;
				}
			}
			else
			{
        ...
        else if($register_type == "wxxcx")
        {
            require_once (ROOT_PATH . 'includes/lib_validate_record.php');
            $db = $GLOBALS['db']; 
            $wx_unionid = ! empty($_POST['wx_unionid']) ? trim($_POST['wx_unionid']) : '';
            $xcx_id = ! empty($_POST['xcx_id']) ? trim($_POST['xcx_id']) : '';
            $parentid = ! empty($_POST['parentid']) ? trim($_POST['parentid']) : '0';
             
			$sql = "select count(user_id) from " . $ecs->table('users') . " where xcx_id='".$xcx_id."' or wx_unionid='".$wx_unionid."' ";
			$cc=$db->getOne($sql);
			if($cc==0)
			{ 
				$username = 'xc_'.substr($xcx_id,8,9);
 
				$result = register_by_xcx($username, $password, $wx_unionid.','.$xcx_id, $other);
				...
				{  
					...
					$userid=$db->getOne($sql);
					//unionid
					$sql = "update " . $ecs->table('users') . " set froms='xcx',parent_id=".$parentid." where user_id='".$userid."'";
					$db->query($sql);
					echo $userid;exit;
				}
				else{ 
        ...
        else if($register_type == "appxcx")
        {
			...
			$xcx_id = '';
            $parentid = ! empty($_POST['parentid']) ? trim($_POST['parentid']) : '0';
             //file_put_contents('20201108.txt',"unionid\r\n=".$wx_unionid."\r\n" , FILE_APPEND);
			$sql = "select count(user_id) from " . $ecs->table('users') . " where wx_unionid='".$wx_unionid."' ";
			$cc=$db->getOne($sql);
			if($cc==0)
			{ //file_put_contents('20201108.txt',"cc=".$cc."\r\n" , FILE_APPEND);
				$username = 'ap_'.substr($wx_unionid,8,9);
 //file_put_contents('20201108.txt',"username=".$username."\r\n" , FILE_APPEND);
				$result = register_by_app($username, $password, $wx_unionid.','.$xcx_id, $other);
 //file_put_contents('20201108.txt',"register_by_app=".$result."\r\n" , FILE_APPEND);
				...
				{  
					...
					$userid=$db->getOne($sql);
					//unionid
					$sql = "update " . $ecs->table('users') . " set froms='app',parent_id=".$parentid." where user_id='".$userid."'";
					$db->query($sql);
					echo $userid;exit;
				}
				else{ 
			...
			{//file_put_contents('20201108.txt',"00\r\n" , FILE_APPEND);
				echo 0;exit;
			}
        }
        else
...
//            $GLOBALS['db']->query("UPDATE ".$GLOBALS['ecs']->table('users')." SET is_fenxiao = 2 WHERE user_id = '" . $_SESSION['user_id'] . "'");
            ...
            if(0 < (int)$_POST['goods_id']){
                $link_lang[] = '查看商品';
                $link[] = 'goods.php?id='.$_POST['goods_id'];
            }
            $link_lang[] = '进入个人中心';
            $link[] = '/mobile/user.php';
            ...
            show_message('注册成功', $link_lang,$link, 'info');
            exit();
