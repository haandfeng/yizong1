...
$smarty->assign('app_user_id',$_SESSION['user_id']);

if(!empty($_REQUEST['act']) && $_REQUEST['act'] == 'get_good_code_act')
{


    if(!isset($_SESSION['user_id']) || empty($_SESSION['user_id'])){
        echo json_encode(array('error' => 1, 'message' => '请登录后再获取商品推广二维码', 'info' => $select_info));
        exit;
    }

    if(empty($_REQUEST['goods_id'])){
        echo json_encode(array('error' => 2, 'message' => '系统繁忙', 'info' => $select_info));
        exit;
    }
    $user_id = $_SESSION['user_id'];
    $goods_id = $_REQUEST['goods_id'];
    $res = $GLOBALS['db']->getOne("SELECT goods_id FROM ". $GLOBALS['ecs']->table('goods') ." WHERE goods_id = '{$goods_id}'");
    if(!$res){
        echo json_encode(array('error' => 2, 'message' => '系统繁忙', 'info' => $select_info));
        exit;
    }
...
//    if(!$qr_code){
//        /*********** 二维码 ***********/
//        require(dirname(ROOT_PATH).'/includes/phpqrcode.php');
//        $errorCorrectionLevel = 'L';//容错级别
//        $matrixPointSize = 6;//生成图片大小
//        if(!file_exists(dirname(ROOT_PATH).'/images/good_user_qr_code/'.date('Ym'))){
//            mkdir(dirname(ROOT_PATH).'/images/good_user_qr_code/'.'/'.date('Ym'), 0777);
//        }
//        $filename = dirname(ROOT_PATH).'/images/good_user_qr_code/'.date('Ym').'/'.$user_id.'_'.$goods_id.'.png';
//        $file = date('Ym').'/'.$user_id.'_'.$goods_id.'.png';
//
//        $invite_code_self =  $GLOBALS['db']->getOne("SELECT invite_code FROM ". $GLOBALS['ecs']->table('users') ." WHERE user_id = '{$user_id}'");
//        $data = 'http://'.$_SERVER['HTTP_HOST'].'/mobile/register.php?goods_id='.$goods_id.'&id='.$user_id.'|'.$invite_code_self;
//        //生成二维码图片
//        QRcode::png($data,$filename, $errorCorrectionLevel, $matrixPointSize, 2);
//        /*********** 二维码 ***********/
//        $time = time();
//        $ins_res = $GLOBALS['db']->query("INSERT INTO ". $GLOBALS['ecs']->table('goods_qr_code') ."(id, user_id, goods_id, code_url, addtime) value(null,'{$user_id}','{$goods_id}','{$file}','{$time}')");
//        if(!$ins_res){
//            echo json_encode(array('error' => 3, 'message' => '系统繁忙', 'info' => $select_info));
//            exit;
//        }
//        $qr_code['code_url'] = $file;
//
//    }
    $res = array('goods_id'=>$goods_id,'user_id'=>$user_id,'qr_code'=>$invite_code_self);
    echo json_encode(array('error' => 0, 'message' => '获取成功', 'info' => $res));
    exit;
}
...
if (!empty($_REQUEST['act']) && $_REQUEST['act'] == 'price')
{
    ...
    else
    {
        ...
        //供应商为人民币结算
        $is_exchange = $GLOBALS['db']->getOne("select ss.is_exchange from ".$GLOBALS['ecs']->table('goods')." as g left join ".$GLOBALS['ecs']->table('suppliers')." as ss on g.suppliers_id = ss.suppliers_id where g.goods_id = $goods_id");
        if($is_exchange == '1'){
            $res['result'] = price_exchange_format($shop_price * $number);
        }else{
        ...
        //秒杀
        $off_seckill = $GLOBALS['db']->getOne("SELECT 1 FROM ".$GLOBALS['ecs']->table('goods')." WHERE goods_id = $goods_id AND is_seckill = '1' AND (promote_start_date > '$now' OR promote_end_date < '$now')" );
        $res['off_seckill'] = $off_seckill ? 1 : 0;
    }
    die($json->encode($res));
}
...
if (!$smarty->is_cached('goods.dwt', $cache_id))
{
    ...
    /* 代码增加 By  www.yshop100.com 自提点 End */
    ...
    else
    {
        ...
        //商品详情页面显示的价格为 shop_price_formated 所以需要计算出一个 港币价格
        $goods['hkd_shop_price_formated'] = $goods['shop_price_formated']*$_CFG['exchange_rate'];


        //税费新增
        if ($goods['tax_id'] > 0){
            $sql = "SELECT tax_rate, tax_discount FROM zs_tax WHERE tax_id = '$goods[tax_id]'";
            $row = $db->getRow($sql);

            if($row){
                if($row['tax_rate'] > 0){
                    if($goods['promote_price_org'] > 0){
                        $goods['price_tax'] = $goods['promote_price_org']*$row['tax_rate'];
                    }else{
                        $goods['price_tax'] = $goods['shop_price']*$row['tax_rate'];
                    }
                    
                    if($row['tax_discount'] > 0){
                        $goods['price_tax'] = $goods['price_tax']*$row['tax_discount'];
                    }
                    //转换人民币
                    $goods['price_tax'] = number_format($goods['price_tax']*$GLOBALS['_CFG']['exchange_rate'], 2, '.', '');
                    $goods['price_tax'] = '预计进口税：CNY '.$goods['price_tax'];
                }
            }
            $smarty->assign('price_tax', $goods['price_tax']);
        }

        $path = '/<img.*?src="(.*?)".*?>/is';
        //使用正则
        preg_match_all($path,$goods['goods_desc'],$array,PREG_PATTERN_ORDER);
       //图片路径
        $smarty->assign('goods_path_img',$array[1]);
        $ishide_path_img = $db->getOne("SELECT ishide_path_img FROM " .$ecs->table('goods'). " WHERE goods_id = '".$goods['goods_id']."'");
        $smarty->assign('ishide_path_img',$ishide_path_img);
        //视频路径
        $path = '/<source.*?src="(.*?)".*?>/is';
        preg_match_all($path,$goods['goods_desc'],$video,PREG_PATTERN_ORDER);
        $smarty->assign('goods_path_video',$video[1]);
		...
		if($goods_comcontent_TOP_BOTTOM=='0')
		{
			$smarty->assign('goods_comcontent_top',              ($goods_comcontent));
			$smarty->assign('goods_comcontent_bottom',              '');
		}
		else
		{
			$smarty->assign('goods_comcontent_top',              '');
			$smarty->assign('goods_comcontent_bottom',              ($goods_comcontent));
		}
        ...
        $results_www_yshop100_com = $GLOBALS['db']->getAll($sql);
        $return_arr = array();
        foreach ($results_www_yshop100_com as $value_yshop100)
        {
            $return_arr[$value_yshop100['goods_attr_id']] = $value_yshop100['attr_value'];
        }
        ...
        foreach ($properties['spe'] AS $skey_yshop100=>$sval_yshop100)
        {
            $hahaha_zhyh = 0;
            $sskey_www_yshop100_com = '-1';
            foreach ($sval_yshop100['values'] AS $sskey_yshop100=>$ssval_yshop100)
            {
                if ( is_exist_prod($selected_first, $ssval_yshop100['id'], $prod_exist_arr))
                {
                    $hahaha_zhyh = $hahaha_zhyh ? $hahaha_zhyh : $ssval_yshop100['id'];
                    $sskey_www_yshop100_com = ($sskey_www_yshop100_com != '-1') ? $sskey_www_yshop100_com : $sskey_yshop100;
                }
                else
                {
                    $properties['spe'][$skey_yshop100]['values'][$sskey_yshop100]['disabled'] = "disabled";
                }

                if ($skey_yshop100==$attr_id_gallery)
                {
                    $goods_attr_id_qq = $ssval_yshop100['id'] ;
                    ...
                    $properties['spe'][$skey_yshop100]['values'][$sskey_yshop100]['goods_attr_thumb'] = $db->getOne($sql_qq_qq87139667);
                }
            }
            if ($hahaha_zhyh)
            {
                $selected_first[$skey_yshop100] =  $hahaha_zhyh;
            }
            if ($sskey_www_yshop100_com!='-1')
            {
                $properties['spe'][$skey_yshop100]['values'][$sskey_www_yshop100_com]['selected_key_yshop100'] = "1";
            }
        }
...
//        print_r(get_goods_gallery($goods_id));
		...
		$count = $GLOBALS['db']->getOne("SELECT COUNT(*) FROM " . $GLOBALS['ecs']->table('comment') . " where comment_type=0 and id_value in(select goods_id from ecs_goods where goods_name='".str_replace("'","''",$goods['goods_name'])."') and status=1");
...
if($pictures[0]['thumb_url']){
    $smarty->assign('fen_img',$pictures[0]['thumb_url']);
}else{
    $smarty->assign('fen_img',"https://www.quickact.net/".$pictures[0]['img_url']);
}
//取出logo图片
...
$user_id = $_SESSION['user_id'];
$is_fenxiao = $db->getOne("SELECT 1 FROM ".$ecs->table("users")." WHERE is_fenxiao = '1' AND status = '1' AND user_id = '".$user_id."'");
...
$back_act = 'goods.php?id='.$goods_id;
$smarty->assign('back_act', $back_act);


$smarty->assign('goods_id',$_GET['id']);
...
function get_linked_goods($goods_id)
{
    ...
    while ($row = $GLOBALS['db']->fetchRow($res))
    {
		...
		$pricea=get_goods_price($row['goods_id']);
		$arr[$row['goods_id']]['promote_price'] = $pricea;
...
function get_zhekou($goods_id)
{
    ...
    else
    {
        ...
        {
            $zhekou = (number_format(ceil($goods['promote_price'])/ceil($goods['market_price']),3))*10;
        }
        else
        {
            $zhekou = (number_format(ceil($goods['shop_price'])/ceil($goods['market_price']),3))*10;
        }
    }
...
{
    $now = gmtime();
    $sql = "SELECT ga.act_id,ga.ext_info, ga.act_name, ga.act_desc 
            FROM " . $GLOBALS['ecs']->table('goods_activity') . " AS ga, " . $GLOBALS['ecs']->table('package_goods') . " AS pg
            WHERE pg.package_id = ga.act_id
            AND ga.start_time <= '" . $now . "'
            AND ga.end_time >= '" . $now . "'
    ...
    foreach ($res as $tempkey => $value)
    {
        ...
        $res[$tempkey]['saving']        = price_format(($subtotal - $res[$tempkey]['package_price']));
        $res[$tempkey]['package_price_hkd'] = "HKD ".number_format($res[$tempkey]['package_price'], 2, '.', '');
        $res[$tempkey]['package_price'] = price_format($res[$tempkey]['package_price']);
