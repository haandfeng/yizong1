...
//    //对应楼层下广告位
//    $smarty->assign('wap_new_adv',get_wap_advlist('新品上市广告位', 1));
...
$smarty->assign('flash_count',count(get_flash_xml()));
    ...
    $sql = 'SELECT id, number, type FROM ' . $GLOBALS['ecs']->table('ecsmart_template') .
        " WHERE filename = 'index' AND type > 0 AND remarks ='' AND theme='" . $GLOBALS['_CFG']['template'] . "'";
    ...
    foreach ($res AS $row)
    {
        switch ($row['type'])
        {
            ...
            case 3:
                ...
                $data['articles_list'] = $cat_articles['arr'];//最新动态
    ...
    //返回 app json 数据
    $data['goods_list']['best'] = get_recommend_goods('best','',$_CFG['exchange_rate']); //推荐商品
    $data['goods_list']['new'] = get_recommend_goods('new','',$_CFG['exchange_rate']); //最新商品
    $data['goods_list']['hot'] = get_recommend_goods('hot','',$_CFG['exchange_rate']); //热销商品
    $data['nav_list'] = get_menu(); //首页导航栏
    $data['adv_list'] = get_wap_advlist('wap首页幻灯广告', 5); //首页轮播图
    $data['three_adv_list']['left'] = get_wap_advlist('首页导航下左边广告图', 1); //导航栏下三个广告图左边
    $data['three_adv_list']['right_top'] = get_wap_advlist('首页导航下右边上部分广告图', 1); //导航栏下三个广告图右边上
    $data['three_adv_list']['right_bottom'] = get_wap_advlist('首页导航下右边下部分广告图', 1); //导航栏下三个广告图右边下
    $data['goods_adv_list']['best'] = get_wap_advlist('新品上市广告位', 1); //新品上市广告图
    $data['goods_adv_list']['new'] = get_wap_advlist('热销商品广告位', 1); //热销商品广告图
    $data['goods_adv_list']['hot'] = get_wap_advlist('精品推荐广告位', 1); //精品推荐广告图
...
function get_wap_advlist( $position, $num )
{
		...
		foreach ( $res as $idx => $row )
		{
				...
				$arr[$row['ad_id']]['image'] = 'http://'.$_SERVER['HTTP_HOST'].'/mobile/'."data/afficheimg/".$row['ad_code'];
...
function get_menu()
{
	...
	foreach($list as $key => $rows)
	{
		...
		$arr[$key]['menu_img'] = 'http://'.$_SERVER['HTTP_HOST'].'/mobile/'.$rows['menu_img'];
