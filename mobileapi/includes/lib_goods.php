 ...
 * @return  array
 */
function get_recommend_goods($type = '', $cats = '',$rate='1')
{
    ...
    if (empty($type_goods[$type]))
    {
        ...
        foreach ($result AS $idx => $row)
        {
            ...
            $goods[$idx]['class_name']   = isset($row['cat_name']) ? $row['cat_name'] : ''; //所属分类 名称
            ...
            $goods[$idx]['thumb']        = 'http://'.$_SERVER['HTTP_HOST'].'/'.get_image_path($row['goods_id'], $row['goods_thumb'], true);
            $goods[$idx]['goods_img']    = 'http://'.$_SERVER['HTTP_HOST'].'/'.get_image_path($row['goods_id'], $row['goods_img']);
