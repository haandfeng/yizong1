...
//LSJ End
    $data['categories_one'] = get_categories_tree($cat_id); //筛选条件 一级分类
    $data['categories_two'] = get_children_tree($cat_id); //筛选条件 二级分类
    ...
    //print_r(get_categories_tree($cat_id));exit;
    /* 调查 */
    $vote = get_vote();
    if (!empty($vote)) {
    ...
    $data['goods_list'] = $goodslist; //筛选后商品列表
...
function category_get_goods($children, $other_has, $other_youhuo, $brand, $min, $max, $ext, $size, $page, $sort, $order, $limit = '') {
    ...
    while ($row = $GLOBALS['db']->fetchRow($res)) {
        ...
        $arr[$row['goods_id']]['goods_thumb'] =  'http://'.$_SERVER['HTTP_HOST'].'/'.get_image_path($row['goods_id'], $row['goods_thumb'], true);
