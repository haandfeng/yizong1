...
$arr['id'] = (isset($arr['id'])) ? intval($arr['id']) : $arr['id'];
    ...
    else
    {
        if ($static_res[$arr['id']] === NULL)
        {
            ...
            $static_res[intval($arr['id'])] = $GLOBALS['db']->GetAll($sql);
        }
        $res = $static_res[intval($arr['id'])];
    }
    $ads = array();
    $position_style = '';
    ...
    foreach ($res AS $row)
    {
        if ($row['position_id'] != intval($arr['id']))
        {
            continue;
        }
...
function insert_supplier_list($arr){
	...
	if(empty($tag_ret)){//没有标签
		...
		$tag_ret[intval($arr['id'])]['select'] = 1;
		$tag_id = $tag_ret[intval($arr['id'])]['id'];
