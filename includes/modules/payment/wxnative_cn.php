...
class wxnative_cn
{
    ...
    function respond() {
        ...
        if(!empty($_GET["check"])){
            ...
            $outTradeNo=$_GET["out_trade_no"]; //订单号
            $time=$_GET["time"];
            ...
            //根据订单号获取order_id
            $sql = 'SELECT order_id FROM ' . $GLOBALS['ecs']->table('order_info') . " WHERE order_sn = '$outTradeNo'";
            ...
            if(!empty($_GET["redirect"])){
                if($result["error"]>0){
                    ...
                    $url='http://www.chongdk.com/QT/wxpay.aspx?outid='.$outTradeNo; 
                    $curl = curl_init(); 
                    ...
                    curl_close($curl);  
                    //微信报关
            $url = 'https://www.quickact.net/customs/Custom.weixin.php?out_trade_no='.$outTradeNo.'&transaction_id='.$res1.''; 
            $curl = curl_init(); 
            curl_setopt($curl, CURLOPT_URL, $url);
            curl_setopt($curl, CURLOPT_HEADER, 0);
            curl_setopt($curl, CURLOPT_RETURNTRANSFER, 1);
            ...
            //平台申报
            $sql = "select 1 from ecs_order_info_apply where trade_no='".$res1."'";
            if($GLOBALS['db']->getOne($sql)!='1')
            {
                $sql = "insert into ecs_order_info_apply (`order_id`, `order_sn`, `isapply`, `addtime`,`applytype`,trade_no)".
                            "select order_id,order_sn,0,now(),'orderlist','".$res1."' from ecs_order_info where order_sn='".$outTradeNo."'";
            ...
            if ($wxsign == $sign) {
                ...
                if ($postdata['result_code'] == 'SUCCESS') {
                    ...
                    $trade_no = $postdata['transaction_id'];
