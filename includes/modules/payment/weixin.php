...
class weixin {
	...
	function respond() {
		...
		if ($notify->checkSign () == TRUE) {
			if ($notify->data ["return_code"] == "FAIL") {
				...
				$order_info = $GLOBALS['db']->getRow("SELECT * FROM " . $GLOBALS['ecs']->table('order_info') . " AS oi LEFT JOIN " . $GLOBALS['ecs']->table('pay_log') . " AS pl ON ( oi.order_id = pl.order_id) WHERE pl.log_id = '$order_sn' limit 1");
	...
	function natpayHtml($order) {
		if (! strpos ( $_SERVER ['HTTP_USER_AGENT'], 'MicroMessenger' )) {
            ...
            $unifiedOrder->setParameter ( "fee_type", "HKD" );// 总金额
