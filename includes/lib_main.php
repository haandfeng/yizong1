...
function get_parent_cats($cat)
{
    ...
    $arr = $GLOBALS['db']->GetAll('SELECT cat_id, cat_name, parent_id FROM ' . $GLOBALS['ecs']->table('category') . ' WHERE is_show = 1 and is_virtual=0');
...
function get_dyna_libs($theme, $tmp)
{
	...
	$tmp_arr_www_yshop100_com = explode('.', $tmp);
    $ext = end($tmp_arr_www_yshop100_com);
...
{
	$arr = "";
	$cat_all = $GLOBALS['db']->getAll("SELECT cat_id, cat_name FROM " . $GLOBALS['ecs']->table('category') . " WHERE parent_id = " . $p . " AND is_show = 1 and is_virtual=0");
