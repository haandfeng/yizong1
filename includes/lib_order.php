 ...
 * @return  array   配送方式数组
 */
function available_payment_list($support_cod, $cod_fee = 0, $is_online = false, $is_virtual = 0, $suppliers_id = false)
{
...
function order_info($order_id, $order_sn = '')
{
    ...
    if ($order_id > 0)
    {
       $sql = "SELECT o.*, p.shop_name, p.address as zt_address, p.phone, p.contact, " . $total_fee . ",0 as exchange_goods_amount FROM " . $GLOBALS['ecs']->table('order_info') .
    ...
    if ($order)
    {
        ...
        $order['invoices'] = $GLOBALS['db']->getAll($sql_invoices);
            ...
            $sql = "SELECT sum(exchange_price*goods_number) FROM " . $GLOBALS['ecs']->table('order_goods')." WHERE order_id = ".$order['order_id']." ";
            $order['exchange_goods_amount'] = $GLOBALS['db']->getOne($sql);
...
function order_goods($order_id)
{
    ...
    //手机专享价格  app  jx  
    $sql = "SELECT rec_id, og.goods_id, og.product_id,og.goods_name, g.goods_thumb, g.suppliers_id, og.goods_sn, og.market_price,og.exclusive, og.goods_number, " .
            ...
            " WHERE order_id = '$order_id'";// 修改   增加 package_attr_id 字段
...
$sql_where = $_SESSION['user_id']>0 ? "c.user_id='". $_SESSION['user_id'] ."' " : "c.session_id = '" . SESS_ID . "' AND c.user_id=0 ";
    ...
    foreach ($arr as $key => $value)
    {
        ...
        {
            $arr[$key]['package_goods_list'] = get_package_goods($value['goods_id'], $value['package_attr_id']); //修改  
        }
    }
    return $arr;
...
function addto_cart($goods_id, $num = 1, $spec = array(), $parent = 0)
{
    ...
    'add_time'   => gmtime(),   //代码增加   By  www.yshop100.com
    ...
    if ($num > 0)
    {
        ...
        $sql = "SELECT goods_number FROM " .$GLOBALS['ecs']->table('cart').
                ...
                " AND parent_id = 0 AND goods_attr = '" .get_goods_attr_info($spec). "' " .
                " AND extension_code <> 'package_buy' " .
                " AND user_id= '".$_SESSION['user_id']."'". //代码增加  By  www.yshop100.com
...
/* 重写方法_start  By  www.yshop100.com */
/**
 * 清空购物车
...
/* 重构方法_start  By  www.yshop100.com */
/**
 * 获得购物车中的商品
 *
 * @access  public
 ...
 * @param   bool    $storage     减库存的时机，1，下订单时；0，发货时；2，支付时；
 */
...
function send_order_bonus($order_id,$supplier_id)
{
    ...
    if ($bonus_list)
    {
        ...
        {   
            $sql = "";
            //用户领取红包上限
            $sql_bouns = "SELECT COUNT(bonus_id) AS user_bonus_max FROM " . $GLOBALS['ecs']->table('user_bonus') . " WHERE bonus_type_id = '".$bonus['type_id']."' AND user_id = '".$user['user_id']."'";
            $user_bonus_max = $GLOBALS['db']->getOne($sql_bouns);
            
            if(intval($bonus['user_bonus_max']) > $user_bonus_max){
                $count += $bonus['number'];
...
{   
    ...
    $sql = "SELECT b.type_id, b.type_money, SUM(o.goods_number) AS number,send_maxcount,sentcount_0, user_bonus_max " .
            "FROM " . $GLOBALS['ecs']->table('order_goods') . " AS o, " .
                      $GLOBALS['ecs']->table('goods') . " AS g, " .
    ...
    /* 查询按订单发的红包 */
     $sql = "SELECT type_id, type_money, IFNULL(FLOOR('$amount' / min_amount), 1) AS number,send_maxcount,sentcount_0, user_bonus_max " .
            "FROM " . $GLOBALS['ecs']->table('bonus_type') .
            "WHERE send_type = '" . SEND_BY_ORDER . "' " .
...
//    }
    ...
    $parent = array(
        ...
        'market_price'  => $package_pricea[0] ? $package_pricea[0] :  $package['market_package'], //修改  
        'goods_price'   => $package_pricea[1] ? $package_pricea[1] :  $package['package_price'], //修改  
        'package_attr_id' =>$package_attr_id, //增加  
        'goods_number'  => $num,
        'goods_attr'    => '',
        'goods_attr_id' => '',
    ...
    if ($num > 0)
    {
        ...
        $sql = "SELECT goods_number FROM " .$GLOBALS['ecs']->table('cart').
                ...
                " AND package_attr_id = '$package_attr_id'  AND rec_type = '" . CART_GENERAL_GOODS . "'";   //修改   增加一条件
...
function put_virtual_cart($order_id){
    /* 代码增加_虚拟团购_START  www.yshop100.com */
