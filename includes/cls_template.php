...
class cls_template
{
    ...
    function select($tag)
    {
        ...
        else
        {
			...
			$tag_arr_www_yshop100_com = explode(' ', $tag);
			$tag_sel = array_shift($tag_arr_www_yshop100_com);
    ...
    function smarty_prefilter_preCompile($source)
    {
        ...
        if ($file_type == '.dwt')
        {
			...
			/* By www.yshop100.com 代码增加_start */
			$source = preg_replace('/<head>/i', "<head>\r\n<base href=\"". $GLOBALS['ecs']->url() ."\" />",  $source);
			/* By www.yshop100.com 代码增加_end */
    ...
    function html_select_date($arr)
    {
        ...
        if (isset($arr['time']))
        {
            ...
            {
                $arr['time'] = gmdate('Y-m-d', $arr['time']);
            }
            $t     = explode('-', $arr['time']);
            $year  = strval($t[0]);
            $month = strval($t[1]);
    ...
    function html_select_time($arr)
    {
        ...
        if (isset($arr['time']))
        {
            $arr['time'] = gmdate('H-i-s', $arr['time']);
            $t     = explode('-', $arr['time']);
            $hour  = strval($t[0]);
            $minute = strval($t[1]);
            $second   = strval($t[2]);
