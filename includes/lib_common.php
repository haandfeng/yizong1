 ...
 */
// 代码修改_start_derek20150129admin_goods  www.yshop100.com
function get_brand_list($t = false)
// 代码修改_end_derek20150129admin_goods  www.yshop100.com
{
...
function price_format($price, $change_price = true)
{
    ...
    $price_exchange = number_format($price*$GLOBALS['_CFG']['exchange_rate'], 2, '.', '');
    if ($change_price && defined('ECS_ADMIN') === false)
    {
        switch ($GLOBALS['_CFG']['price_format'])
        {
            ...
            case 2: // 不四舍五入，保留1位
                ...
                $price_exchange = substr(number_format($price*$GLOBALS['_CFG']['exchange_rate'], 2, '.', ''), 0, -1);
                break;
            case 3: // 直接取整
                $price = intval($price);
                $price_exchange = intval($price*$GLOBALS['_CFG']['exchange_rate']);
                break;
            case 4: // 四舍五入，保留 1 位
                $price = number_format($price, 1, '.', '');
                $price_exchange = number_format($price*$GLOBALS['_CFG']['exchange_rate'], 1, '.', '');
                break;
            case 5: // 先四舍五入，不保留小数
                $price = round($price);
                $price_exchange = round($price*$GLOBALS['_CFG']['exchange_rate']);
                break;
        }
    }
    else
    ...
    return sprintf($GLOBALS['_CFG']['currency_format'], $price,$price_exchange);
}

/*
 * 根据 后台设置的 货币 兑换比例 换算出对应的港币值
 * */
function exchange_rate($price){
    return sprintf('HK '.$GLOBALS['_CFG']['currency_format'], $price*$GLOBALS['_CFG']['exchange_rate']);
}
...
/* 代码增加_虚拟团购_END  www.yshop100.com */
...
/**
 *  清除手机指定后缀的模板缓存或编译文件
 *  wei2 增加 start by www.yshop100.com
 * @access  public
 * @param  bool       $is_cache  是否清除缓存还是清出编译文件
 * @param  string     $ext       需要删除的文件名，不包含后缀
 *
...
/* wei2 增加 end by www.yshop100.com */
/**
 * 清除模版编译文件
 *
 * @access  public
...
/* wei2 修改 end by www.yshop100.com */
/**
 * 页面上调用的js文件
 *
 * @access  public
...
function get_final_price($goods_id, $goods_num = '1', $is_spec_price = false, $spec = array())
{
	...
	$discount = 1;//$discount = isset($GLOBALS['tongbu_user_discount']) ? $GLOBALS['tongbu_user_discount'] : $_SESSION['discount'];
...
function get_package_goods($package_id, $package_attr_id='')
{
	...
	if ($package_attr_id)
	{
	 ...
	 // 下面SQL语句增加两个字段，注意逗号 , g.goods_thumb, g.shop_price   By  www.yshop100.com
    $sql = "SELECT pg.goods_id, g.goods_name, pg.goods_number, p.goods_attr, p.product_number, p.product_id, g.goods_thumb, g.shop_price, ga.attr_price 
            ...
            WHERE pg.package_id = '$package_id' $sql_package_attr_id "; //有修改   注意最后那个 $sql_package_attr_id
    if ($package_id == 0)
    {
...
function Recordkeyword($word_www_yshop100_com, $items = 0, $searchengine = 'ecshop')
{
	if(strlen($word_www_yshop100_com) < 3 || strlen($word_www_yshop100_com) > 30 || strpos($word_www_yshop100_com, ' ') !== false) return;
	$sql_www_yshop100_com = "SELECT * FROM " .$GLOBALS['ecs']->table('keyword'). " WHERE searchengine='ecshop' AND word='$word_www_yshop100_com'";
	$r = $GLOBALS['db']->getRow($sql_www_yshop100_com);
	if($r)
	{
		$items = intval($items) ;   //www.yshop100.com
        ...
        $sql_www_yshop100_com = "UPDATE " . $GLOBALS['ecs']->table('keyword') . " SET " .
                "items = '$items', " .
                "updatetime = '".gmtime()."', " .
				...
				"WHERE w_id = '".$r['w_id']."'";
		$GLOBALS['db']->query($sql_www_yshop100_com);
		$w_id = $r['w_id'];
	}
	else
	{
		$letter_www_yshop100_com   = gb2py($word_www_yshop100_com);
		$sql_www_yshop100_com = "INSERT INTO " . $GLOBALS['ecs']->table('keyword') . " (searchengine, word, keyword, letter, items, updatetime, total_search, ".
		"month_search, week_search, today_search, status) " .
                " VALUES ('$searchengine', '$word_www_yshop100_com', '$word_www_yshop100_com', '$letter_www_yshop100_com', '$items', '".gmtime()."', '1', '1', '1', '1', '1')";
        $GLOBALS['db']->query($sql_www_yshop100_com);
		$w_id = $GLOBALS['db']->insert_id();
	}
	if (!empty($w_id))
	{
    	$ip_www_yshop100_com       = real_ip();
    	$area_www_yshop100_com     = ecs_geoip($ip);
    	$sql_www_yshop100_com = 'INSERT INTO ' . $GLOBALS['ecs']->table('keyword_area') . ' ( ' .
                'w_id, access_time, ip_address, area) VALUES (' .
                "'$w_id', '".gmtime()."', '$ip_www_yshop100_com', '$area_www_yshop100_com')";
    	$GLOBALS['db']->query($sql_www_yshop100_com);
	}
}
/* 代码增加_END    by   www.yshop100.com */
...
function get_dir($type, $id)
{
 ...
 #记录文件  $dir 文件目录；$filename 文件名；$data 存贮数据；//add by happytao 2018-02-23
 function RecordsFile( $dir,$filename,$data=array()){
