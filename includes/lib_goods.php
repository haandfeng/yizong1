...
function get_top($cats = '')
{
    ...
    $sql .= ' AND og.order_id = o.order_id AND og.goods_id = g.goods_id ' .
           ...
           'GROUP BY g.goods_id ORDER BY goods_number DESC, g.goods_id DESC LIMIT 30';
...
function get_recommend_goods($type = '', $cats = '')
{
	...
	if($ishide=='1')
	{
		$where1=' and ((ishide=0 and g.supplier_id=0) or (ishide=1 and g.supplier_id>0)) ';
	}
    ...
    if (empty($type_goods[$type]))
    {
        ...
        $sql = 'SELECT cat.cat_name,g.goods_id, g.goods_name,g.click_count, g.goods_name_style, g.market_price, g.shop_price AS org_price, g.promote_price, ' .
                ...
                "promote_start_date, promote_end_date, g.goods_brief, g.goods_thumb, g.goods_img, RAND() AS rnd, g.supplier_id, g.suppliers_id, ss.suppliers_desc " .
                'FROM ' . $GLOBALS['ecs']->table('goods') . ' AS g ' .
...
function get_goods_info($goods_id)
{
    ...
    $sql = 'SELECT g.*, c.measure_unit, b.brand_id, b.brand_name AS goods_brand, m.type_money AS bonus_money, ' .
                ...
                "IFNULL(mp.user_price, g.shop_price * '$_SESSION[discount]') AS rank_price , ss.suppliers_desc " .
            'FROM ' . $GLOBALS['ecs']->table('goods') . ' AS g ' .
