...
$ui_arr = array(
	...
	/*余额额支付密码_更改_START_www.yshop100.com*/
	'act_forget_pass' ,'back_order_act' , 're_pass', 'auction_list', 'forget_surplus_password', 'act_forget_surplus_password', 'update_surplus_password', 'act_update_surplus_password', 'verify_reset_surplus_email', 'get_verify_code'
); // 代码修改
// By
// www.yshop100.com
/* 余额额支付密码_更改_END_www.yshop100.com */
...
/* 代码增加_Start By www.yshop100.com  用户中心获取会员信息*/
...
function action_default ()
{
	...
	$ecs = $GLOBALS['ecs'];
	$user_id = $_SESSION['user_id'];
	/* 代码注释 by www.yshop100.com _star 路由前已调用会员信息，去掉重复调用*/
	$rank = get_rank_info();
	/* 	include_once (ROOT_PATH . 'includes/lib_clips.php');
	 if($rank = get_rank_info())
	 {
	...
	/* 代码增加--cb--推荐分成-- by www.yshop100.com _star */
	$rn = $rank['rank_name'];
	$recomm = $db->getOne("SELECT is_recomm FROM " . $GLOBALS['ecs']->table('user_rank') . " WHERE rank_name= '$rn'");

	$smarty->assign('recomm', $recomm); // 获取当前用户是否是分成用户判断是否显示我的推荐
	/* 代码增加--cb--推荐分成-- by www.yshop100.com _end */
...
function action_getverifycode ()
{
	...
	$content = '您的验证码为' . $verifycode . '【yshop102】';
	/* 发送注册手机短信验证 */
...
// 代码增加--yshop100--侧边栏登录 判断登录是否开启验证码
...
{
file_put_contents('test001.txt', 'profile',FILE_APPEND);
	// 获取全局变量
	$user = $GLOBALS['user'];
	$_CFG = $GLOBALS['_CFG'];
	$_LANG = $GLOBALS['_LANG'];
...
function action_shiming ()
{
	...
	$sql = "select sm_status from " . $GLOBALS['ecs']->table('users') . " where user_id = '" . $user_id . "'";
	$res_status = $db->query($sql);  
	while($row_status =  $db->fetchRow($res_status))
	{ 
		$user_info['status']=$row_status['sm_status']; 
	}
	$smarty->assign('extend_info_list', $extend_info_list);

	/* 密码提示问题 */
	$smarty->assign('passwd_questions', $_LANG['passwd_questions']);
	if(strlen($user_info['card'])>5){$user_info['card']=substr($user_info['card'],0,5)."*********";}
	$smarty->assign('profile', $user_info);
	$smarty->display('user_transaction_shiming.dwt');
}
/* 修改个人资料的处理 */
...
function action_act_identity ()
{
	...
	if($num > 0)
	{
		show_message('您已申请实名认证，请等待管理员的审核！', '返回上一页', 'user_shengfen.php?act=shiming');
	}
	else
	{
...
function action_act_edit_password ()
{
	...
	/* 代码添加_yshop100_20150729_STAR */
	$confirm_password = isset($_POST['confirm_password']) ? trim($_POST['confirm_password']) : '';
	/* 代码添加_yshop100_20150729_END */
...
//	$order['shipping_status'] = $_LANG['ss'][$order['shipping_status']];
	...
	foreach($goods_list as $goods_key => $goods_val)
	{
		...
		/* 代码增加_start By www.yshop100.com _20150804 */
		...
		/* 代码增加_end By www.yshop100.com _20150804 */
...
function action_back_order_act ()
{
	...
	if($have_tuikuan)
	{
		if ($_POST['order_all'])
		{
			...
			if(floatval($price_refund)==0){
				$price_refund = $GLOBALS['db']->getOne("SELECT surplus FROM " . $GLOBALS['ecs']->table('order_info') . " WHERE order_id = " . $order_id);
			}
		}
		else
		{
...
function action_act_account ()
{
	...
	{
		$surplus['defaultbank'] = isset($_POST['www_yshop100_com_bank']) ? trim($_POST['www_yshop100_com_bank']) : '';
	}
...
function action_act_edit_payment ()
{
	...
	{
		$defaultbank = $_POST['www_yshop100_com_bank'];
...
function action_tg_login_act ()
{
	...
	$_SESSION['takegoods_sn_yshop100'] = $tg_sn;
	$_SESSION['takegoods_id_yshop100'] = $tgrow['tg_id'];
