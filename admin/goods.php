...
require_once(ROOT_PATH . '/' . ADMIN_PATH. "/nusoap/nusoap.php");   //代码增加  By  www.yshop100.com
...
if ($_REQUEST['act'] == 'list' || $_REQUEST['act'] == 'trash')
{
    ...
    $expressfee_id = isset($_REQUEST['expressfee_id']) ? ((empty($_REQUEST['expressfee_id']) && $_REQUEST['expressfee_id'] === 0) ? '' : trim($_REQUEST['expressfee_id'])) : '';
    $handler_list = array();
...
{
     /* 修改 by www.yshop100.com 百度编辑器 begin */
    //include_once(ROOT_PATH . 'includes/fckeditor/fckeditor.php'); // 包含 html editor 类文件
    /* 修改 by www.yshop100.com 百度编辑器 end */

	// 代码增加_start_derek20150129admin_goods  www.yshop100.com

	include_once(ROOT_PATH . '/includes/Pinyin.php');

	// 代码增加_end_derek20150129admin_goods  www.yshop100.com
    ...
    if ($is_add)
    {
        ...
        $goods = array(
            ...
            'goods_number'  => 0,
            'warn_number'   => 1,
            /* 代码修改 By  www.yshop100.com 促销商品时间精确到时分 Start */
...
//            'promote_end_date'   => local_date('Y-m-d', local_strtotime('+1 month')),
            ...
            /* 代码修改 By  www.yshop100.com 促销商品时间精确到时分 End */
...
//                'promote_end_date'   => local_date('Y-m-d', gmstr2tome('+1 month')),
        ...
        $weight_unit=$goods['weight_unit'];

        /* 根据商品重量的单位重新计算 */
        if ($goods['goods_weight'] > 0)
        {
            $goods['goods_weight_by_unit'] = number_format($goods['goods_weight'], 2);//($goods['goods_weight'] >= 1) ? $goods['goods_weight'] : ($goods['goods_weight'] / 0.001);
        }
...
//            $goods['promote_end_date'] = local_date('Y-m-d', $goods['promote_end_date']);
    ...
    /* 创建 html editor */
    create_html_editor('goods_desc', htmlspecialchars($goods['goods_desc'])); /* 修改 by www.yshop100.com 百度编辑器 */
	$sql = "SELECT content FROM " . $ecs->table('goods_comcontent') . " WHERE goods_id = 0";
    $goods_comcontent = $db->getOne($sql);
	create_html_editor2('goods_comcontent', htmlspecialchars($goods_comcontent));
	$sql = "SELECT TOP_BOTTOM FROM " . $ecs->table('goods_comcontent') . " WHERE goods_id = 0";
    $goods_comcontent_TOP_BOTTOM = $db->getOne($sql);
	$goods_comcontent_TOP_BOTTOMstr="<option value='0' selected>上方</option><option value='9'>底部</option>";
	if($goods_comcontent_TOP_BOTTOM==9)
	{
		$goods_comcontent_TOP_BOTTOMstr="<option value='0'>上方</option><option value='9' selected>底部</option>";
	} 
	$smarty->assign('goods_comcontent_TOP_BOTTOMstr', $goods_comcontent_TOP_BOTTOMstr);
    ...
    $smarty->assign('cat_other_list',     cat_list(0, 0, true, 0, false));
    ...
    $smarty->assign('weight_unit_list',unit_list($goods['weight_unit']));
    ...
    $smarty->assign('expressfee_list', expressfee_list($goods['expressfee_id']));
    $smarty->assign('goods_model', $goods['goods_model']);
    $smarty->assign('gname', $goods['gname']);
    $smarty->assign('gcode', $goods['gcode']);
    $smarty->assign('barCode', $goods['barCode']);
    $smarty->assign('itemRecordNo', $goods['itemRecordNo']);
    ...
    //税费
    $tax_list_name = tax_list_name();
    $smarty->assign('tax_list_name', $tax_list_name);
    $smarty->assign('is_intax', $goods['is_intax']);
    $smarty->assign('tax_id', $goods['tax_id']);
...
elseif ($_REQUEST['act'] == 'insert' || $_REQUEST['act'] == 'update')
{
    ...
    $goods_weight = !empty($_POST['goods_weight']) ? number_format($_POST['goods_weight'], 2) : 0;
    $weight_unit = !empty($_POST['weight_unit']) ? $_POST['weight_unit']: 0;
    $is_best = isset($_POST['is_best']) ? 1 : 0;
    $is_new = isset($_POST['is_new']) ? 1 : 0;
    $is_hot = isset($_POST['is_hot']) ? 1 : 0;
    $is_bs = isset($_POST['is_bs']) ? 1 : 0;
    $is_on_sale = isset($_POST['is_on_sale']) ? 1 : 0;
    $is_alone_sale = isset($_POST['is_alone_sale']) ? 1 : 0;
    $is_seckill = isset($_POST['is_seckill']) ? 1 : 0;
    $ishide_path_img = isset($_POST['ishide_path_img']) ? 1 : 0;
    ...
    $unitcode = isset($_POST['unitcode']) ? $_POST['unitcode'] : 0;
    $expressfee_id = isset($_POST['expressfee_id']) ? $_POST['expressfee_id'] : '';
    $goods_model = isset($_POST['goods_model']) ? $_POST['goods_model'] : '';
    $gname = isset($_POST['gname']) ? $_POST['gname'] : '';
    $gcode = isset($_POST['gcode']) ? $_POST['gcode'] : '';
    $barCode = isset($_POST['barCode']) ? $_POST['barCode'] : '';
    $itemRecordNo = isset($_POST['itemRecordNo']) ? $_POST['itemRecordNo'] : '';
    ...
    $is_intax = isset($_POST['is_intax']) ? 1 : 0;
    $tax_id = isset($_POST['tax_id']) ? $_POST['tax_id'] : 0;
    ...
    $expressfee_id = empty($_POST['expressfee_id']) ? '' : intval($_POST['expressfee_id']);
    ...
    $goods_guige = isset($_POST['goods_guige']) ? $_POST['goods_guige'] : '';
    $goods_gongxiao = isset($_POST['goods_gongxiao']) ? $_POST['goods_gongxiao'] : '';
    $goods_shuoming = isset($_POST['goods_shuoming']) ? $_POST['goods_shuoming'] : '';
    $goods_comcontent = isset($_POST['goods_comcontent']) ? $_POST['goods_comcontent'] : '';
    $goods_comcontent_TOP_BOTTOM = isset($_POST['goods_comcontent_TOP_BOTTOM']) ? $_POST['goods_comcontent_TOP_BOTTOM'] : '';
	$sql="update ecs_goods_comcontent set TOP_BOTTOM='".$goods_comcontent_TOP_BOTTOM."',content='".$goods_comcontent."',createtime=NOW() where goods_id=0";
	$db->query($sql);
    ...
    if ($is_insert)
    {
        if ($code == '')
        {
            $sql = "INSERT INTO " . $ecs->table('goods') . " (goods_name, expressfee_id, goods_name_style, ghost_count, goods_sn, " .
                    ...
                    "seller_note, goods_weight, goods_number, warn_number, integral, give_integral, is_best, is_new, is_hot, is_bs, " .
                    "is_on_sale, is_alone_sale, is_seckill, ishide_path_img, is_shipping, goods_desc, add_time, last_update, goods_type, rank_integral,exclusive, suppliers_id, cost_price, is_intax, tax_id, guige, gongxiao, shuoming,classid)" .
                "VALUES ('$_POST[goods_name]', '$_POST[expressfee_id]', '$goods_name_style', '$ghost_count', '$goods_sn', '$catgory_id', " .
                    ...
                    " '$warn_number', '$_POST[integral]', '$give_integral', '$is_best', '$is_new', '$is_hot', '$is_bs', '$is_on_sale', '$is_alone_sale', '$is_seckill', '$ishide_path_img', $is_shipping, ".
                    " '$_POST[goods_desc]', '" . gmtime() . "', '". gmtime() ."', '$goods_type', '$rank_integral','$exclusive', '$suppliers_id', '$cost_price', '$is_intax', '$tax_id', '$goods_guige', '$goods_gongxiao', '$goods_shuoming','$classid')";
        }
        else
        {
            $sql = "INSERT INTO " . $ecs->table('goods') . " (goods_name, expressfee_id, goods_name_style, ghost_count, goods_sn, " .
                    "cat_id, brand_id, shop_price, market_price, is_promote, zhekou, promote_price, " .
                    "promote_start_date, promote_end_date, is_buy,buymax,buymax_start_date,buymax_end_date,goods_img, goods_thumb, original_img, keywords, goods_brief, " .
                    "seller_note, goods_weight, goods_number, warn_number, integral, give_integral, is_best, is_new, is_hot, is_bs, is_real, " .
                    "is_on_sale, is_alone_sale, is_seckill, ishide_path_img, is_shipping, goods_desc, add_time, last_update, goods_type, extension_code,exclusive, rank_integral, cost_price, is_intax, tax_id, guige, gongxiao, shuoming,classid)" .
                "VALUES ('$_POST[goods_name]', '$_POST[expressfee_id]', '$goods_name_style', '$ghost_count', '$goods_sn', '$catgory_id', " .
                    "'$brand_id', '$shop_price', '$market_price', '$is_promote', '$zhekou', '$promote_price', ".
                    "'$promote_start_date', '$promote_end_date', '$is_buy','$buymax','$buymax_start_date','$buymax_end_date', '$goods_img', '$goods_thumb', '$original_img', ".
                    "'$_POST[keywords]', '$_POST[goods_brief]', '$_POST[seller_note]', '$goods_weight', '$goods_number',".
                    " '$warn_number', '$_POST[integral]', '$give_integral', '$is_best', '$is_new', '$is_hot', '$is_bs', 0, '$is_on_sale', '$is_alone_sale', '$is_seckill', '$ishide_path_img', $is_shipping, ".
                    " '$_POST[goods_desc]', '" . gmtime() . "', '". gmtime() ."', '$goods_type', '$code','$exclusive', '$rank_integral', '$cost_price', '$is_intax', '$tax_id', '$goods_guige', '$goods_gongxiao', '$goods_shuoming','$classid')";
        }
    }
    else
    {
		...
		$goods_info = getGoodsInfo('shop_price',$_REQUEST[goods_id]);
        ...
        $sql = "UPDATE " . $ecs->table('goods') . " SET " .
                ...
                "expressfee_id = '$_POST[expressfee_id]', " .
        ...
        $sql .= "keywords = '$_POST[keywords]', " .
                ...
                "ishide_path_img = '$ishide_path_img', " .
                "is_shipping = '$is_shipping', " .
                "goods_desc = '$_POST[goods_desc]', " .
                "last_update = '". gmtime() ."', ".
                "goods_type = '$goods_type', " .
                "guige = '$goods_guige', " .
                "gongxiao = '$goods_gongxiao', " .
                "shuoming = '$goods_shuoming' " .
    ...
    //新增商品时 生成商品二维码
    created_invite_qrcode($goods_id);
    ...
    {
    	$log_str = $_POST['goods_name'];
    	if ($goods_info['shop_price'] != $shop_price)
    	{
    		$log_str .= "，价格：".$goods_info['shop_price']." -> ".$shop_price;
    	}
        admin_log($log_str, 'edit', 'goods');
    }
...
elseif ($_REQUEST['act'] == 'edit_goods_price')
{
	...
	$goods_info = getGoodsInfo('goods_name,shop_price',$goods_id);
    ...
    else
    {
        ...
        {
        	if ($goods_info['shop_price'] != $goods_price)
        	{
        		$log_str = $goods_info['goods_name']."，商品价格：".$goods_info['shop_price']." -> ".$goods_price;
        		admin_log($log_str, 'edit', 'goods');
        	}
...
elseif ($_REQUEST['act'] == 'edit_goods_number')
{
	...
	$goods_info = getGoodsInfo('goods_name,goods_number',$goods_id);
    ...
    {
    	if ($goods_info['goods_number'] != $goods_num)
    	{
    		$log_str = $goods_info['goods_name']."，商品库存：".$goods_info['goods_number']." -> ".$goods_num;
    		admin_log($log_str, 'edit', 'goods');
    	}
        clear_cache_files();
...
//-- 修改保税推荐状态
/*------------------------------------------------------ */
elseif ($_REQUEST['act'] == 'toggle_bs')
{
    check_authz_json('goods_manage');

    $goods_id       = intval($_POST['id']);
    $is_bs         = intval($_POST['val']);

    if ($exc->edit("is_bs = '$is_bs', last_update=" .gmtime(), $goods_id))
    {
        clear_cache_files();
        make_json_result($is_bs);
    }
}
...
//-- 修改秒杀状态
/*------------------------------------------------------ */
elseif ($_REQUEST['act'] == 'toggle_seckill')
{
    check_authz_json('goods_manage');

    $goods_id       = intval($_POST['id']);
    $is_seckill         = intval($_POST['val']);

    if ($exc->edit("is_seckill = '$is_seckill', last_update=" .gmtime(), $goods_id))
    {
        clear_cache_files();
        make_json_result($is_seckill);
    }
}
/*------------------------------------------------------ */
//-- 修改隐藏详情页状态
/*------------------------------------------------------ */
elseif ($_REQUEST['act'] == 'toggle_ishide_path_img')
{
    check_authz_json('goods_manage');

    $goods_id       = intval($_POST['id']);
    $ishide_path_img         = intval($_POST['val']);

    if ($exc->edit("ishide_path_img = '$ishide_path_img', last_update=" .gmtime(), $goods_id))
    {
        clear_cache_files();
        make_json_result($ishide_path_img);
    }
}
...
//-- 修改商品排序
/*------------------------------------------------------ */
elseif ($_REQUEST['act'] == 'edit_second_order')
{
    check_authz_json('goods_manage');

    $goods_id       = intval($_POST['id']);
    $second_order     = intval($_POST['val']);

    if ($exc->edit("second_order = '$second_order', last_update=" .gmtime(), $goods_id))
    {
        clear_cache_files();
        make_json_result($second_order);
    }
}
...
elseif ($_REQUEST['act'] == 'drop_goods')
{
    ...
    /* 取得商品信息 */
    $sql = "SELECT goods_id, goods_name, is_delete, is_real, goods_thumb, goods_desc, " . //代码修改    
	...
	$domain_www_yshop100_com = $ecs->get_domain();
	$fullurl_www_yshop100_com = $ecs->url();
	$mainurl_www_yshop100_com = str_replace($domain_www_yshop100_com, "", $fullurl_www_yshop100_com);
	$mainurl_www_yshop100_com = str_replace("/", "\/", $mainurl_www_yshop100_com);
	//echo $mainurl_www_yshop100_com;
	//echo "<pre>";
	//print_r($DesImgs[1] );
	//echo "</pre>";	
	foreach ($DesImgs[1] AS $img_del_www_yshop100_com)
	{
		//echo $img_del_www_yshop100_com."<br />";		
		//$final_img_del_www_yshop100_com = preg_replace("/". $mainurl_www_yshop100_com ."/i", "../", $img_del_www_yshop100_com, 1);
		//echo $final_img_del_www_yshop100_com."<br />";
		//删除商品描述的图片
		$stt = explode('../../',$img_del_www_yshop100_com);
		foreach($stt as $str){
			}
		@unlink($str);

	//	@unlink($final_img_del_www_yshop100_com);
	}
	//exit;
	/* 代码增加_end    */
...
elseif ($_REQUEST['act'] == 'export')
{
    ...
    if (isset($_REQUEST['expressfee_id']) && !empty($_REQUEST['expressfee_id']))
    {
        $where .= " AND expressfee_id = " . $_REQUEST['expressfee_id'];
    }
    // 推荐
...
file_put_contents('20211206.txt',"sql::".$sql."\r\n" , FILE_APPEND);
...
/* 代码增加 By  www.yshop100.com End */
//添加税费
elseif ($_REQUEST['act'] == 'tax')
{
    $sql = "SELECT * FROM zs_tax order by is_use desc, tax_id desc" ; 
    ...
    { 
        if($row['tax_rate'] <= 0){
            ...
            $row['rate'] = $row['tax_rate'] * 100 . "%";
        }
        if($row['tax_discount'] <= 0){
            $row['discount'] = "x";
        }else{
            $row['discount'] = $row['tax_discount'] * 100 . "%";
        }
        $row['is_use'] = $row['is_use']==1 ? "启用中":"x";
        $tax_list[] = $row;
    }
    $smarty->assign('tax_list', $tax_list);
    $smarty->assign('action_link', array('href'=>'goods.php?act=tax_add',
        'text' => "设置新税费"));
    $smarty->assign('full_page', 1);

    $smarty->display('goods_tax_config.htm');
}
elseif ($_REQUEST['act'] == 'tax_edit' || $_REQUEST['act'] == 'tax_add')
{ 
    admin_priv('shiparea_manage'); 

    $tax_id = empty($_REQUEST['id']) ? 0 : intval($_REQUEST['id']);

    if($tax_id > 0){
        $sql = "SELECT * FROM zs_tax WHERE tax_id = '$tax_id'" ; 
        $row = $GLOBALS['db']->getRow($sql);
        $smarty->assign('tax_id', $row['tax_id']);
        $smarty->assign('tax_name', $row['tax_name']);
        $smarty->assign('tax_rate', $row['tax_rate']);
        $smarty->assign('tax_discount', $row['tax_discount']);
        $smarty->assign('is_use', $row['is_use']);
        $smarty->assign('form_action',  'tax_update'); 
    }else{
        $smarty->assign('is_use',  1); 
        $smarty->assign('form_action',  'tax_insert'); 
    }
    
    $smarty->display('goods_tax_config_info.htm');
}
elseif ($_REQUEST['act'] == 'tax_insert')
{
    admin_priv('shiparea_manage');
  
    if($_POST['is_use']=="on")$_POST['is_use']=1;else $_POST['is_use']=0;
    
    $sql = "insert into zs_tax(tax_name,tax_rate,tax_discount,is_use) values('".$_POST['tax_name']."','".$_POST['tax_rate']."','".$_POST['tax_discount']."','".$_POST['is_use']."') " ; 
    $db->query($sql);

    admin_log($_POST['tax_name']."税费", 'edit', 'taxinsert');
    $lnk[] = array('text' => "返回上一页", 'href'=>'goods.php?act=tax');
    sys_msg("保存成功", 1,$lnk);
}
elseif ($_REQUEST['act'] == 'tax_update')
{
    admin_priv('shiparea_manage');
    ...
    if($tax_id > 0){
        ...
        $sql = "UPDATE zs_tax SET tax_name='".$_POST['tax_name']."',tax_rate='".$_POST['tax_rate']."',tax_discount='".$_POST['tax_discount']."',is_use='".$_POST['is_use']."'WHERE tax_id='".$tax_id."'";
        $db->query($sql);

        admin_log($_POST['tax_name']."税费", 'edit', 'taxupdate');
        $lnk[] = array('text' => "返回上一页", 'href'=>'goods.php?act=tax');
    }
...
elseif ($_REQUEST['act'] == 'number_edit')
{
    admin_priv('goods_number_edit'); // 检查权限
    ...
    if($_POST['goods_number']){
        ...
        $goods_number = floatval($_POST['add_sub_goods_number']) * abs(floatval($_POST['goods_number']));

        if ($goods_number == 0)
        {
            sys_msg($_LANG['no_goods_number_change']);
        }

        $old_goods_number = getGoodsInfo('goods_number',$goods_id);
        ...
        //记录日志
        $goods_info = getGoodsInfo('goods_name,goods_number,goods_sn',$goods_id);
        $log_str = "[".$goods_info['goods_sn']."] ".$goods_info['goods_name']."，商品库存：".$old_goods_number['goods_number']." -> ".$goods_info['goods_number'];
        admin_log(addslashes($log_str), 'edit', 'goods');

        /* 提示信息 */
        $links = array(
            array('href' => 'goods.php?act=list', 'text' => $_LANG['01_goods_list'])
        );
        sys_msg($_LANG['goods_number_change_ok'], 0, $links);
    }else{
        /* 检查参数 */
        ...
        $goods_info = getGoodsInfo('goods_id,goods_name,goods_number,suppliers_id',$goods_id);
        $goods_info['suppliers_desc'] = get_suppliers_desc($goods_info['suppliers_id']);
        ...
        $smarty->display('goods_number_edit.htm');
    }
 ...
 * @return string
 */
function created_invite_qrcode($goodsid)
{
    /* 生成邀请二维码 及 邀请码 */
    require(ROOT_PATH.'includes/phpqrcode.php');
    $errorCorrectionLevel = 'L';//容错级别
    $matrixPointSize = 6;//生成图片大小
    if(!file_exists(ROOT_PATH.'images/goods_qrimg/'.date('Ym'))){
        mkdir(ROOT_PATH.'images/goods_qrimg/'.'/'.date('Ym'), 0777);
    }

    $filename = ROOT_PATH.'images/goods_qrimg/'.date('Ym').'/'.$goodsid.'.png';
    $file = date('Ym').'/'.$goodsid.'.png';
    $data = 'http://'.$_SERVER['HTTP_HOST'].'/mobile/goods.php?id='.$goodsid;
    //生成二维码图片

    QRcode::png($data,$filename, $errorCorrectionLevel, $matrixPointSize, 2);

    //更新 注册会员的 邀请码 及 二维码
    $sql = 'UPDATE ' . $GLOBALS['ecs']->table('goods') . ' SET `qr_img` = "' . $file .'" WHERE goods_id = ' . $goodsid;
    $GLOBALS['db']->query($sql);
}
