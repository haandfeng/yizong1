...
if ($_REQUEST['act'] == 'insert')
{
	...
	$send_maxcount = !empty($_POST['send_maxcount'])  ? intval($_POST['send_maxcount'])    : 0;
    
    $is_use_link = !empty($_POST['is_use_link'])  ? intval($_POST['is_use_link'])    : 0;
    $goods_ids = !empty($_POST['goods_ids'])    ? trim($_POST['goods_ids'])    : '';
    $link_goods_ids = explode(',', $goods_ids);
...
//    }
    ...
    /* 插入数据库。 */
	$sql = "INSERT INTO ".$ecs->table('bonus_type')." (type_name, type_money,send_start_date,send_end_date,use_start_date,use_end_date,send_type,bonus_code,user_bonus_max,min_amount,min_goods_amount,send_maxcount,sentcount_0, is_use_link)
    VALUES ('$type_name',
            '$_POST[type_money]',
            '$send_startdate',
            '$send_enddate',
			...
			'$ad_code',
			'$user_bonus_max',			
            '$min_amount','" . floatval($_POST['min_goods_amount']) . "','$send_maxcount',0, '$is_use_link')";

    $db->query($sql);
    $bonus_type_id = $GLOBALS['db']->insert_id();

    //商品绑定
    if($is_use_link != '0' && $link_goods_ids){
    	$sql = "DELETE FROM " . $ecs->table('link_bonus') . " WHERE bonus_type_id = '$bonus_type_id'";
	    $db->query($sql);

	    foreach ($link_goods_ids AS $val){
		    $sql = "INSERT INTO " . $ecs->table('link_bonus') . " (bonus_type_id, link_goods_id) VALUES ('$bonus_type_id', '$val')";
		    $db->query($sql);
		}
    }
...
if ($_REQUEST['act'] == 'edit')
{
    ...
    //搜索商品
    $sql = "SELECT g.goods_id, g.goods_sn, g.goods_name FROM " .$ecs->table('link_bonus'). " AS lb LEFT JOIN  " .$ecs->table('goods'). " AS g ON (lb.link_goods_id = g.goods_id) WHERE lb.bonus_type_id = '".$type_id."'";
    $goods_list = $db->getAll($sql);
    if($goods_list){
    	$i = 0;
    	foreach ($goods_list as $val) {
    		$link_goods_list .= '<option value="'.$val['goods_id'].'">['.$val['goods_sn'].']'.$val['goods_name'].'</option>';
    		if($i!=0){
    			$link_goods_ids .= ',' . $val['goods_id']; 
    		}
    		$i++;
    	}
    }
    ...
    $smarty->assign('link_goods_ids',   $link_goods_ids);
...
if ($_REQUEST['act'] == 'update')
{
    ...
    $send_startdate = local_strtotime($_POST['send_start_date']);
    $send_enddate   = local_strtotime($_POST['send_end_date']);
    /*$send_startdate = local_strtotime($_POST['use_start_date']);
    $send_enddate   = local_strtotime($_POST['use_end_date']);*/
...
//    }
    ...
    $sql = "UPDATE " .$ecs->table('bonus_type'). " SET ".
           ...
           "min_amount      = '$min_amount', " .
			...
			"send_maxcount      = '$send_maxcount', " .
			// "bonus_code      = '$ad_code', " .
           "min_goods_amount = '" . floatval($_POST['min_goods_amount']) . "', ".
           "is_use_link = '$is_use_link' ".
   ...
   //商品绑定
    if($is_use_link != '0' && $link_goods_ids){
    	$sql = "DELETE FROM " . $ecs->table('link_bonus') . " WHERE bonus_type_id = '$type_id'";
	    $db->query($sql);

	    foreach ($link_goods_ids AS $val){
		    $sql = "INSERT INTO " . $ecs->table('link_bonus') . " (bonus_type_id, link_goods_id) VALUES ('$type_id', '$val')";
		    $db->query($sql);
		}
    }
...
elseif ($_REQUEST['act'] == 'remove_bonus')
{
    ...
    $sql = "DELETE FROM " . $ecs->table('link_bonus') . " WHERE bonus_id = '$id'";
	$db->query($sql);
...
if ($_REQUEST['act'] == 'new_get_goods_list')
{
    include_once(ROOT_PATH . 'includes/cls_json.php');
    $json = new JSON;
    ...
    $where .= " AND (goods_sn LIKE '%H%' OR goods_sn LIKE '%Y%' OR goods_sn LIKE '%W%' OR goods_sn LIKE '%J%' OR goods_sn LIKE '%C%')";
...
{
    /* 获得所有红包类型的发放数量 */
    $sql = "SELECT bonus_type_id, case send_type when 5 then max(sentcount_0) else COUNT(*) end  AS sent_count,send_maxcount ".
            " FROM " .$GLOBALS['ecs']->table('user_bonus') .
    		" a inner join ".$GLOBALS['ecs']->table('bonus_type')." t on a.bonus_type_id=t.type_id WHERE a.supplier_id=0".
            " GROUP BY bonus_type_id, send_maxcount ";
    ...
    while ($row = $GLOBALS['db']->fetchRow($res))
    {
		...
		if($row['send_maxcount']=='0') 
		{
			$row['send_maxcount'] ='无限';
		}
