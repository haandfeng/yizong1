...
//        $filter['keyword']          = empty($_REQUEST['keyword']) ? '' : trim($_REQUEST['keyword']);
        ...
        $filter['expressfee_id'] = isset($_REQUEST['expressfee_id']) ? ((empty($_REQUEST['expressfee_id']) && $_REQUEST['expressfee_id'] === 0) ? '' : trim($_REQUEST['expressfee_id'])) : '';
        ...
        $filter['second_order']       = empty($_REQUEST['second_order']) ? 'DESC' : trim($_REQUEST['second_order']);
        ...
        switch ($filter['intro_type'])
        {
            ...
            case 'is_bs':
                ...
                break;
            case 'is_seckill':
                $where .= ' AND is_seckill=1';
                break;
             case 'ishide_path_img':
                $where .= ' AND ishide_path_img=1';
                break;
            case 'is_promote':
        ...
        /* 运费大类 */
        if ($filter['expressfee_id'])
        {
            $where .= " AND expressfee_id='$filter[expressfee_id]'";
        }
       ...
       if ($real_goods > -1)
        ...
        if(intval($_REQUEST['supp'])>0){
            $sql = "SELECT goods_id, goods_name, goods_type, goods_sn, shop_price, is_on_sale, is_best, is_new, is_hot, is_bs, is_seckill, ishide_path_img, sort_order, second_order, goods_number,exclusive, integral, " .
                    " (promote_price > 0 AND promote_start_date <= '$today' AND promote_end_date >= '$today') AS is_promote ".
                    ", supplier_status, g.supplier_id,supplier_name,qr_img,replace(replace(ef.name,'{weight}','-'),'{money}',goodsamount) as expressfee_id ".
                    " FROM " . $GLOBALS['ecs']->table('goods') . " AS g ".
                    " LEFT JOIN " . $GLOBALS['ecs']->table('supplier') . " AS s ON s.supplier_id = g.supplier_id ".
                    " LEFT JOIN zs_expressfeeconfig AS ef ON ef.id = g.expressfee_id ".
            ...
            $sql = "SELECT goods_id, goods_name, goods_type, goods_sn, shop_price, is_on_sale, is_best, is_new, is_hot, is_bs, is_seckill, ishide_path_img, sort_order, second_order, goods_number,exclusive, integral, " .
                    " (promote_price > 0 AND promote_start_date <= '$today' AND promote_end_date >= '$today') AS is_promote ".
                    ", supplier_status, supplier_id,qr_img,replace(replace(ef.name,'{weight}','-'),'{money}',goodsamount) expressfee_id ".  //代码增加   By  www.yshop100.com
                    " FROM " . $GLOBALS['ecs']->table('goods') . " AS g ".
                    " LEFT JOIN zs_expressfeeconfig AS ef ON ef.id = g.expressfee_id ".
                    " WHERE is_delete='$is_delete' $where" .
                    " ORDER BY $filter[sort_by] $filter[sort_order] ".
    ...
    foreach($row as $k=>$v)
    {
        ...
        $row[$k]['qr_img'] = 'http://'.$_SERVER['HTTP_HOST'].'/images/goods_qrimg/'.$v['qr_img'];
    }
	...
	if($filter['city']){
       ...
       if ($real_goods > -1)
        ...
        if(intval($_REQUEST['supp'])>0){
        	...
        	$sql = "SELECT distinct(g.goods_id), goods_name, goods_type, goods_sn, shop_price, is_on_sale, is_best, is_new, is_hot, sort_order, goods_number, integral, " .
                    " (promote_price > 0 AND promote_start_date <= '$today' AND promote_end_date >= '$today') AS is_promote ".
					", supplier_status, g.supplier_id ".	//代码增加   By  www.yshop100.com
                    " FROM " . $GLOBALS['ecs']->table('goods') . " AS g ".
                    "left join ". $GLOBALS['ecs']->table('virtual_district') ." as dis on dis.goods_id=g.goods_id ".
                   "left join ".$GLOBALS['ecs']->table('virtual_goods_district')."  as dig on dis.district_id = dig.district_id ".
                     " WHERE is_delete='$is_delete' $where" .
