 ...
 */
function getGoodsInfo($fields = "*",$goods_id)
{
	$sql = "SELECT ".$fields." FROM " . $GLOBALS['ecs']->table("goods") . " WHERE goods_id = ". $goods_id;
...
function suppliers_list_name()
{
    ...
    if (count($suppliers_list) > 0)
    {
        ...
        {
            $suppliers_name[$suppliers['suppliers_id']] = "【" . $suppliers['suppliers_desc'] . "】" . $suppliers['suppliers_name']; 
        }
    }
...
function get_suppliers_desc($suppliers_id)
{
    if ($suppliers_id)
    {
        ...
        $suppliers_desc = $GLOBALS['db']->getOne($sql);
    }
 ...
 * @return  string
 */
function unit_list($selected)
{
    $sql = 'SELECT unitcode, unitname FROM ' . $GLOBALS['ecs']->table('unit') . ' WHERE enabled = 1 ORDER BY unitname ASC';
    $res = $GLOBALS['db']->query($sql);

    $lst = '';
    while ($row = $GLOBALS['db']->fetchRow($res))
    {
        $lst .= "<option value='$row[unitcode]'";
        $lst .= ($selected == $row['unitcode']) ? ' selected="true"' : '';
        $lst .= '>' . htmlspecialchars($row['unitname']). '</option>';
    }
 ...
 * @return  string
 */
function unit_list_name($selected)
{
    $sql = 'SELECT unitcode, unitname FROM ' . $GLOBALS['ecs']->table('unit') . ' WHERE enabled = 1 ORDER BY unitname ASC';
    $res = $GLOBALS['db']->query($sql);
    ...
    while ($row = $GLOBALS['db']->fetchRow($res))
    {
        $lst .= "<option value='".htmlspecialchars($row['unitname'])."'";
        $lst .= ($selected == $row['unitcode']) ? ' selected="true"' : '';
        $lst .= '>' . htmlspecialchars($row['unitname']). '</option>';
    }
...
function expressfee_list($selected)
{
    $sql = 'SELECT id, name,goodsamount FROM zs_expressfeeconfig WHERE isuse = 1 and id>0';
    $res = $GLOBALS['db']->query($sql);

    $lst = '';
    while ($row = $GLOBALS['db']->fetchRow($res))
    {
        $row['name']=str_replace('{weight}','-',str_replace('{money}',$row['goodsamount'],$row['name']));
...
{
    $sql = 'SELECT countrycode, countryname FROM ' . $GLOBALS['ecs']->table('country') . ' WHERE enabled = 1';
    $res = $GLOBALS['db']->query($sql);

    $lst = '';
    while ($row = $GLOBALS['db']->fetchRow($res))
    {
        $lst .= "<option value='$row[countrycode]'";
        $lst .= ($selected == $row['countrycode']) ? ' selected="true"' : '';
        $lst .= '>' . htmlspecialchars($row['countryname']). '</option>';
    }
 ...
 * @return      array
 */
function tax_list_info($conditions = '')
{
    $where = '';
    if (!empty($conditions))
    {
    ...
    /* 查询 */
    $sql = "SELECT tax_id, tax_name, tax_rate, tax_discount, is_use
            FROM zs_tax
            $where";
 ...
 * @return  array
 */
function tax_list_name()
{
    /* 查询 */
    $tax_list = tax_list_info();
    ...
    if (count($tax_list) > 0)
    {
        ...
        {
            if($tax['is_use'] >0){
                if($tax['tax_discount'] > 0){
                     $tax_name[$tax['tax_id']] = $tax['tax_name']. $tax['tax_rate']*100 . '%' . '（' .  ($tax['tax_discount']*100) .'% 税费折扣)' . ' ——总计: ' . ($tax['tax_rate']*$tax['tax_discount']*100) . '%';
                }else{
                     $tax_name[$tax['tax_id']] = $tax['tax_name']. $tax['tax_rate']*100 . '%' . ' ——总计: ' . ($tax['tax_rate']*100) . '%';
                }
           }
        }
    }
