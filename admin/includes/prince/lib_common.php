...
//file_put_contents('20211210.txt',"pintuan_info:".$sql."\r\n" , FILE_APPEND);
	/* 如果为空，返回空数组 */
	if (empty($pintuan)) {
		return array();
	}
...
function update_pintuan_info($pt_id = 0)
{
	...
	if($pt_id>0)
	{
		...
		$sql = "SELECT a.* " .
			"FROM " . $GLOBALS['ecs']->table('pintuan') . " AS a " .
			"WHERE status=0  and pt_id=" .$pt_id.
			" ORDER BY a.create_time asc ";
		$row = $GLOBALS['db']->getAll($sql);
	}
	foreach ($row AS $key => $val) {
		if ($val['create_succeed'] == 1) {//处理开团成功的拼团及订单
			...
			if ($val['available_people'] == 0) {// 所需人数剩余0 开团成功
				...
				//file_put_contents('20211210.txt',"所需人数剩余0 开团成功:".$sql."\r\n" , FILE_APPEND);
			} else {//所需人数大于零
				$sql = "SELECT count(*) " .
				...
				$sql = 'UPDATE ' . $GLOBALS['ecs']->table('pintuan') . ' SET `available_people` =`need_people`-' . $valid_orders .
					" WHERE pt_id = '" . $val['pt_id'] . "'";
				$GLOBALS['db']->query($sql);
				//file_put_contents('20211210.txt',"所需人数大于零:".$sql."\r\n" , FILE_APPEND);
				if ($val['need_people'] <= $valid_orders) {
					$sql = 'UPDATE ' . $GLOBALS['ecs']->table('pintuan') . ' SET status =1 ' .
						"WHERE pt_id = '" . $val['pt_id'] . "'";
					$GLOBALS['db']->query($sql);
					//file_put_contents('20211210.txt',"status =1 :".$sql."\r\n" , FILE_APPEND);
				} else {
					$sql = 'UPDATE ' . $GLOBALS['ecs']->table('pintuan') . ' SET status =2 ' .
						"WHERE pt_id = '" . $val['pt_id'] . "' and end_time<$now ";
					$GLOBALS['db']->query($sql);
					//file_put_contents('20211210.txt',"status =2 :".$sql."\r\n" , FILE_APPEND);
				}
			}

		} else {//处理开团中的拼团及订单
			$shonow=strtotime($now);
			$shoend=strtotime($val['end_time']);
			//file_put_contents('20211210.txt',"处理开团中的拼团及订单:\r\n" , FILE_APPEND);
			//file_put_contents('20211210.txt',"".$now."(".$shonow.")\r\n" , FILE_APPEND);
			//file_put_contents('20211210.txt',"".$val['end_time']."(".$shoend.")\r\n" , FILE_APPEND);
			if ($shoend > $shonow) {//未开团 未超时
				$sql = "SELECT count(o.order_id) " .
					"FROM  " . $GLOBALS['ecs']->table('pintuan_orders') . " AS pto  " .
					"LEFT JOIN " . $GLOBALS['ecs']->table('order_info') . " AS o ON pto.order_id    = o.order_id    " .
					"WHERE pto.pt_id=" . $val['pt_id'] . " and o.pay_status =2";
				$ocount = $GLOBALS['db']->getOne($sql); 
				$sql = 'UPDATE ' . $GLOBALS['ecs']->table('pintuan') . ' SET `available_people` =`need_people`-' . $ocount .
					" WHERE pt_id = '" . $val['pt_id'] . "'";
				$GLOBALS['db']->query($sql);
				//file_put_contents('20211210.txt',"ocount:".$ocount."\r\n" , FILE_APPEND);
				$sql = "SELECT need_people " .
					"FROM  " . $GLOBALS['ecs']->table('pintuan') . " AS pto  " . 
					"WHERE pto.pt_id=" . $val['pt_id'] . "";
				$npeople = $GLOBALS['db']->getOne($sql);
				//file_put_contents('20211210.txt',"npeople:".$npeople."\r\n" , FILE_APPEND);
				//file_put_contents('20211210.txt',"npeoplesql:".$sql."\r\n" , FILE_APPEND); 
				//file_put_contents('20211210.txt',"npeople:".$npeople."\r\n" , FILE_APPEND); 
				if ($ocount>=$npeople) {
					$sql = 'UPDATE ' . $GLOBALS['ecs']->table('pintuan') . ' SET create_succeed =1 ' .
						"WHERE pt_id = '" . $val['pt_id'] . "'";
					$GLOBALS['db']->query($sql);
					update_pintuan_info($val['pt_id']);
				}
			} else {//未开团，已超时
				$sql = 'UPDATE ' . $GLOBALS['ecs']->table('pintuan') . ' SET status =2 ' .
					"WHERE pt_id = '" . $val['pt_id'] . "'";
				$GLOBALS['db']->query($sql);
				//file_put_contents('20211210.txt',"未开团，已超时:".$sql."\r\n" , FILE_APPEND);
			}
		}
...
function pintuan_list($size, $page)
{
	...
	while ($pintuan = $GLOBALS['db']->fetchRow($res)) {
		...
		$pintuan['lowest_price'] = get_lowest_price($price_ladder);
...
function get_pintuan_info($id)
{
	...
	$sql = "SELECT act_id, act_name AS cut_name, goods_id, product_id, goods_name, start_time, end_time, act_desc, ext_info" .
		" FROM " . $GLOBALS['ecs']->table('goods_activity') .
		" WHERE act_id='$id' AND act_type = 0 ";
