...
$sel_fenxiao = isset($_REQUEST['sel_fenxiao']) && !empty($_REQUEST['sel_fenxiao']) ? $_REQUEST['sel_fenxiao'] : "";

$sql_fenxiao  = "select user_id,user_name,real_name from ". $GLOBALS['ecs']->table('users') .
" where is_fenxiao = '1' AND status = '1' AND user_id != '0' order by user_id asc";
$res_fenxiao = $GLOBALS['db']->query($sql_fenxiao);
while($row_fenxiao = $db->fetchRow($res_fenxiao))
{
    $fenxiao_list_name[$row_fenxiao['user_id']] = $row_fenxiao['real_name'] ? $row_fenxiao['user_name'] . "-" . $row_fenxiao['real_name'] : $row_fenxiao['user_name'] ;
}
...
if ($_REQUEST['act'] == 'list')
{
    ...
    // 分销商列表
    $smarty->assign('fenxiao_list_name', $fenxiao_list_name);
...
elseif ($_REQUEST['act'] == 'query')
{
    ...
    $result = get_sell_detail($start_date, $end_date, $cat_id, $brand_id, $sel_shop, $supplier_id, $sel_fenxiao);
...
elseif ($_REQUEST['act'] == 'export')
{
    ...
    {
        // 平台方
        $where .= ' AND u.parent_id = 0';
    }elseif($sel_fenxiao > 0){
        $where .= ' AND u.parent_id = ' . $sel_fenxiao;
    }
    ...
    $sql = "SELECT og.goods_name, og.goods_attr, i.order_sn, og.goods_number sells_count, og.goods_price, i.add_time FROM "
        ...
        . ' b ON g.brand_id = b.brand_id LEFT JOIN ' . $GLOBALS['ecs']->table('users') . ' u ON i.user_id = u.user_id ' . $where . ' GROUP BY og.goods_name, i.order_sn, og.goods_price, i.add_time ORDER BY sells_count';
    ...
    $outputFileName = '商品销售明细_' . $start_date .'-'. $end_date . '.xls';
 ...
 * @return array
 */
function get_sell_detail($start_date, $end_date, $cat_id, $brand_id, $sel_shop, $supplier_id, $sel_fenxiao)
{
    $result = get_filter();
    if($result === false)
    {
        ...
        $filter['sel_fenxiao'] = empty($_REQUEST['sel_fenxiao']) ? $sel_fenxiao : $_REQUEST['sel_fenxiao'];
        ...
        $filter['record_count'] = $GLOBALS['db']->getOne(
            ...
            . $GLOBALS['ecs']->table('brand') . ' b ON g.brand_id = b.brand_id LEFT JOIN ' . $GLOBALS['ecs']->table('users') . ' u ON i.user_id = u.user_id ' . $where
        ...
        $sql = "SELECT og.goods_name, og.goods_attr, i.order_sn, og.goods_number sells_count, og.goods_price, i.add_time FROM "
            ...
            . ' i ON og.order_id = i.order_id LEFT JOIN ' . $GLOBALS['ecs']->table('goods')
            . ' g ON g.goods_id = og.goods_id LEFT JOIN ' . $GLOBALS['ecs']->table('brand')
            . ' b ON g.brand_id = b.brand_id LEFT JOIN ' . $GLOBALS['ecs']->table('users') . ' u ON i.user_id = u.user_id ' . $where . ' GROUP BY og.goods_name, i.order_sn, og.goods_price, i.add_time '
