...
elseif ($_REQUEST['act'] == 'list')
{
    ...
    $other_fenxiao = 1;
    $smarty->assign('fenxiaouser_list',   get_fenxiao_user($other_fenxiao));
    $smarty->assign('fenxiaotopuser_list',   get_fenxiao_topuser());
    ...
    if(check_authz('order_seller')){
        $smarty->assign('seller_view',  1);
    }
    /* 显示模板 */
    assign_query_info();
...
elseif ($_REQUEST['act'] == 'listts')
{
    ...
    /* 显示模板 */
    assign_query_info();
    $smarty->display('order_listts.htm');
}
elseif ($_REQUEST['act'] == 'haiguanlist')
{
    /* 检查权限 */
    admin_priv('order_view');
    ...
    /* 显示模板 */
    assign_query_info();
    $smarty->display('order_haiguan_list.htm');
}
//微信对账单
elseif ($_REQUEST['act'] == 'bill')
{
    /* 检查权限 */
    admin_priv('order_view');
    /* 模板赋值 */
    ...
    $smarty->assign('full_page',        1);
    $smarty->assign('is_rebate',check_authz('order_rebate') ? true : false);
    $bill_data = trim($_REQUEST['bill_data']);
    if(empty($bill_data)){
        $bill_data = date('Ymd',(time()-3600*24));
    }
    $plugin_file = ROOT_PATH.'/mobile/includes/modules/payment/weixin.php';
    if (file_exists($plugin_file))
    {
        include_once($plugin_file);
        $payment = new weixin();
        $list = $payment->downloadbill($bill_data);     
    }
    $data = $list['data'];
    $new_data = array();
    $i=0;
    foreach($data as $key=>$val){
        $new_data[$i][] = $val;
        if((($key+1)%23)==0){
            $new_data[$i]['real_amount'] = $new_data[$i]['21'] - $new_data[$i]['16'];
            $i++;
        }
    }
    
    $is_down = $bill_data ? 1 : 0;
    $smarty->assign('status',   $list['status']);
    $smarty->assign('order_list',   $new_data);
    $smarty->assign('bill_data',   $bill_data);
    $smarty->assign('is_down',   $is_down);
    $smarty->assign('sort_order_time', '<img src="images/sort_desc.gif">');
    /* 显示模板 */
    assign_query_info();
    $smarty->display('order_bill.htm');
}

//快闪付对账单
elseif ($_REQUEST['act'] == 'quick_pay')
{
    /* 检查权限 */
    admin_priv('order_view');
    /* 模板赋值 */
    ...
    $bill_data = trim($_REQUEST['bill_data']);
    if(empty($bill_data)){
        $bill_data = date('Y-m-d',(time()-3600*24));
    }
    $plugin_file = ROOT_PATH.'/mobile/includes/modules/payment/quickpay.php';
    if (file_exists($plugin_file))
    {
        include_once($plugin_file);
        $payment = new quickpay();
        $list = $payment->getquickpayquery($bill_data);     
    }
    $data = $list['rows'];
    $new_data = array(); 
    foreach($data as $key=>$val){
        $new_data[$key][] = $val['createtime'];
        $new_data[$key][] = $val['order_sn'];
        $new_data[$key][] = $val['id'];
        $new_data[$key][] = $val['amount'];
        $new_data[$key][] = $val['amountHDK'];
        $new_data[$key][] = $val['exchange'];
        $str='成功';
        if($val['state']=='-1')
        {$str='撤销';}
        $new_data[$key][] = $str;
    ...
    /* 显示模板 */
    assign_query_info();
    $smarty->display('order_bill_quickpay.htm');
}

//查询结算资金的明细
elseif ($_REQUEST['act'] == 'tlement')
{
    /* 检查权限 */
    admin_priv('order_view');
    /* 模板赋值 */
    ...
    if(empty($date_start)){
        $date_start = date('Ymd',(time()-3600*24*31));
    }
    if(empty($date_end)){
        $date_end = date('Ymd',(time()-3600*24));
    }
    $plugin_file = ROOT_PATH.'/mobile/includes/modules/payment/weixin.php';
    if (file_exists($plugin_file))
    {
        include_once($plugin_file);
        $payment = new weixin();
        $list = $payment->settlementquery($usetag,$date_start,$date_end);       
    }
    $data = $list['data'];
    $new_data = array();
    $i=0;
    ...
    $smarty->assign('status',   $list['status']);
    $smarty->assign('order_list',   $new_data);
    $smarty->assign('bill_start',   $bill_start);
    $smarty->assign('bill_end',   $bill_end);
    $smarty->assign('usetag',   $usetag);
    $smarty->assign('is_down',   $is_down);
    $smarty->assign('sort_order_time', '<img src="images/sort_desc.gif">');
    /* 显示模板 */
    assign_query_info();
    $smarty->display('order_tlement.htm');
}

//下载快闪付对账单
elseif ($_REQUEST['act'] == 'download_quickpay_record')
{
    /* 检查权限 */
    admin_priv('order_view');
    /* 模板赋值 */
...
//下载微信对账单
elseif ($_REQUEST['act'] == 'download_bill_record')
{
    /* 检查权限 */
    admin_priv('order_view');
    /* 模板赋值 */
    ...
    $filename='wx_pay_'.$bill_data;
    header("Content-type: application/vnd.ms-excel; charset=gbk");
    header("Content-Disposition: attachment; filename=$filename.xls");
    $data = "<table><tr>
                <th>Transaction time</th>
                <th>Official account ID(appid)</th>
                <th>Vendor ID(mch_id)</th>
                <th>Sub vendor ID(sub_mch_id)</th>
                <th>Device ID(Device_info)</th>
                <th>Wechat order number(transaction_id)</th>
                <th>Vendor order number(out_transaction_id)</th>
                <th>User tag(openid)</th>
                <th>Transaction type(trade_type)</th>
                <th>Transaction status(trade_state)</th>
                <th>Payment bank(bank_type)</th>
                <th>Currency type(fee_type)</th>
                <th>Total amount(total_fee)</th>
                <th>Coupon amount</th>
                <th>Product name</th>
                <th>Vendor's data package(attach)</th>
                <th>Fee</th>
                <th>Rate</th>
                <th>Payment Currency type(Cash_fee_type)</th>
                <th>Cash payment amount(Cash_fee)</th>
                <th>Settlement currency type</th>
                <th>Settlement currency amount</th>
                <th>Exchange rate</th></tr>";
    foreach ($new_data as $key => $value) 
    {
        $data .= "<tr>";
...
//下载结算资金
elseif ($_REQUEST['act'] == 'download_tlement_record')
{
    /* 检查权限 */
    admin_priv('order_view');
    /* 模板赋值 */
    ...
    $filename='wx_jiesuan_'.$date_end;
    header("Content-type: application/vnd.ms-excel; charset=gbk");
    header("Content-Disposition: attachment; filename=$filename.xls");
    $data = "<table><tr>
                <th>date_settlement </th>
                <th>date_start</th>
                <th>date_end</th>
                <th>settlement_fee</th>
                <th>settlementfee_type</th>
                <th>pay_fee</th>
                <th>refund_fee</th>
                <th>pay_net_fee</th>
                <th>poundage_fee</th>
                <th>unsettlement_fee</th>
                <th>fbatchno</th></tr>";
    foreach ($new_data as $key => $value) 
    {
        $data .= "<tr>";
...
//微信对账单
elseif ($_REQUEST['act'] == 'query_order_pay')
{
    /* 检查权限 */
    //admin_priv('order_view');
    /* 模板赋值 */
    $smarty->assign('ur_here', '订单交易信息查询');
    $smarty->assign('os_unconfirmed',   OS_UNCONFIRMED);
    $smarty->assign('cs_await_pay',     CS_AWAIT_PAY);
    $smarty->assign('cs_await_ship',    CS_AWAIT_SHIP);
    $smarty->assign('full_page',        1);
    ...
    if (file_exists($plugin_file))
    {
        include_once($plugin_file);
        $payment = new weixin();
        $list = $payment->queryOrderWinPayInfo($bill_data);
    }
    $data = $list['data'];
    $new_data = array();
    $i=0;
...
//微信对账单
elseif ($_REQUEST['act'] == 'order_query_pay')
{
    /* 检查权限 */
    admin_priv('order_wxpay_query');
    /* 模板赋值 */
    $smarty->assign('ur_here', '订单交易信息查询');
    $smarty->assign('os_unconfirmed',   OS_UNCONFIRMED);
    $smarty->assign('cs_await_pay',     CS_AWAIT_PAY);
    ...
    $smarty->assign('full_page',        1);
    $smarty->assign('is_rebate',check_authz('order_rebate') ? true : false);
    $bill_data = trim($_REQUEST['out_trade_no']);

    $plugin_file = ROOT_PATH.'/mobile/includes/modules/payment/weixin.php';
    $info = [];
    if (file_exists($plugin_file) && $bill_data)
    {
        include_once($plugin_file);
        $payment = new weixin();
        $info = $payment->queryOrderWinPayInfo($bill_data);
    }
    $info['total_fee'] = $info['total_fee'] / 100;
    $info['cash_fee'] = $info['cash_fee'] / 100;
    $info['time_end'] = date('Y-m-d H:i:s',strtotime($info['time_end']));
    $smarty->assign('info',   $info);
    $smarty->assign('out_trade_no',   $bill_data);
    $smarty->assign('sort_order_time', '<img src="images/sort_desc.gif">');
    /* 显示模板 */
    assign_query_info();
    $smarty->display('order_query_pay.htm');
}
...
elseif ($_REQUEST['act'] == 'query')
{
    ...
    /* 检查权限 */
    /*增值税发票_更改_START_www.yshop100.com*/
    ...
    /*增值税发票_更改_END_www.yshop100.com*/

    $order_list = order_list();
    
    if(intval($_REQUEST['supp']) > 0){
        $supplier_list = get_supplier_list();
        $smarty->assign('supp_list',   $supplier_list);
    }
...
elseif ($_REQUEST['act'] == 'info')
{
    ...
    if ($order['user_id'] > 0)
    {
        ...
        if (!empty($user))
        {
            ...
            $order['invite_parent_username'] = get_invite_parent_username($user['parent_id']);
        }
    }
    ...
    //实收货币总价
    $is_exchange = $db->getOne("SELECT is_exchange FROM " . $ecs->table('suppliers') . " WHERE suppliers_id='". $order['suppliers_id'] ."'");
    if($is_exchange > 0){
        $order['exchange_amount'] = "HKD".$order['exchange_goods_amount'];
    }else{
        $order['exchange_amount'] = "CNY".$order['goods_amount'];
    }

    //实体店、导购员
    $order['store_name'] = $db->getOne("SELECT store_name FROM zs_stores WHERE store_id='". $order['store_id'] ."'");
    $order['store_name'] = $order['store_name'] ? $order['store_name'] : '平台方';
    $order['seller_name'] = $db->getOne("SELECT seller_name FROM zs_stores_seller WHERE seller_id='". $order['seller_id'] ."' AND deleted = '0'");

    $order['pt_id'] = $db->getOne("SELECT pt_id FROM ecs_pintuan_orders WHERE order_id='". $order['order_id'] ."'");
    /* 参数赋值：订单 */
    ...
    $goods_list = array();
    $goods_attr = array();
    $sql = "SELECT o.*, IF(o.product_id > 0, p.product_number, g.goods_number) AS storage, o.goods_attr, g.suppliers_id, IFNULL(b.brand_name, '') AS brand_name, p.product_sn, ss.suppliers_name, ss.is_exchange, o.exchange_price 
            FROM " . $ecs->table('order_goods') . " AS o
                LEFT JOIN " . $ecs->table('products') . " AS p
                    ON p.product_id = o.product_id
                LEFT JOIN " . $ecs->table('goods') . " AS g
                    ON o.goods_id = g.goods_id 
                LEFT JOIN " . $ecs->table('suppliers') . " AS ss
                    ON ss.suppliers_id = g.suppliers_id  
    ...
    while ($row = $db->fetchRow($res))
    {
        ...
        if($row['is_exchange'] > 0){
            $row['exchange_title'] = 'HKD';
            $row['exchange_subtotal'] = $row['exchange_price'] * $row['goods_number'];
            
        }else{
            $row['exchange_title'] = 'CNY';
            $row['exchange_subtotal'] = $row['goods_price'] * $row['goods_number'];
            $row['exchange_price'] = $row['goods_price'];
        }
        ...
        if ($row['extension_code'] == 'package_buy')
        {
            $row['storage'] = '';
            $row['brand_name'] = '';
            $row['package_goods_list'] = get_package_goods($row['goods_id'], $row['package_attr_id']); //修改  
        }
    ...
    //获取线下店铺店员列表
    $sellers_list = array();
    $sql = "SELECT store_id, store_name, store_desc AS seller_name FROM zs_stores";
    ...
    while ($row = $db->fetchRow($res))
    {
        $sellers_list[$row['store_id']][] = $row;

        $sql_seller = "SELECT * FROM zs_stores_seller WHERE store_id = $row[store_id] AND deleted = '0'";
        $res_seller = $db->query($sql_seller);
        while ($row_seller = $db->fetchRow($res_seller))
        {
            $sellers_list[$row['store_id']][] = $row_seller;
        }
    }
    $smarty->assign('sellers_list', $sellers_list);
...
elseif ($_REQUEST['act'] == 'step_post')
{
    ...
    elseif ('shipping' == $step)
    {
        ...
        // $shipping_fee = shipping_fee($shipping['shipping_code'], $shipping['configure'], $weight_amount['weight'], $weight_amount['amount'], $weight_amount['number']);echo"<br/>";
        $order = array(
...
elseif ($_REQUEST['act'] == 'merge')
{
    ...
    $sql = "SELECT o.order_sn, u.user_name " .
            ...
            "WHERE o.user_id > 0 " .
            "AND o.extension_code = '' " . order_query_sql('unprocessed', 'o.');
...
elseif (isset($_POST['to_sf_kuajing']))
{
    $order_id = '';
    /* 检查权限 */
    admin_priv('order_os_edit');
    ...
    $action_note    = isset($_REQUEST['action_note']) ? trim($_REQUEST['action_note']) : '';
        ...
        $order['total_discount'] = $order['discount'] + $order['bonus'];
        $order['total_amount'] = $order['goods_amount'] + $order['shipping_fee'] + $order['tax'] - $order['total_discount'];
        
        //顺丰
        $sql = "SELECT keyname, keyvalue FROM sf_config";
        $res = $db->query($sql);
        while ($row = $db->fetchRow($res)){
            $order[$row['keyname']] = $row['keyvalue'];
        }

        foreach ($goods_list as $k => $goods) {
            $order['orderTotalCount'] += $goods['goods_number'];
            $goods_list[$k]['productionMarketingCountry'] = $db->getOne("SELECT countryname FROM " . $ecs->table('country') . " WHERE countrycode = '".$goods['countrycode']."'");
            $goods_list[$k]['declareMeasureUnit'] = $db->getOne("SELECT unitname FROM " . $ecs->table('unit') . " WHERE unitcode = '".$goods['unitcode']."'");
        }

        if($order['exchange_discount'] > 0 || $order['exchange_bonus'] > 0){
            $order['dynamic1'] = $order['exchange_discount'] + $order['exchange_bonus'];
        }else{
            $order['dynamic1'] = 0;
        }

        $order['pay_desc'] = $db->getOne("SELECT pay_desc FROM " . $ecs->table('payment') . " WHERE pay_id = '".$order['pay_id']."'");
        $order['out_trade_no'] = $db->getOne("SELECT trade_no FROM " . $ecs->table('order_info_apply') . " WHERE order_id = '".$order_id."'");
        $order['payment_time'] = local_date($_CFG['time_format'], $order['pay_time']);

        /* 模板赋值 */
        $smarty->assign('country_list', country_list($goods['countrycode']));
        $smarty->assign('unit_list', unit_list_name($goods['unitcode']));
        $smarty->assign('order', $order);
        $smarty->assign('declTime', date("Ymd")); 
        $smarty->assign('goods_list', $goods_list);
        $smarty->assign('order_id', $order_id);  
        $smarty->assign('shenbao_time', $shenbao_time);  
        $smarty->assign('shenbao_result', $shenbao_result);  
        assign_query_info();
        $smarty->display('order_sf_kuajing.htm');
        exit;
...
elseif (isset($_POST['to_dw_kuajing']))
{
    $order_id = '';
    /* 检查权限 */
    admin_priv('order_os_edit');
    ...
    $action_note    = isset($_REQUEST['action_note']) ? trim($_REQUEST['action_note']) : '';
        ...
        $sql = "SELECT keyname, keyvalue FROM dw_config";
        $res = $db->query($sql);
        while ($row = $db->fetchRow($res)){
            $order[$row['keyname']] = $row['keyvalue'];
        }
        ...
        $order['total_discount'] = $order['discount'] + $order['bonus'];
        // $order['exchange_goods_discount_amount'] = $order['exchange_goods_amount'] - $order['exchange_total_discount'];
        $order['total_amount'] = $order['goods_amount'] + $order['shipping_fee'] + $order['tax'] - $order['total_discount'];
        foreach ($goods_list as $k => $goods) {
            $order['orderTotalCount'] += $goods['goods_number'];
            $goods_list[$k]['productionMarketingCountry'] = $db->getOne("SELECT countryname FROM " . $ecs->table('country') . " WHERE countrycode = '".$goods['countrycode']."'");
        ...
        /* 模板赋值 */
        $smarty->assign('country_list', country_list($goods['countrycode']));
        $smarty->assign('unit_list', unit_list($goods['unitcode']));
        $smarty->assign('order', $order);
        $smarty->assign('goods_list', $goods_list);
        $smarty->assign('order_id', $order_id);  
        $smarty->assign('shenbao_time', $shenbao_time);  
        $smarty->assign('shenbao_result', $shenbao_result);  
        assign_query_info();
        $smarty->display('order_dw_kuajing.htm');
        exit;
...
elseif (isset($_POST['to_gz_shenbao']))
{
    $order_id = '';
    /* 检查权限 */
    admin_priv('order_os_edit');
    ...
    $action_note    = isset($_REQUEST['action_note']) ? trim($_REQUEST['action_note']) : '';
        ...
        $act_list = array();
        $sql = "SELECT keyname, keyvalue FROM zs_config";
        $res = $db->query($sql);
        while ($row = $db->fetchRow($res))
        {
            if("ebpCode"==$row['keyname'])
            $order["c_ebpCode"]    = $row['keyvalue'];
            else if("systime"==$row['keyname'])
            $order["c_systime"]    = $row['keyvalue'];
            else if("sysdate"==$row['keyname'])
            $order["c_sysdate"]    = $row['keyvalue'];
            else if("version"==$row['keyname'])
            $order["c_version"]    = $row['keyvalue'];
            else if("ebpName"==$row['keyname'])
            $order["c_ebpName"]    = $row['keyvalue'];
            else if("ebcCode"==$row['keyname'])
            $order["c_ebcCode"]    = $row['keyvalue'];
            else if("ebcName"==$row['keyname'])
            $order["c_ebcName"]    = $row['keyvalue'];
            else if("payCode1"==$row['keyname'])
            $order["c_payCode1"]    = $row['keyvalue']; 
            else if("payCode2"==$row['keyname'])
            $order["c_payCode2"]    = $row['keyvalue'];
            else if("payName1"==$row['keyname'])
            $order["c_payName1"]    = $row['keyvalue'];
            else if("payName2"==$row['keyname'])
            $order["c_payName2"]    = $row['keyvalue'];
            else if("copCode"==$row['keyname'])
            $order["c_copCode"]    = $row['keyvalue'];
            else if("copName"==$row['keyname'])
            $order["c_copName"]    = $row['keyvalue']; 
            else if("dxpMode"==$row['keyname'])
            $order["c_dxpMode"]    = $row['keyvalue'];
            else if("dxpId"==$row['keyname'])
            $order["c_dxpId"]    = $row['keyvalue'];
            else if("assureCode"==$row['keyname'])
            $order["c_assureCode"]    = $row['keyvalue'];
            // else if("emsNo"==$row['keyname'])
            // $order["c_emsNo"]    = $row['keyvalue'];
            else if("gz_customsCode"==$row['keyname'])
            $order["c_customsCode"]    = $row['keyvalue'];
            else if("gz_portCode"==$row['keyname'])
            $order["c_portCode"]    = $row['keyvalue'];
            else if("agentCode"==$row['keyname'])
            $order["c_agentCode"]    = $row['keyvalue'];
            else if("agentName"==$row['keyname'])
            $order["c_agentName"]    = $row['keyvalue'];
            // else if("areaCode"==$row['keyname'])
            // $order["c_areaCode"]    = $row['keyvalue'];
            // else if("areaName"==$row['keyname'])
            // $order["c_areaName"]    = $row['keyvalue'];
            else if("tradeMode"==$row['keyname'])
            $order["c_tradeMode"]    = $row['keyvalue'];
            else if("trafMode"==$row['keyname'])
            $order["c_trafMode"]    = $row['keyvalue'];
            else if("country"==$row['keyname'])
            $order["c_country"]    = $row['keyvalue'];
            else if("currency"==$row['keyname'])
            $order["c_currency"]    = $row['keyvalue'];
            // else if("itemRecordNo"==$row['keyname'])
            // $order["c_itemRecordNo"]    = $row['keyvalue'];
            else if("logisticsCode"==$row['keyname'])
            $order["c_logisticsCode"]    = $row['keyvalue'];
            else if("logisticsName"==$row['keyname'])
            $order["c_logisticsName"]    = $row['keyvalue'];
            else if("BaseTransfernote"==$row['keyname'])
            $order["c_BaseTransfernote"]    = $row['keyvalue'];
        }
        $sql = "SELECT  addtime,guid FROM `zs_purchaseorder` WHERE oldorderid='$order_id' order by addtime desc LIMIT 1";
        $shenbao = $db->query($sql); 
        $shenbao_time = "";
        $shenbao_guid = "";
        while ($row = $db->fetchRow($shenbao))
        {
            $shenbao_time = $row['addtime'];
            $shenbao_guid = $row['guid'];
        } 
        if($shenbao_guid!="")
        { 
                $shenbao_result =getshenbaoresult($shenbao_guid);
        }

        $row = $db->getOne("SELECT mailNo FROM sf_kuajing_log WHERE customerOrderNo = '".$order['order_sn']."'");
        if($row){
            $order['mailNo'] = $row['mailNo'];
        }
        ...
        // if($order['exchange_total_discount'] > 0){
        //     $goods_count = count($goods_list);
        //     foreach ($goods_list as $k => $goods) {
        //         if($k != $goods_count-1){
        //             $goods_discount = $goods['exchange_price'] / $order['exchange_goods_amount'] * $order['exchange_goods_discount_amount'];
        //             $goods_list[$k]['exchange_goods_discount'] = round($goods_discount, 2);
        //             $total_discount = $goods_list[$k]['exchange_goods_discount'] + $total_discount;
        //         }else{
        //             //最后一件减去所有折扣的差
        //             $goods_discount = $order['exchange_goods_discount_amount'] - $total_discount;
        //             $goods_list[$k]['exchange_goods_discount'] = round($goods_discount, 2);
        //         }
        ...
        $smarty->assign('shenbao_time', $shenbao_time);  
        $smarty->assign('shenbao_result', $shenbao_result);  
        $smarty->assign('operation', 'gz_haiguan');
        assign_query_info();
        $smarty->display('order_haiguan.htm');
        exit;
...
elseif (isset($_POST['to_fj_shenbao']))
{
    $order_id = '';
    /* 检查权限 */
    admin_priv('order_os_edit');
    ...
    $action_note    = isset($_REQUEST['action_note']) ? trim($_REQUEST['action_note']) : '';
        ...
        while ($row = $db->fetchRow($res))
        {
            ...
            $order["c_emsNo"]    = $row['keyvalue'];
            else if("customsCode"==$row['keyname'])
            $order["c_customsCode"]    = $row['keyvalue'];
            else if("portCode"==$row['keyname'])
            $order["c_portCode"]    = $row['keyvalue'];
            else if("agentCode"==$row['keyname'])
            $order["c_agentCode"]    = $row['keyvalue'];
            else if("agentName"==$row['keyname'])
        ...
        $smarty->assign('shenbao_time', $shenbao_time);  
        $smarty->assign('shenbao_result', $shenbao_result);  
        $smarty->assign('operation', 'fj_haiguan');
        assign_query_info();
        $smarty->display('order_haiguan.htm');
        exit;
}
...
/*------------------------------------------------------ */
//-- start一键发货 增加 by www.yshop100.com
/*------------------------------------------------------ */
...
//                    if (empty($value['extension_code']) || $value['extension_code'] == 'virtual_card')
    ...
    //更新售货员信息
    if(!empty($_REQUEST['seller']) && $_REQUEST['seller'] != 0){
        $seller = array();
        $seller = explode(",",$_REQUEST['seller']);
        $arr['store_id'] = (trim($seller[0]) > 0) ? trim($seller[0]) : 0;
        $arr['seller_id'] = (trim($seller[1]) > 0) ? trim($seller[1]) : 0;
    }else{
        $arr['store_id'] = 0;
        $arr['seller_id'] = 0;
    }
...
/*------------------------------------------------------ */
//-- end一键发货 增加 by www.yshop100.com
/*------------------------------------------------------ */
    /* 付款 */
    ...
    elseif (isset($_POST['assign']))
    {
        ...
        /* 操作成功 */
        $links[] = array('href' => 'order.php?act=list&' . list_link_postfix(), 'text' => $_LANG['01_order_list']);//supplier.yshop100.com
        sys_msg($_LANG['act_ok'], 0, $links);
    }
    /* 订单删除 */
    ...
    elseif (isset($_POST['print']))
    {
        ...
        foreach ($order_sn_list as $order_sn)
        {
            ...
            $goods_list = array();
            $goods_attr = array(); 
            $sql = "SELECT o.*, g.goods_number AS storage, o.goods_attr, IFNULL(b.brand_name, '') AS brand_name , ss.suppliers_name " .
                    "FROM " . $ecs->table('order_goods') . " AS o ".
                    "LEFT JOIN " . $ecs->table('goods') . " AS g ON o.goods_id = g.goods_id " .
                    "LEFT JOIN " . $ecs->table('suppliers') . " AS ss ON g.suppliers_id = ss.suppliers_id " .
...
elseif ($_REQUEST['act'] == 'batch_operate_post')
{
    ...
    elseif ('invalid' == $operation)
    {
        ...
        {
            $sql = "SELECT * FROM " . $ecs->table('order_info') .
                " WHERE order_sn = $id_order" . order_query_sql('unpay_unship', 'o.');
...
//    $action_note = $_REQUEST['action_note'];
    ...
    else
    {
        ...
        $smarty->assign('action_link', array('href' => 'order.php?act=list', 'text' => $_LANG['01_order_list']));//supplier.yshop100.com
...
{
    /* 申报福建海关 */
    if ('fj_haiguan' == $_REQUEST['operation'])
    {  
        $order_id   = intval(trim($_POST['order_id']));    
        $ebpCode  = str_replace("'","''",$_POST['ebpCode']);
        $ebpName  = str_replace("'","''",$_POST['ebpName']);
        $ebcCode  = str_replace("'","''",$_POST['ebcCode']);
        $ebcName  = str_replace("'","''",$_POST['ebcName']);
        $agentCode  = str_replace("'","''",$_POST['agentCode']);
        $agentName  = str_replace("'","''",$_POST['agentName']);
        $areaCode  = str_replace("'","''",$_POST['areaCode']);
        $areaName  = str_replace("'","''",$_POST['areaName']); 
        $logisticsNo  = str_replace("'","''",$_POST['logisticsNo']);
        $logisticsCode  = str_replace("'","''",$_POST['logisticsCode']);
        $logisticsName  = str_replace("'","''",$_POST['logisticsName']);
        $declTime  = str_replace("'","''",$_POST['declTime']);
        $customsCode  = str_replace("'","''",$_POST['customsCode']);
        $portCode  = str_replace("'","''",$_POST['portCode']);
        $ieDate  = str_replace("'","''",$_POST['ieDate']);
        $copNo  = str_replace("'","''",$_POST['copNo']); 
        $preNo  = str_replace("'","''",$_POST['preNo']);
        $assureCode  = str_replace("'","''",$_POST['assureCode']);
        $emsNo  = str_replace("'","''",$_POST['emsNo']);
        $invtNo  = str_replace("'","''",$_POST['invtNo']);
        $ieFlag  = str_replace("'","''",$_POST['ieFlag']);
        $countrym  = str_replace("'","''",$_POST['countrym']);
        $tradeMode  = str_replace("'","''",$_POST['tradeMode']);
        $buyerIdNumber  = str_replace("'","''",$_POST['buyerIdNumber']);
        $buyerName  = str_replace("'","''",$_POST['buyerName']);
        $buyerTelephone  = str_replace("'","''",$_POST['buyerTelephone']);
        $consigneeAddress  = str_replace("'","''",$_POST['consigneeAddress']);  
        $trafMode  = str_replace("'","''",$_POST['trafMode']);
        $freight  = str_replace("'","''",$_POST['freight']);
        $insuredFee  = str_replace("'","''",$_POST['insuredFee']);
        $currency  = str_replace("'","''",$_POST['currency']);
        $packNo  = str_replace("'","''",$_POST['packNo']);
        $grossWeight  = str_replace("'","''",$_POST['grossWeight']);
        $netWeight  = str_replace("'","''",$_POST['netWeight']);
        $action_note  = str_replace("'","''",$_POST['action_note']);
        $order_sn  = str_replace("'","''",$_POST['order_sn']);
        
        $sql = "SELECT LPAD(lastnumber+1,numberlength,'0') as num,`prefix` FROM `zs_number` WHERE `table`='zs_purchaseorder'";
        $_num = $db->query($sql);
        $s_num = $db->fetchRow($_num);
        $neworder_id= $s_num['prefix'].((string)$s_num['num']); 
        $sql = "UPDATE `zs_number` SET `lastnumber`=" . ((string)$s_num['num']) . " WHERE `table`='zs_purchaseorder'";
        $db->query($sql);
        
        $sql = "INSERT INTO `zs_purchaseorder`(`order_id`,oldorderid, `appType`, `appTime`, `appStatus`, `ebpCode`, `ebpName`, `ebcCode`, `ebcName`, `logisticsNo`, `logisticsCode`,";
        $sql = $sql ."`logisticsName`, `copNo`, `preNo`, `assureCode`, `emsNo`, `invtNo`, `ieFlag`, `declTime`, `customsCode`, `portCode`, `ieDate`, `buyerIdNumber`, `buyerName`,";
        $sql = $sql ."`buyerTelephone`, `consigneeAddress`, `agentCode`, `agentName`, `areaCode`, `areaName`, `tradeMode`, `trafMode`, `trafNo`, `voyageNo`, `billNo`, `loctNo`, `licenseNo`,";
        $sql = $sql ."`country`, `freight`, `insuredFee`, `currency`, `wrapType`, `packNo`, `grossWeight`, `netWeight`, `note`,addtime,order_sn) VALUES (";
        $sql = $sql ."'".(string)$neworder_id."','".(string)$order_id."', '1', '{systime}', '2', '".(string)$ebpCode."', '".(string)$ebpName."', '".(string)$ebcCode."', '".(string)$ebcName."', '".(string)$logisticsNo."', '".(string)$logisticsCode."',";
        $sql = $sql ."'".(string)$logisticsName."', '".(string)$copNo."', '".(string)$preNo."', '".(string)$assureCode."', '".(string)$emsNo."', '".(string)$invtNo."', '".(string)$ieFlag."', '".(string)$declTime."', '".(string)$customsCode."', '".(string)$portCode."', '".(string)$ieDate."', '".(string)$buyerIdNumber."', '".(string)$buyerName."',";
        $sql = $sql ."'".(string)$buyerTelephone."', '".(string)$consigneeAddress."', '".(string)$agentCode."', '".(string)$agentName."', '".(string)$areaCode."', '".(string)$areaName."', '".(string)$tradeMode."', '".(string)$trafMode."', '', '', '', '', '',";
        $sql = $sql ."'".(string)$countrym."', '".(string)$freight."', '".(string)$insuredFee."', '".(string)$currency."', '', '".(string)$packNo."', '".(string)$grossWeight."', '".(string)$netWeight."', '".(string)$action_note."',now(),'".(string)$order_sn."'";
        $sql = $sql .")";
        // file_put_contents("haiguan.txt", " sql_zs_purchaseorder:".$sql, FILE_APPEND);
        $db->query($sql);
        
        $sql = "insert into zs_purchaseorder_apply( `m_id`, `isapply`, `applytype`, `createtime`, `applycount`)values('".(string)$neworder_id."', '0', 'goodslist', now(), 0)";
        $db->query($sql);
        
        $itemRecordNo  = $_POST['itemRecordNo'];
        $itemNo  = $_POST['itemNo'];
        ...
        {
            $sql = "INSERT INTO `zs_purchaseorder_goods`(`Order_id`, `gnum`, `itemRecordNo`, `itemNo`, `itemName`, `gcode`, `gname`, `gmodel`, `barCode`,";
            $sql = $sql."`country`, `currency`, `qty`, `unit`,`qty1`,`unit1`, `qty2`,`unit2`, `price`, `totalPrice`, `note`) VALUES (";
            $sql = $sql."'".$neworder_id."', '".(string)($i+1)."', '".str_replace("'","''",$itemRecordNo[$i])."', '".str_replace("'","''",$itemNo[$i])."', '".str_replace("'","''",$itemName[$i])."', '".str_replace("'","''",$gcode[$i])."', '".str_replace("'","''",$gname[$i])."', '".str_replace("'","''",$gmodel[$i])."', '".str_replace("'","''",$barCode[$i])."',";
            $sql = $sql."'".str_replace("'","''",(string)$country[$i])."', '142', '".(string)$qty[$i]."', '".(string)$unit[$i]."','".(string)$qty1[$i]."','".(string)$unit1[$i]."','".(string)$qty2[$i]."','".(string)$unit2[$i]."', '".(string)$price[$i]."', '".(string)$totalPrice[$i]."', '".str_replace("'","''",$note[$i])."')";
            // file_put_contents("haiguan.txt", " zs_purchaseorder_goods:".$sql, FILE_APPEND);
            $db->query($sql);
        }
        
        $links[] = array('text' => $_LANG['order_info'], 'href' => 'order.php?act=info&order_id=' . $order_id);
        sys_msg("已经申报，请等待返回结果！", 0, $links);
        exit;
    }

    /* 申报广州海关 */
    if ('gz_haiguan' == $_REQUEST['operation'])
    {
        $order_id   = intval(trim($_POST['order_id']));    
        $appType   = intval(trim($_POST['appType']));   
        $ebpCode  = str_replace("'","''",$_POST['ebpCode']);
        $ebpName  = str_replace("'","''",$_POST['ebpName']);
        $ebcCode  = str_replace("'","''",$_POST['ebcCode']);
        $ebcName  = str_replace("'","''",$_POST['ebcName']);
        ...
        $taxTotal  = str_replace("'","''",$_POST['taxTotal']);
        $goodsAmount  = str_replace("'","''",$_POST['goodsAmount']);
        $orderAmount  = str_replace("'","''",$_POST['orderAmount']);
        $userName  = str_replace("'","''",$_POST['userName']);

        if($_POST['xmltype'] == '311'){
    		$guid = create_guid(); 
	        ...
	        <ceb:CEB311Message guid="'.$guid.'" version="1.0"  xmlns:ceb="http://www.chinaport.gov.cn/ceb" xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance">
	             <ceb:Order>
	                <ceb:OrderHead>
	                    ...
	                    <ceb:orderType>'.$ieFlag.'</ceb:orderType>
	                    <ceb:orderNo>'.$copNo.'</ceb:orderNo>
						...
						<ceb:consignee>'.$buyerName.'</ceb:consignee>
						<ceb:consigneeTelephone>'.$buyerTelephone.'</ceb:consigneeTelephone>
	        ...
	        {
	            $xml .= '<ceb:OrderList>
	                <ceb:gnum>'.($i+1).'</ceb:gnum>
				    <ceb:itemNo>'.$itemNo[$i].'</ceb:itemNo>
				    <ceb:itemName>'.$itemName[$i].'</ceb:itemName>
				    <ceb:gmodel>'.$gmodel[$i].'</ceb:gmodel>
					<ceb:unit>'.$unit[$i].'</ceb:unit>
					<ceb:qty>'.$qty[$i].'</ceb:qty>
					<ceb:price>'.$price[$i].'</ceb:price>
					<ceb:totalPrice>'.$totalPrice[$i].'</ceb:totalPrice>
					<ceb:currency>142</ceb:currency>
					<ceb:country>'.$country[$i].'</ceb:country>
					<ceb:note></ceb:note>
	            </ceb:OrderList>
	            ';
	        }
	        $xml .= '</ceb:Order>
	        <ceb:BaseTransfer>
	            <ceb:copCode>'.$ebpCode.'</ceb:copCode>
	            <ceb:copName>'.$ebpName.'</ceb:copName>
	            <ceb:dxpMode>DXP</ceb:dxpMode>
	            <ceb:dxpId>DXPENT0000018843</ceb:dxpId>
	            <ceb:note></ceb:note>
	        </ceb:BaseTransfer>
	        </ceb:CEB311Message>';

			$filename='CEB311_'.$date.'.xml';
        ...
        if($_POST['xmltype'] == '621'){
            $guid = create_guid(); 
            $date = date('ymdhis',time());
            header('Content-type:text/json;charset=UTF-8');
            $xml = '<?xml version="1.0" encoding="UTF-8" standalone="yes"?>
            <ceb:CEB621Message guid="'.$guid.'" version="1.0"  xmlns:ceb="http://www.chinaport.gov.cn/ceb" xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance">
                 <ceb:Order>
                    <ceb:OrderHead>
                        <ceb:guid>'.$guid.'</ceb:guid>
                        <ceb:appType>'.$appType.'</ceb:appType>
                        ...
                        <ceb:logisticsName>'.$logisticsName.'</ceb:logisticsName>
                        <ceb:copNo>'.$copNo.'</ceb:copNo>
                        <ceb:preNo>'.$preNo.'</ceb:preNo>
                        <ceb:assureCode>'.$assureCode.'</ceb:assureCode>
                        <ceb:emsNo>'.$emsNo.'</ceb:emsNo>
                        <ceb:invtNo>'.$invtNo.'</ceb:invtNo>
                        <ceb:ieFlag>'.$ieFlag.'</ceb:ieFlag>
                        <ceb:declTime>'.$declTime.'</ceb:declTime>
                        <ceb:customsCode>'.$customsCode.'</ceb:customsCode>
                        <ceb:portCode>'.$portCode.'</ceb:portCode>
                        <ceb:ieDate>'.$ieDate.'</ceb:ieDate>
                        <ceb:buyerIdType>1</ceb:buyerIdType>
                        <ceb:buyerIdNumber>'.$buyerIdNumber.'</ceb:buyerIdNumber>
                        <ceb:buyerName>'.$buyerName.'</ceb:buyerName>
                        <ceb:buyerTelephone>'.$buyerTelephone.'</ceb:buyerTelephone>
                        ...
                        <ceb:agentName>'.$agentName.'</ceb:agentName>
                        <ceb:areaCode>'.$areaCode.'</ceb:areaCode>
                        <ceb:areaName>'.$areaName.'</ceb:areaName>
                        <ceb:tradeMode>'.$tradeMode.'</ceb:tradeMode>
                        <ceb:trafMode>'.$trafMode.'</ceb:trafMode>
                        <ceb:trafNo>'.$trafNo.'</ceb:trafNo>
                        <ceb:voyageNo>'.$voyageNo.'</ceb:voyageNo>
                        <ceb:billNo>'.$billNo.'</ceb:billNo>
                        <ceb:loctNo>'.$loctNo.'</ceb:loctNo>
                        <ceb:licenseNo>'.$licenseNo.'</ceb:licenseNo>
                        <ceb:country>'.$countrym.'</ceb:country>
                        <ceb:freight>'.$freight.'</ceb:freight>
                        <ceb:insuredFee>'.$insuredFee.'</ceb:insuredFee>
                        <ceb:currency>142</ceb:currency>
                        <ceb:packNo>'.$packNo.'</ceb:packNo>
            ...
            // $sql = $sql ."'".(string)$countrym."', '".(string)$freight."', '".(string)$insuredFee."', '".(string)$currency."', '', '".(string)$packNo."', '".(string)$grossWeight."', '".(string)$netWeight."', '".(string)$action_note."',now(),'".(string)$order_sn."'";
            // $sql = $sql .")";
            // file_put_contents("gz_haiguan.txt", " sql_zs_purchaseorder:".$sql, FILE_APPEND);
            // $db->query($sql);
            
            // $sql = "insert into zs_purchaseorder_apply( `m_id`, `isapply`, `applytype`, `createtime`, `applycount`)values('".(string)$neworder_id."', '0', 'sf_goodslist', now(), 0)";
            // $db->query($sql);
            ...
            {
                $xml .= '<ceb:OrderList>
                    <ceb:itemRecordNo>'.$itemRecordNo[$i].'</ceb:itemRecordNo>
                    <ceb:itemNo>'.$itemNo[$i].'</ceb:itemNo>
                    <ceb:itemName>'.$itemName[$i].'</ceb:itemName>
                    <ceb:gcode>'.$gcode[$i].'</ceb:gcode>
                    <ceb:gname>'.$gname[$i].'</ceb:gname>
                    <ceb:gmodel>'.$gmodel[$i].'</ceb:gmodel>
                    <ceb:barCode>'.$barCode[$i].'</ceb:barCode>
                    <ceb:country>'.$country[$i].'</ceb:country>
                    <ceb:currency>142</ceb:currency>
                    <ceb:qty>'.$qty[$i].'</ceb:qty>
                    <ceb:unit>'.$unit[$i].'</ceb:unit>
                    <ceb:qty1>'.$qty1[$i].'</ceb:qty1>
                    <ceb:unit1>'.$unit1[$i].'</ceb:unit1>
                    <ceb:price>'.$price[$i].'</ceb:price>
                    <ceb:totalPrice>'.$totalPrice[$i].'</ceb:totalPrice>
                    <ceb:note'.$note[$i].'></ceb:note>
                </ceb:OrderList>
                ';
                ...
                // $sql = $sql."'".$neworder_id."', '".(string)($i+1)."', '".str_replace("'","''",$itemRecordNo[$i])."', '".str_replace("'","''",$itemNo[$i])."', '".str_replace("'","''",$itemName[$i])."', '".str_replace("'","''",$gcode[$i])."', '".str_replace("'","''",$gname[$i])."', '".str_replace("'","''",$gmodel[$i])."', '".str_replace("'","''",$barCode[$i])."',";
                // $sql = $sql."'".str_replace("'","''",(string)$country[$i])."', '142', '".(string)$qty[$i]."', '".(string)$unit[$i]."','".(string)$qty1[$i]."','".(string)$unit1[$i]."','".(string)$qty2[$i]."','".(string)$unit2[$i]."', '".(string)$price[$i]."', '".(string)$totalPrice[$i]."', '".str_replace("'","''",$note[$i])."')";
                // file_put_contents("gz_haiguan.txt", " zs_purchaseorder_goods:".$sql, FILE_APPEND);
                // $db->query($sql);
            }
        ...
        $links[] = array('text' => $_LANG['order_info'], 'href' => 'order.php?act=info&order_id=' . $order_id);
        sys_msg("申报失败！", 0, $links);
        exit;
    }

    /* 提交东莞保税仓 */
    if ('dw_kuajing' == $_REQUEST['operation'])
    {  

        $order_id   = intval(trim($_POST['order_id']));    
        $isUpdate  = str_replace("'","''",$_POST['isUpdate']);
        $orderNo = str_replace("'","''",$_POST['orderNo']);
        $clientCode = str_replace("'","''",$_POST['clientCode']);
        $soOrderCode = str_replace("'","''",$_POST['soOrderCode']);
        $shopCode = str_replace("'","''",$_POST['shopCode']);
        $ieFlag = str_replace("'","''",$_POST['ieFlag']);
        $businessModelCode = str_replace("'","''",$_POST['businessModelCode']);
        $orderTotalAmount = str_replace("'","''",$_POST['orderTotalAmount']);
        $orderGoodsAmount = str_replace("'","''",$_POST['orderGoodsAmount']);
        $orderTotalCount = str_replace("'","''",$_POST['orderTotalCount']);
        $freightCharge = str_replace("'","''",$_POST['freightCharge']);
        $tariffCharge = str_replace("'","''",$_POST['tariffCharge']);
        $valuationFee = str_replace("'","''",$_POST['valuationFee']);
        $discount = str_replace("'","''",$_POST['discount']);
        $orderTime = str_replace("'","''",$_POST['orderTime']);
        $orderStatus = str_replace("'","''",$_POST['orderStatus']);
        $roughWeight = str_replace("'","''",$_POST['roughWeight']);
        $netWeight = str_replace("'","''",$_POST['netWeight']);
        $skuDesc = str_replace("'","''",$_POST['skuDesc']);
        $buyerName = str_replace("'","''",$_POST['buyerName']);
        $buyerPhone = str_replace("'","''",$_POST['buyerPhone']);
        $buyerAddress = str_replace("'","''",$_POST['buyerAddress']);
        $buyerIdentityCardType = str_replace("'","''",$_POST['buyerIdentityCardType']);
        $buyerIdentityCardNo = str_replace("'","''",$_POST['buyerIdentityCardNo']);
        $buyerIdentityCardNo = strtoupper($buyerIdentityCardNo);
        $consigneeName = str_replace("'","''",$_POST['consigneeName']);
        $consigneePhone = str_replace("'","''",$_POST['consigneePhone']);
        $consigneeAddress = str_replace("'","''",$_POST['consigneeAddress']);
        $consigneeIdentityCardType = str_replace("'","''",$_POST['consigneeIdentityCardType']);
        $consigneeIdentityCardNo = str_replace("'","''",$_POST['consigneeIdentityCardNo']);
        $consigneeIdentityCardNo = strtoupper($consigneeIdentityCardNo);
        $destinationCountry = str_replace("'","''",$_POST['destinationCountry']);
        $destinationCountryCode = str_replace("'","''",$_POST['destinationCountryCode']);
        $province = str_replace("'","''",$_POST['province']);
        $city = str_replace("'","''",$_POST['city']);
        $district = str_replace("'","''",$_POST['district']);
        $businessType  = str_replace("'","''",$_POST['businessType']);
        $action_note  = str_replace("'","''",$_POST['action_note']);
        $order_sn  = str_replace("'","''",$_POST['order_sn']);

        //post数组
        $postData = array('OMSOrder' => array('isUpdate' => $isUpdate, 'orderNo' => $orderNo, 'clientCode' => $clientCode, 'soOrderCode' => $soOrderCode, 'shopCode' => $shopCode, 'ieFlag' => $ieFlag, 'businessModelCode' => $businessModelCode, 'orderTotalAmount' => $orderTotalAmount, 'orderGoodsAmount' => $orderGoodsAmount, 'orderTotalCount' => 
            $orderTotalCount, 'freightCharge' => $freightCharge, 'tariffCharge' => $tariffCharge, 'valuationFee' => $valuationFee, 'discount' => $discount, 'orderTime' => $orderTime, 'orderStatus' => $orderStatus, 'roughWeight' => $roughWeight, 'netWeight' => $netWeight, 'skuDesc' => $skuDesc, 'businessType' => $businessType, 'platformCode' => 'KUAISHAN'));

        $sql = "INSERT INTO `dw_purchaseorder`(`order_id`,guid, `isUpdate`, `orderNo`, `clientCode`, `soOrderCode`, `shopCode`, `ieFlag`, `businessModelCode`, `orderTotalAmount`, `orderGoodsAmount`,";
        $sql = $sql ."`orderTotalCount`, `freightCharge`, `tariffCharge`, `valuationFee`, `discount`, `orderTime`, `orderStatus`, `roughWeight`, `netWeight`, `skuDesc`, `businessType`, `buyerName`, `buyerPhone`, `buyerAddress`, `buyerIdentityCardType`, `buyerIdentityCardNo`, `consigneeName`, `consigneeAddress`,";
        $sql = $sql ."`consigneeIdentityCardType`, `consigneeIdentityCardNo`, `destinationCountry`, `destinationCountryCode`, `province`, `city`, `district`, `note`, `order_sn`, `addtime`) VALUES (";
        $sql = $sql ."'".(string)$order_id."','".(string)$guid."', '".(string)$isUpdate."', '".(string)$orderNo."', '".(string)$clientCode."', '".(string)$soOrderCode."', '".(string)$shopCode."', '".(string)$ieFlag."', '".(string)$businessModelCode."', '".(string)$orderTotalAmount."', '".(string)$orderGoodsAmount."',";
        $sql = $sql ."'".(string)$orderTotalCount."', '".(string)$freightCharge."', '".(string)$tariffCharge."', '".(string)$valuationFee."', '".(string)$discount."', '".(string)$orderTime."', '".(string)$orderStatus."', '".(string)$roughWeight."', '".(string)$netWeight."', '".(string)$skuDesc."', '".(string)$businessType."', '".(string)$buyerName."', '".(string)$buyerPhone."', '".(string)$buyerAddress."', '".(string)$buyerIdentityCardType."', '".(string)$buyerIdentityCardNo."', '".(string)$consigneeName."', '".(string)$consigneeAddress."',";
        $sql = $sql ."'".(string)$consigneeIdentityCardType."', '".(string)$consigneeIdentityCardNo."', '".(string)$destinationCountry."', '".(string)$destinationCountryCode."', '".(string)$province."', '".(string)$city."', '".(string)$district."', '".(string)$action_note."', '".(string)$order_sn."', now()";
        $sql = $sql .")";
        // file_put_contents("kuajing.txt", " sql_dw_purchaseorder:".$sql, FILE_APPEND);
        $db->query($sql);
        $porder_id = $db->insert_id();

        $orderDetailItemIndex = $_POST['orderDetailItemIndex'];
        ...
        $goodsItemNo  = $_POST['goodsItemNo'];
        $productionMarketingCountry  = $_POST['productionMarketingCountry'];
        $goodsNetWeight  = $_POST['goodsNetWeight'];
        $goodsRoughWeight  = $_POST['goodsRoughWeight'];
        $bargainCurrency  = $_POST['bargainCurrency'];
        $declareCount  = $_POST['declareCount'];
        $declareMeasureUnit  = $_POST['declareMeasureUnit'];
        $firstUnit  = $_POST['firstUnit'];
        $firstCount  = $_POST['firstCount'];
        $secondUnit  = $_POST['secondUnit'];
        $secondCount  = $_POST['secondCount'];
        $note  = $_POST['note'];
         
        // $postData .= '{"orderDetailList":';
        for($i = 0; $i < count($itemCode); $i++)
        {
            $sql = "INSERT INTO `dw_purchaseorder_goods`(`order_id`, `porder_id`, `orderDetailItemIndex`, `itemCode`, `itemName`, `unitPrice`, `itemCount`, `labelCode`, `stockLevel`, `goodsItemNo`, `productionMarketingCountry`,";
            $sql = $sql."`goodsNetWeight`, `goodsRoughWeight`, `bargainCurrency`, `declareCount`,`declareMeasureUnit`,`firstUnit`, `firstCount`,`secondUnit`, `secondCount`, `note`) VALUES (";
            $sql = $sql."'".$order_id."', '".$porder_id."', '".str_replace("'","''",(string)$orderDetailItemIndex[$i])."', '".str_replace("'","''",(string)$itemCode[$i])."', '".addslashes($itemName[$i])."', '".str_replace("'","''",(string)$unitPrice[$i])."', '".str_replace("'","''",(string)$itemCount[$i])."', '".str_replace("'","''",(string)$labelCode[$i])."', '".str_replace("'","''",(string)$stockLevel[$i])."', '".str_replace("'","''",(string)$goodsItemNo[$i])."', '".str_replace("'","''",(string)$productionMarketingCountry[$i])."', '".str_replace("'","''",(string)$goodsNetWeight[$i])."', '".str_replace("'","''",$goodsRoughWeight[$i])."',";
            $sql = $sql."'".str_replace("'","''",(string)$bargainCurrency[$i])."', '".(string)$declareCount[$i]."', '".(string)$declareMeasureUnit[$i]."','".(string)$firstUnit[$i]."','".(string)$firstCount[$i]."','".(string)$secondUnit[$i]."','".(string)$secondCount[$i]."', '".str_replace("'","''",$note[$i])."')";
            // file_put_contents("kuajing.txt", " dw_purchaseorder_goods:".$sql, FILE_APPEND);
            $db->query($sql);

            $postData['OMSOrder']['orderDetailList'][] = array('orderDetailItemIndex' => $orderDetailItemIndex[$i], 'itemCode' => $itemCode[$i], 'itemName' => $itemName[$i], 'unitPrice' => $unitPrice[$i], 'itemCount' => $itemCount[$i], 'labelCode' => $labelCode[$i], 'stockLevel' => $stockLevel[$i], 'goodsItemNo' => $goodsItemNo[$i], 'productionMarketingCountry' => $productionMarketingCountry[$i], 'goodsNetWeight' => $goodsNetWeight[$i], 'goodsRoughWeight' => $goodsRoughWeight[$i], 'bargainCurrency' => $bargainCurrency[$i], 'declareCount' => $declareCount[$i], 'declareMeasureUnit' => $declareMeasureUnit[$i], 'firstUnit' => $firstUnit[$i], 'firstCount' => $firstCount[$i], 'secondUnit' => $secondUnit[$i], 'secondCount' => $secondCount[$i]);
        }

        $postData['OMSOrder']['buyer'] = array('buyerName' => $buyerName, 'buyerPhone' => $buyerPhone, 'buyerAddress' => $buyerAddress, 'buyerIdentityCardType' => $buyerIdentityCardType, 'buyerIdentityCardNo' => $buyerIdentityCardNo);

        $postData['OMSOrder']['consignee'] = array('consigneeCountry' => '中国','consigneeName' => $consigneeName, 'consigneePhone' => $consigneePhone, 'consigneeAddress' => $consigneeAddress, 'consigneeIdentityCardType' => $consigneeIdentityCardType, 'destinationCountry' => $destinationCountry, 'destinationCountryCode' => $destinationCountryCode, 'province' => $province, 'city' => $city, 'district' => $district);
        
        include_once("/kuajingCommon.php");
        $kuajingCommon = new kuajingCommon();
        $host = $kuajingCommon->getHost();
        $key = $kuajingCommon->getkey();
        ...
        // $postData = '{"OMSOrder":{"isUpdate":"1","orderNo":"2020080375302","clientCode":"GZKS","soOrderCode":"2020080375302","shopCode":"KS","ieFlag":"IM","businessModelCode":"PIB000206","orderTotalAmount":"401.02","orderGoodsAmount":"401.02","orderTotalCount":"3","freightCharge":"0.00","tariffCharge":"0.00","valuationFee":"","discount":"0","orderTime":"2020-08-03 22:57:06","orderStatus":"ORDER","roughWeight":"0.7","netWeight":"0.5","skuDesc":"","businessType":"","platformCode":"KUAISHAN","orderDetailList":[{"orderDetailItemIndex":"1","itemCode":"Y0173","itemName":"\u864e\u6807\u9547\u75db\u836f\u5e03\u6e05\u51c9 9\u7247\u88c5","unitPrice":"89.29","itemCount":"2","labelCode":"8888650401031","stockLevel":"A","goodsItemNo":"3005109000","productionMarketingCountry":"\u65b0\u52a0\u5761","goodsNetWeight":"0.220","goodsRoughWeight":"0.3","bargainCurrency":"142","declareCount":"2","declareMeasureUnit":"\u76d2","firstUnit":"035","firstCount":"0.220","secondUnit":"","secondCount":""},{"orderDetailItemIndex":"2","itemCode":"Y0153","itemName":"Life space \u5a74\u513f\u76ca\u751f\u83cc 60g","unitPrice":"222.44","itemCount":"1","labelCode":"9331927003333","stockLevel":"A","goodsItemNo":"2106909090","productionMarketingCountry":"\u6fb3\u5927\u5229\u4e9a","goodsNetWeight":"0.060","goodsRoughWeight":"0.1","bargainCurrency":"142","declareCount":"1","declareMeasureUnit":"\u74f6","firstUnit":"035","firstCount":"0.060","secondUnit":"","secondCount":""}],"buyer":{"buyerName":"\u83ab\u6653\u5f66","buyerPhone":"15913182686","buyerAddress":"\u6c38\u5b81\u8857\u9053\u65b0\u65b0\u4e94\u8def\u91d1\u5730\u516c\u56ed\u4e0a\u57ce\u6d45\u5c71\u96625\u680b","buyerIdentityCardType":"1","buyerIdentityCardNo":"440105198205020917"},"consignee":{"consigneeCountry":"\u4e2d\u56fd","consigneeName":"\u83ab","consigneePhone":"15913182686","consigneeAddress":"\u6c38\u5b81\u8857\u9053\u65b0\u65b0\u4e94\u8def\u91d1\u5730\u516c\u56ed\u4e0a\u57ce\u6d45\u5c71\u96625\u680b","consigneeIdentityCardType":"1","destinationCountry":"\u4e2d\u56fd","destinationCountryCode":"CN","province":"\u5e7f\u4e1c","city":"\u5e7f\u5dde","district":"\u589e\u57ce\u533a"}}}';
        
        // file_put_contents("kuajing.txt", " dw_postData:".$sql, FILE_APPEND);
        $sign=$kuajingCommon ->getsign($security,$key,$method,$timestamp,$dataFormat,$version,$notifyId,$postData);

        $returnToken = $kuajingCommon->getToken($host, $key, $security);
        $returnTokenArray = json_decode($returnToken, TRUE);
        if($returnTokenArray['resultCode'] == '1'){
            $token = $returnTokenArray['resultData']['token'];
            $return = $kuajingCommon ->postmessage($host,$security,$key,$method,$timestamp,$dataFormat,$version,$notifyId,$postData,$sign,$token);
            
            if($return == TRUE){
                $result = json_decode($return, TRUE);
                if($result['resultCode'] == '1'){
                    //返回数据
                    $resultCode = isset($result['resultCode']) ? intval($result['resultCode']) : 0;
                    $resultContent = isset($result['resultContent']) ? trim($result['resultContent']) : '';
                    $resultData = isset($result['resultData']) ? trim($result['resultData']) : '';
                    $resultDataArray = json_decode($resultData, TRUE);
                    
                    $mailNo = isset($resultDataArray['mailNo']) ? trim($resultDataArray['mailNo']) : '';
                    $packageCenterCode = isset($resultDataArray['packageCenterCode']) ? trim($resultDataArray['resultData']) : '';
                    $destinationCode = isset($resultDataArray['destinationCode']) ? trim($resultDataArray['destinationCode']) : '';
                    $trackingNo = isset($resultDataArray['trackingNo']) ? trim($resultDataArray['trackingNo']) : '';
                    $destCode = isset($resultDataArray['destCode']) ? trim($resultDataArray['destCode']) : '';
                    $clientExpressCode = isset($resultDataArray['clientExpressCode']) ? trim($resultDataArray['clientExpressCode']) : '';
                    $shortAddress = isset($resultDataArray['shortAddress']) ? trim($resultDataArray['shortAddress']) : '';
                    $packageCenterName = isset($resultDataArray['packageCenterName']) ? trim($resultDataArray['packageCenterName']) : '';
                    $clientExpressName = isset($resultDataArray['clientExpressName']) ? trim($resultDataArray['clientExpressName']) : '';
                    $businessModelCode = isset($resultDataArray['businessModelCode']) ? trim($resultDataArray['businessModelCode']) : '';
                    $orderNo = isset($resultDataArray['orderNo']) ? trim($resultDataArray['orderNo']) : '';
                    $pdfFile = isset($resultDataArray['pdfFile']) ? trim($resultDataArray['pdfFile']) : '';
                    $distributeCode = isset($resultDataArray['distributeCode']) ? trim($resultDataArray['distributeCode']) : '';
                    $parcelCode = isset($resultDataArray['parcelCode']) ? trim($resultDataArray['parcelCode']) : '';

                    $sql = "INSERT INTO dw_purchaseorder_return ( order_id, porder_id, resultCode, resultContent, resultData, mailNo, packageCenterCode, destinationCode, trackingNo, destCode, clientExpressCode, shortAddress, packageCenterName, clientExpressName, businessModelCode, orderNo, pdfFile, distributeCode, parcelCode, return_time) VALUES ( '".$order_id."',  '".$porder_id."', '".$resultCode."', '".$resultContent."', '".$resultData."', '".$mailNo."', '".$packageCenterCode."', '".$destinationCode."', '".$trackingNo."', '".$destCode."', '".$clientExpressCode."', '".$shortAddress."', '".$packageCenterName."', '".$clientExpressName."', '".$businessModelCode."', '".$orderNo."', '".$pdfFile."', '".$distributeCode."', '".$parcelCode."', now())";
                    // file_put_contents("kuajing.txt", " dw_purchaseorder_return:".$sql, FILE_APPEND);
                    $db->query($sql);
                }else{
                    echo $return;
                    exit;
    ...
    /* 提交顺丰直邮 */
    if ('sf_kuajing' == $_REQUEST['operation'])
    {  
        $order_id   = intval(trim($_POST['order_id']));    
        $ebpCode  = str_replace("'","''",$_POST['ebpCode']);
        $ebpName  = str_replace("'","''",$_POST['ebpName']);
        ...
        $netWeight  = str_replace("'","''",$_POST['netWeight']);
        $action_note  = str_replace("'","''",$_POST['action_note']);
        $operate_type   = intval(trim($_POST['operate_type']));
        $reference_no1 = str_replace("'","''",$_POST['reference_no1']);
        $reference_no2 = str_replace("'","''",$_POST['reference_no2']);
        $mailno = str_replace("'","''",$_POST['mailno']);
        $parcel_quantity = str_replace("'","''",$_POST['parcel_quantity']);
        $is_get_bill_no = str_replace("'","''",$_POST['is_get_bill_no']);
        $j_contact = str_replace("'","''",$_POST['j_contact']);
        $j_tel = str_replace("'","''",$_POST['j_tel']);
        $j_area_code = str_replace("'","''",$_POST['j_area_code']);
        $j_mobile = str_replace("'","''",$_POST['j_mobile']);
        $j_country = str_replace("'","''",$_POST['j_country']);
        $j_province = str_replace("'","''",$_POST['j_province']);
        $j_city = str_replace("'","''",$_POST['j_city']);
        $j_county = str_replace("'","''",$_POST['j_county']);
        $j_address = str_replace("'","''",$_POST['j_address']);
        $j_post_code = str_replace("'","''",$_POST['j_post_code']);
        $d_contact = str_replace("'","''",$_POST['d_contact']);
        $d_contact_cn = str_replace("'","''",$_POST['d_contact_cn']);
        $d_tel = str_replace("'","''",$_POST['d_tel']);
        $d_country = str_replace("'","''",$_POST['d_country']);
        $d_province = str_replace("'","''",$_POST['d_province']);
        $d_city = str_replace("'","''",$_POST['d_city']);
        $d_county = str_replace("'","''",$_POST['d_county']);
        $d_address = str_replace("'","''",$_POST['d_address']);
        $d_post_code = str_replace("'","''",$_POST['d_post_code']);
        $express_type = str_replace("'","''",$_POST['express_type']);
        $pay_method = str_replace("'","''",$_POST['pay_method']);
        $custid = str_replace("'","''",$_POST['custid']);
        $tax_pay_type = str_replace("'","''",$_POST['tax_pay_type']);
        $tax_set_accounts = str_replace("'","''",$_POST['tax_set_accounts']);
        $cargo_length = str_replace("'","''",$_POST['cargo_length']);
        $cargo_width = str_replace("'","''",$_POST['cargo_width']);
        $cargo_height = str_replace("'","''",$_POST['cargo_height']);
        $currency = str_replace("'","''",$_POST['currency']);
        $printSize = str_replace("'","''",$_POST['printSize']);
        $buyers_nickname = str_replace("'","''",$_POST['buyers_nickname']);
        $tax = str_replace("'","''",$_POST['tax']);
        $freight = str_replace("'","''",$_POST['freight']);
        $dynamic4 = str_replace("'","''",$_POST['dynamic4']);
        $dynamic1 = str_replace("'","''",$_POST['dynamic1']);
        $turnover = str_replace("'","''",$_POST['turnover']);
        $payment_company_name = str_replace("'","''",$_POST['payment_company_name']);
        $payment_tool = str_replace("'","''",$_POST['payment_tool']);
        $payment_number = str_replace("'","''",$_POST['payment_number']);
        $payment_time = str_replace("'","''",$_POST['payment_time']);
        $payer_name = str_replace("'","''",$_POST['payer_name']);
        $order_cert_type = str_replace("'","''",$_POST['order_cert_type']);
        $order_cert_no = str_replace("'","''",$_POST['order_cert_no']);
        $ddp_remark = str_replace("'","''",$_POST['ddp_remark']);
        $store_name = str_replace("'","''",$_POST['store_name']);
        $dynamic9 = str_replace("'","''",$_POST['dynamic9']);
        $j_company = str_replace("'","''",$_POST['j_company']);
        $order_sn  = str_replace("'","''",$_POST['order_sn']);
        $customerCode = 'OSMS_7677';

        set_time_limit(0);
        header('Content-type:text/json;charset=UTF-8');
        //post数组
        $xml = '<?xml version="1.0" encoding="utf-8"?><Request service="apiOrderService" lang="zh-CN"><Head>'.$customerCode.'</Head><Body><Order orderid="'.$reference_no1.'" reference_no1="'.$reference_no1.'"';
        $xml .= $reference_no2 ? ' reference_no2="'.$reference_no2.'"': '';
        $xml .= $operate_type ? ' operate_type="'.$operate_type.'"': '';
        $xml .= $mailno ? ' mailno="'.$mailno.'"': '';
        $xml .= $parcel_quantity ? ' parcel_quantity="'.$parcel_quantity.'"': '';
        $xml .= $is_get_bill_no ? ' is_get_bill_no="'.$is_get_bill_no.'"': '';
        $xml .= $j_contact ? ' j_contact="'.$j_contact.'"': '';
        $xml .= $j_tel ? ' j_tel="'.$j_tel.'"': '';
        $xml .= $j_area_code ? ' j_area_code="'.$j_area_code.'"': '';
        $xml .= $j_mobile ? ' j_mobile="'.$j_mobile.'"': '';
        $xml .= $j_country ? ' j_country="'.$j_country.'"': '';
        $xml .= $j_province ? ' j_province="'.$j_province.'"': '';
        $xml .= $j_city ? ' j_city="'.$j_city.'"': '';
        $xml .= $j_county ? ' j_county="'.$j_county.'"': '';
        $xml .= $j_address ? ' j_address="'.$j_address.'"': '';
        $xml .= $j_post_code ? ' j_post_code="'.$j_post_code.'"': '';
        $xml .= $d_contact ? ' d_contact="'.$d_contact.'"': '';
        $xml .= $d_contact_cn ? ' d_contact_cn="'.$d_contact_cn.'"': '';
        $xml .= $d_tel ? ' d_tel="'.$d_tel.'"': '';
        $xml .= $d_country ? ' d_country="'.$d_country.'"': '';
        $xml .= $d_province ? ' d_province="'.$d_province.'"': '';
        $xml .= $d_city ? ' d_city="'.$d_city.'"': '';
        $xml .= $d_county ? ' d_county="'.$d_county.'"': '';
        $xml .= $d_address ? ' d_address="'.$d_address.'"': '';
        $xml .= $d_post_code ? ' d_post_code="'.$d_post_code.'"': '';
        $xml .= $express_type ? ' express_type="'.$express_type.'"': '';
        $xml .= $pay_method ? ' pay_method="'.$pay_method.'"': '';
        $xml .= $custid ? ' custid="'.$custid.'"': '';
        $xml .= $tax_pay_type ? ' tax_pay_type="'.$tax_pay_type.'"': '';
        $xml .= $tax_set_accounts ? ' tax_set_accounts="'.$tax_set_accounts.'"': '';
        $xml .= $cargo_length ? ' cargo_length="'.$cargo_length.'"': '';
        $xml .= $cargo_width ? ' cargo_width="'.$cargo_width.'"': '';
        $xml .= $cargo_height ? ' cargo_height="'.$cargo_height.'"': '';
        $xml .= $currency ? ' currency="'.$currency.'"': '';
        $xml .= $printSize ? ' printSize="'.$printSize.'"': '';
        $xml .= $buyers_nickname ? ' buyers_nickname="'.$buyers_nickname.'"': '';
        $xml .= $tax ? ' tax="'.$tax.'"': '';
        $xml .= $freight ? ' freight="'.$freight.'"': '';
        $xml .= $dynamic4 ? ' dynamic4="'.$dynamic4.'"': '';
        $xml .= $dynamic1 ? ' dynamic1="'.$dynamic1.'"': '';
        $xml .= $turnover ? ' turnover="'.$turnover.'"': '';
        $xml .= $payment_company_name ? ' payment_company_name="'.$payment_company_name.'"': '';
        $xml .= $payment_tool ? ' payment_tool="'.$payment_tool.'"': '';
        $xml .= $payment_number ? ' payment_number="'.$payment_number.'"': '';
        $xml .= $payment_time ? ' payment_time="'.$payment_time.'"': '';
        $xml .= $payer_name ? ' payer_name="'.$payer_name.'"': '';
        $xml .= $order_cert_type ? ' order_cert_type="'.$order_cert_type.'"': '';
        $xml .= $order_cert_no ? ' order_cert_no="'.$order_cert_no.'"': '';
        $xml .= $ddp_remark ? ' ddp_remark="'.$ddp_remark.'"': '';
        $xml .= $store_name ? ' store_name="'.$store_name.'"': '';
        $xml .= $dynamic9 ? ' dynamic9="'.$dynamic9.'"': '';
        $xml .= $j_company ? ' j_company="'.$j_company.'"': '';
        $xml .= '>';

        //顺丰订单
        $sql = "INSERT INTO `sf_kuajing`( `operate_type`, `reference_no1`, `reference_no2`, `mailno`, `parcel_quantity`, `is_get_bill_no`, `j_company`, `j_contact`, `j_tel`, `j_area_code`,";
        $sql = $sql ."`j_mobile`, `j_country`, `j_province`, `j_city`, `j_county`, `j_address`, `j_post_code`, `d_contact`, `d_contact_cn`, `d_tel`, `d_country`, `d_province`, `d_city`, `d_county`,";
        $sql = $sql ."`d_address`, `d_post_code`, `express_type`, `pay_method`, `custid`, `thd3_pay_area`, `tax_pay_type`, `tax_set_accounts`, `cargo_length`, `cargo_width`, ";
        $sql = $sql ."`cargo_height`, `currency`, `printSize`, `buyers_nickname`, `tax`, `freight`, `dynamic4`, `dynamic1`, `turnover`, `payment_company_name`, `payment_tool`, `payment_number`, `payment_time`, `payer_name`, `order_cert_type`, `order_cert_no`, `ddp_remark`, `store_name`, `dynamic9`, `addtime`) VALUES (";
            
        $sql = $sql ."'".(string)$operate_type."', '".(string)$reference_no1."', '".(string)$reference_no2."', '".(string)$mailno."', '".(string)$parcel_quantity."', '".(string)$is_get_bill_no."', '".(string)$j_company."', '".(string)$j_contact."', '".(string)$j_tel."', '".(string)$j_area_code."',";
        $sql = $sql ."'".(string)$j_mobile."', '".(string)$j_country."', '".(string)$j_province."', '".(string)$j_city."', '".(string)$j_county."', '".(string)$j_address."', '".(string)$j_post_code."', '".(string)$d_contact."', '".(string)$d_contact_cn."', '".(string)$d_tel."', '".(string)$d_country."', '".(string)$d_province."', '".(string)$d_city."', '".(string)$d_county."',";
        $sql = $sql ."'".(string)$d_address."', '".(string)$d_post_code."', '".(string)$express_type."', '".(string)$pay_method."', '".(string)$custid."', '".(string)$thd3_pay_area."', '".(string)$tax_pay_type."', '".(string)$tax_set_accounts."', '".(string)$cargo_length."', '".(string)$cargo_width."', ";
        $sql = $sql ."'".(string)$cargo_height."', '".(string)$currency."', '".(string)$printSize."', '".(string)$buyers_nickname."', '".(string)$tax."', '".(string)$freight."', '".(string)$dynamic4."', '".(string)$dynamic1."', '".(string)$turnover."', '".(string)$payment_company_name."', '".(string)$payment_tool."', '".(string)$payment_number."', '".(string)$payment_time."', '".(string)$payer_name."', '".(string)$order_cert_type."', '".(string)$order_cert_no."', '".(string)$ddp_remark."', '".(string)$store_name."', '".(string)$dynamic9."', now()";


        $sql = $sql .")";
        // file_put_contents("sf_kuajing.txt", " sql_sf_kuajing:".$sql, FILE_APPEND);
        $db->query($sql);
        $sforder_id = $db->insert_id();
        ...
        $weight  = $_POST['weight'];
        $currency  = $_POST['goods_currency'];
        $product_record_no  = $_POST['product_record_no'];
        $good_prepard_no  = $_POST['good_prepard_no'];
        $brand  = $_POST['brand'];
        $statebarcode  = $_POST['statebarcode'];
        ...
        {
            $sql = "INSERT INTO `sf_kuajing_goods`( `sforder_id`, `goods_code`, `name`, `count`, `amount`, `total_value`, `unit`, `source_area`, `weight`, `currency`,";
            $sql = $sql."`product_record_no`, `good_prepard_no`, `brand`,`statebarcode`,`specifications`, `hs_code`,`item_no`, `qty1`, `unit1`, `qty2`, `unit2`) VALUES (";
            $sql = $sql."'".$sforder_id."', '".str_replace("'","''",(string)$goods_code[$i])."', '".str_replace("'","''",(string)$name[$i])."', '".str_replace("'","''",(string)$count[$i])."', '".str_replace("'","''",(string)$amount[$i])."', '".str_replace("'","''",(string)$total_value[$i])."', '".str_replace("'","''",(string)$unit[$i])."', '".str_replace("'","''",(string)$source_area[$i])."', '".str_replace("'","''",(string)$weight[$i])."', '".str_replace("'","''",(string)$currency[$i])."', '".str_replace("'","''",(string)$product_record_no[$i])."', '".str_replace("'","''",$good_prepard_no[$i])."',";
            $sql = $sql."'".str_replace("'","''",(string)$brand[$i])."', '".(string)$statebarcode[$i]."', '".(string)$specifications[$i]."','".(string)$hs_code[$i]."','".(string)$item_no[$i]."','".(string)$qty1[$i]."','".(string)$unit1[$i]."', '".str_replace("'","''",$qty2[$i])."', '".str_replace("'","''",$unit2[$i])."')";
            // file_put_contents("sf_kuajing.txt", " sf_kuajing_goods:".$sql, FILE_APPEND);
            $db->query($sql);

            $xml .= '<Cargo';
            $xml .= $goods_code[$i] ? ' goods_code="'.$goods_code[$i].'"' : '';
            $xml .= $name[$i] ? ' name="'.$name[$i].'"' : '';
            $xml .= $count[$i] ? ' count="'.$count[$i].'"' : '';
            $xml .= $amount[$i] ? ' amount="'.$amount[$i].'"' : '';
            $xml .= $total_value[$i] ? ' total_value="'.$total_value[$i].'"' : '';
            $xml .= $unit[$i] ? ' unit="'.$unit[$i].'"' : '';
            $xml .= $source_area[$i] ? ' source_area="'.$source_area[$i].'"' : '';
            $xml .= $weight[$i] ? ' weight="'.$weight[$i].'"' : '';
            $xml .= $currency[$i] ? ' currency="'.$currency[$i].'"' : '';
            $xml .= $product_record_no[$i] ? ' product_record_no="'.$product_record_no[$i].'"' : '';
            $xml .= $good_prepard_no[$i] ? ' good_prepard_no="'.$good_prepard_no[$i].'"' : '';
            $xml .= $brand[$i] ? ' brand="'.$brand[$i].'"' : '';
            $xml .= $statebarcode[$i] ? ' statebarcode="'.$statebarcode[$i].'"' : '';
            $xml .= $specifications[$i] ? ' specifications="'.$specifications[$i].'"' : '';
            $xml .= $hs_code[$i] ? ' hs_code="'.$hs_code[$i].'"' : '';
            $xml .= $item_no[$i] ? ' item_no="'.$item_no[$i].'"' : '';
            $xml .= $qty1[$i] ? ' qty1="'.$qty1[$i].'"' : '';
            $xml .= $unit1[$i] ? ' unit1="'.$unit1[$i].'"' : '';
            $xml .= $qty2[$i] ? ' qty2="'.$qty2[$i].'"' : '';
            $xml .= $unit2[$i] ? ' unit2="'.$unit2[$i].'"' : '';
            $xml .= '/>';
        }

        $xml .= '</Order></Body></Request>';
        

        $checkword = '1d88f1819b1b43e7';
        ...
        $result=$client->sfexpressService(array('data'=>$data,'validateStr'=>$validateStr,'customerCode'=>$customerCode));//查询，返回的是一个结构体   returns a structure by quering
...
elseif($_REQUEST['act'] == 'seller_edit'){
    /* 检查权限 */
    ...
    //实体店、导购员
    $store_list = $db->getALL("SELECT store_id, store_name FROM zs_stores");
    $seller_list = $db->getALL("SELECT seller_id, seller_name FROM zs_stores_seller WHERE deleted = '0'");

    $smarty->assign('store_list',   $store_list);
    $smarty->assign('seller_list',   $seller_list);
    $smarty->assign('order',   $order);
    $smarty->assign('form_action',   'seller_update');

    /* 模板赋值 */
    $smarty->assign('ur_here', '<a href="order.php?act=info&order_id='.$order_id.'">'.
        $_LANG['order_info'].'</a> - 编辑订单导购员');

    $smarty->display('order_seller.htm');
}elseif($_REQUEST['act'] == 'seller_update'){
    /* 检查权限 */
    admin_priv('order_seller');
    ...
    $seller_id  = intval(trim($_POST['seller_id']));

    if($order_id > 0){
        $sql = "UPDATE ".$ecs->table('order_info')." SET store_id='".$store_id."', seller_id='".$seller_id."' WHERE order_id = '".$order_id."'";
        $db->query($sql);

        admin_log($_POST['old_store_id'].$_POST['old_seller_id'].'->'.$store_id.$seller_id, 'edit', 'seller_update');
        $lnk[] = array('text' => "返回上一页", 'href'=>'order.php?act=info&order_id='.$order_id.'');
        sys_msg("保存成功", 1,$lnk);
    }else{
        ecs_header("Location: order.php?act=list");
        exit;
...
function getshenbaoresult($shenbao_guid)
{
    if($shenbao_guid!="")
    {
        $sql="SELECT  `info`, `addtime` FROM `zs_reportreceive_log` WHERE guid='".$shenbao_guid."'";
        $shenbao = $GLOBALS['db']->query($sql); 
        $shenbao_result = ""; 
        while ($row = $GLOBALS['db']->fetchRow($shenbao))
        {
            $shenbao_result =$row['info']."  ".$row['addtime'];
        }
        return $shenbao_result ;
    }
}
/*取消开票*/
...
function order_list()
{
    ...
    if ($result === false)
    {
        ...
        $filter['suppliers_id'] = (isset($_REQUEST['suppliers_id']) && !empty($_REQUEST['suppliers_id']) && intval($_REQUEST['suppliers_id'])>0) ? intval($_REQUEST['suppliers_id']) : 0;
        /*增值税发票_添加_START_www.yshop100.com*/
        ...
        $filter['parent_id'] = empty($_REQUEST['parent_id']) ? '' : trim($_REQUEST['parent_id']);
		$tesh = $_REQUEST['tesh'];  
		$pintuan = $_REQUEST['pintuan'];  
        $filter['invite_parent_username'] = empty($_REQUEST['invite_parent_username']) ? '' : trim($_REQUEST['invite_parent_username']);
        $filter['invite_parent_username'] = empty($_POST['invite_parent_username']) ? $filter['invite_parent_username'] : trim($_POST['invite_parent_username']);
        $filter['country'] = empty($_POST['country']) ? $filter['country'] : intval($_POST['country']);
        $filter['province'] = empty($_POST['province']) ? $filter['province'] : intval($_POST['province']);
        $filter['city'] = empty($_POST['city']) ? $filter['city'] : intval($_POST['city']);
        $filter['district'] = empty($_POST['district']) ? $filter['district'] : intval($_POST['district']);
        /*增值税发票_添加_END_www.yshop100.com*/
        $filter['order_type'] = empty($_POST['order_type']) ? 0 : intval($_POST['order_type']);
        //$where = 'WHERE 1 ';
        ...
        if ($filter['suppliers_id']){
            //$where .= " AND o.supplier_id = ".$filter['suppid'];
            $where = 'WHERE o.suppliers_id = '.$filter['suppliers_id'];
        }

        if ($filter['parent_id'])
        {
            
           $where .= " AND u.parent_id = '$filter[parent_id]'";
        }
        
        if ($filter['invite_parent_username'])
        {
            
           $where .= " AND u.parent_id in(" . mysql_like_quote($filter['invite_parent_username']) . ")"; 
        }
		...
		{ 
			//增加门店订单权限 ecs_users_shopgroup
			$groupsql="insert into ecs_users_shopgroup(`parent_id`, `user_id`, `shopname`)";
			$groupsql=$groupsql."select distinct 0,parent_id,'平台方' from ecs_users where not exists(select 1 from ecs_users_shopgroup where ecs_users_shopgroup.user_id=ecs_users.parent_id)";
			$GLOBALS['db']->query($groupsql);
			
			//file_put_contents("t0523.txt",date('H:i:s',time())."\r\n1:".$groupsql."\r\n", FILE_APPEND);
			$groupsql="update ecs_users_shopgroup a inner join ecs_users c on c.user_id=a.user_id inner join ( SELECT DISTINCT `parent_id`,`shopname` FROM `ecs_users_shopgroup` WHERE parent_id>0 )b on c.user_name like concat(b.shopname,'%') set a.parent_id=b.parent_id,a.shopname=b.shopname";
			$GLOBALS['db']->query($groupsql);
			
			//file_put_contents("t0523.txt",date('H:i:s',time())."\r\n2:".$groupsql."\r\n", FILE_APPEND);
			$pidstr='-1'; 
			$actstr = explode(',',$_SESSION['action_list']); 
			for($index=0;$index<count($actstr);$index++){
				if(strpos($actstr[$index],'order_shop_') !== false){ 
					$idstr = explode('-',$actstr[$index]); 
					$pidstr=$pidstr.','.$idstr[1];  
				}
			} 
			$plikestr="SELECT user_id FROM ecs_users_shopgroup WHERE parent_id in(".$pidstr.")";		
			
			$row1 = $GLOBALS['db']->getAll($plikestr); 
			foreach ($row1 AS $key => $value)
			{
				
				$pidstr=$pidstr.','.$value['user_id'];
				//file_put_contents("t0523.txt",date('H:i:s',time())."\r\n(".$pidstr."\r\n", FILE_APPEND);
			}
			$where .= " AND u.parent_id in(" . mysql_like_quote($pidstr) . ")"; 
		}
		
        if ($tesh=='0')
        {
           $where .= " AND exists(select 1 from ecs_order_tesh where o.order_id=ecs_order_tesh.orderid) "; 
        }
        if ($pintuan=='1')
        {
           $where .= " AND exists(select 1 from ecs_pintuan_orders where o.order_id=ecs_pintuan_orders.order_id) "; 
        }
        else if ($pintuan=='3')
        {
           $where .= " AND not exists(select 1 from ecs_pintuan_orders where o.order_id=ecs_pintuan_orders.order_id) "; 
        }
        if ($filter['order_sn'])
        {
        ...
        if($_REQUEST['act'] == 'invoice_list' || (isset($_REQUEST['act_detail'])&&$_REQUEST['act_detail']=='invoice_query'))
        {
         ...
         /* 普通订单不显示虚拟团购订单 添加_START_www.yshop100.com*/
        $where .= " AND o.extension_code != 'virtual_good'";
         /* 普通订单不显示虚拟团购订单 添加_END_www.yshop100.com*/
        /*增值税发票_添加_END_www.yshop100.com*/
        /* 代码增加_start     */
        switch($filter['order_type'])
        {
        ...
        /* 查询 */
        if($filter['supp']){
            $sql = "SELECT o.order_id, o.order_sn, o.add_time, o.order_status, o.shipping_status, o.order_amount, o.money_paid, o.exchange_amount, o.exchange_goods_amount, ss.is_exchange, ss.suppliers_name, st.store_name, sts.seller_name, p.shop_name, p.address AS shop_address," .
                    "o.pay_status, o.consignee, o.address, o.email, o.tel, o.extension_code, o.extension_id,o.rebate_ispay,o.referer, " .
                    "(" . order_amount_field('o.') . ") AS total_fee, " .
                    "IFNULL(u.user_name, '" .$GLOBALS['_LANG']['anonymous']. "') AS buyer,length(u.card) as cardlength,supplier_name,o.froms,o.is_pickup  ".
                    /*增值税发票_添加_START_www.yshop100.com*/
                    ...
                    ',o.province,o.city,o.district,u.parent_id as invite_parent_username '.
                    /*增值税发票_添加_END_www.yshop100.com*/
                " FROM " . $GLOBALS['ecs']->table('order_info') . " AS o " .
                " LEFT JOIN " . $GLOBALS['ecs']->table('suppliers') . " AS ss ON ss.suppliers_id=o.suppliers_id ".
                " LEFT JOIN " . $GLOBALS['ecs']->table('supplier') . " AS s ON s.supplier_id=o.supplier_id ".
                " LEFT JOIN " . $GLOBALS['ecs']->table('pickup_point') . " AS p ON o.pickup_point = p.id ".
                " LEFT JOIN zs_stores AS st ON st.store_id=o.store_id ".
                " LEFT JOIN zs_stores_seller AS sts ON sts.seller_id=o.seller_id ".
                " LEFT JOIN " .$GLOBALS['ecs']->table('users'). " AS u ON u.user_id=o.user_id ". $where .
            ...
            $sql = "SELECT o.order_id, o.order_sn, o.add_time, o.order_status, o.shipping_status, o.order_amount, o.money_paid, o.exchange_amount, o.exchange_goods_amount, ss.is_exchange, ss.suppliers_name, st.store_name, sts.seller_name, p.shop_name, p.address AS shop_address," .
                    "o.pay_status, o.consignee, o.address, o.email, o.tel, o.extension_code, o.extension_id,o.rebate_ispay,o.referer, " .
                    "(" . order_amount_field('o.') . ") AS total_fee, " .
                    "IFNULL(u.user_name, '" .$GLOBALS['_LANG']['anonymous']. "') AS buyer,length(u.card) as cardlength, o.froms , o.is_pickup ".
                    /*增值税发票_添加_START_www.yshop100.com*/
                    ',o.mobile,o.inv_payee,o.inv_content,o.inv_type,o.vat_inv_company_name'.
                    ',o.vat_inv_taxpayer_id,o.vat_inv_registration_address,o.vat_inv_registration_phone'.
                    ',o.vat_inv_deposit_bank,o.vat_inv_bank_account'.
...
//file_put_contents('orderinvite0306.txt','--chaxun3---'.$sql, FILE_APPEND); 
    $row = $GLOBALS['db']->getAll($sql);

    /* 格式话数据 */
    foreach ($row AS $key => $value)
    {
        $row[$key]['invite_parent_username'] = get_invite_parent_username($value['invite_parent_username']);
        $row[$key]['province'] = get_region($value['province']);
        $row[$key]['city'] = get_region($value['city']);
        $row[$key]['district'] = get_region($value['district']);
...
$row[$key]['money1'] = $row[$key]['exchange_amount'];
$row[$key]['money2'] = $row[$key]['inv_money'];
$row[$key]['money3'] = "HKD" . $row[$key]['exchange_goods_amount'];

        //实收货币
        switch ($row[$key]['is_exchange']){
            case 0 :
                    $row[$key]['exchange_amount'] = "CNY" . $row[$key]['total_fee'];
                    break;
            case 1 :
                    $row[$key]['exchange_amount'] = "HKD" . $row[$key]['exchange_goods_amount'];
                    break;
            default :
                    break;
        }
        ...
        //导购员格式化
        $row[$key]['seller'] = $row[$key]['seller_name'] ? $row[$key]['store_name']."<br/>".$row[$key]['seller_name'] : $row[$key]['store_name'];
        $row[$key]['seller'] = $row[$key]['seller'] ? $row[$key]['seller'] : "平台方";
    }
...
function get_fenxiao_user($other_fenxiao)
{
    if($other_fenxiao == '1'){
        $sql = "select user_id,user_name,real_name from ". $GLOBALS['ecs']->table('users') .
        " where isfenxiao_topuser = '0' AND is_fenxiao = '1' AND status = '1' AND user_id != '0' order by user_name desc";
    }else{
         $sql = "select user_id,user_name,real_name from ". $GLOBALS['ecs']->table('users') .
        " where is_fenxiao = '1' AND status = '1' AND user_id != '0' order by user_name desc";
    }
...
function haiguan_order_list()
{ 
    $where="";
    $filter['order_sn'] = empty($_REQUEST['order_sn']) ? '' : trim($_REQUEST['order_sn']);
    if ($filter['order_sn'])
    {
        $where=$where." o.order_sn='".$filter['order_sn'] ."'";
    }
    $filter['consignee'] = empty($_REQUEST['consignee']) ? '' : trim($_REQUEST['consignee']);
    if ($filter['consignee'])
    {
        $where=$where." p.buyerName='".$filter['consignee']."'";
    }
        /* 分页大小 */
        $filter['page'] = empty($_REQUEST['page']) || (intval($_REQUEST['page']) <= 0) ? 1 : intval($_REQUEST['page']);
        ...
        $sql = "SELECT COUNT(*) FROM zs_purchaseorder AS p inner join  (select order_id,order_sn from ecs_order_info) o on p.oldorderid=o.order_id ". $where;
        

        $filter['record_count']   = $GLOBALS['db']->getOne($sql);
        $filter['page_count']     = $filter['record_count'] > 0 ? ceil($filter['record_count'] / $filter['page_size']) : 1;

    $sql = "SELECT o.order_sn,p.order_id as porderid,'' as result, `guid`, `appType`, `appTime`, `appStatus`, `ebpCode`, `ebpName`, `ebcCode`, `ebcName`, `logisticsNo`, `logisticsCode`, `logisticsName`, `copNo`, `preNo`, `assureCode`, `emsNo`, `invtNo`, `ieFlag`, `declTime`, `customsCode`, `portCode`, `ieDate`, `buyerIdNumber`, `buyerName`, `buyerTelephone`, `consigneeAddress`, `agentCode`, `agentName`, `areaCode`, `areaName`, `tradeMode`, `trafMode`, `trafNo`, `voyageNo`, `billNo`, `loctNo`, `licenseNo`, `country`, `freight`, `insuredFee`, `currency`, `wrapType`, `packNo`, `grossWeight`, `netWeight`, `note`, `oldorderid`, `addtime` FROM `zs_purchaseorder` AS p inner join  (select order_id,order_sn from ecs_order_info) o on p.oldorderid=o.order_id " .
        "  ". $where .
        " LIMIT " . ($filter['page'] - 1) * $filter['page_size'] . ",$filter[page_size]";
    ...
    foreach ($row AS $key => $value)
    {
        $row[$key]['result']=getshenbaoresult($value['guid']);
    }
    $arr = array('orders' => $row, 'filter' => $filter, 'page_count' => $filter['page_count'], 'record_count' => $filter['record_count']);
    return $arr;
}
...
function get_order_goods($order)
{
    $goods_list = array();
    $goods_attr = array();
    $sql = "SELECT o.*, g.suppliers_id AS suppliers_id,IF(o.product_id > 0, p.product_number, g.goods_number) AS storage,o.goods_number*o.goods_price as totalPrice,o.goods_price,o.goods_attr, IFNULL(b.brand_name, '') AS brand_name, p.product_sn,g.goods_model,g.countrycode,g.unitcode,g.gname,g.gcode,g.barCode,g.goods_weight,g.weight_unit,g.itemRecordNo " .
            " ,g.number2_unit,g.number2 ".
...
function get_fenxiao_topuser()
{
	...
	{ 
		...
		$wherestr = " AND user_id in(" . mysql_like_quote($pidstr) . ")"; 
	}
	if($dcount>1)
	{
		$sql = "select * from (SELECT user_id,user_name FROM `ecs_users` where isfenxiao_topuser = '1'   UNION SELECT 0 as user_id,'平台方' as user_name) a where 1=1 ".$wherestr." order by user_id asc ";
		$res = $GLOBALS['db']->getAll($sql);
	}
    if (!is_array($res))
    {
