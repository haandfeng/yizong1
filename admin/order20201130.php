...
elseif ($_REQUEST['act'] == 'info')
{
    ...
    if($is_exchange > 0){
        $order['exchange_amount'] = "CNY".$order['exchange_amount'];
    }else{
        $order['exchange_amount'] = "HKD".$order['goods_amount'];
    }
    ...
    while ($row = $db->fetchRow($res))
    {
        ...
        if($row['is_exchange'] > 0){
            ...
            $row['exchange_price'] = $row['exchange_price'];
        }else{
...
elseif (isset($_POST['to_dw_kuajing']))
{
    ...
    $action_note    = isset($_REQUEST['action_note']) ? trim($_REQUEST['action_note']) : '';
        ...
        $order['exchange_total_discount'] = $order['exchange_discount'] + $order['exchange_bonus'];
...
function order_list()
{
    ...
    if ($result === false)
    {
        ...
        /* 查询 */
        if($filter['supp']){
            $sql = "SELECT o.order_id, o.order_sn, o.add_time, o.order_status, o.shipping_status, o.order_amount, o.money_paid, o.exchange_amount, ss.is_exchange, ss.suppliers_name, st.store_name, sts.seller_name, p.shop_name, p.address AS shop_address," .
    ...
    foreach ($row AS $key => $value)
    {
        ...
        switch ($row[$key]['is_exchange']){
            case 0 :
                    $row[$key]['exchange_amount'] = "HKD" . $row[$key]['total_fee'];
                    break;
            case 1 :
                    $row[$key]['exchange_amount'] = "CNY" . $row[$key]['exchange_amount'];
                    break;
            default :
                    break;
        }
...
{
    $sql = "select user_id,user_name from ". $GLOBALS['ecs']->table('users') .
    " where isfenxiao_topuser = '1' order by user_id asc";
    $res = $GLOBALS['db']->getAll($sql);
