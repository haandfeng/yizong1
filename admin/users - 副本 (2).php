...
function action_edit ()
{
    ...
    u.office_phone, u.home_phone, u.mobile_phone,u.real_name,u.card,u.face_card,u.back_card,u.country,u.province,u.city,u.district,u.address,u.status,u.open_quickpay  " . " FROM " . $ecs->table('users') . " u LEFT JOIN " . $ecs->table('users') . " u2 ON u.parent_id = u2.user_id WHERE u.user_id='$_GET[id]'";
	/* 代码增加2014-12-23 by www.yshop100.com _end */
...
function action_update ()
{
	...
	$sql = "update " . $ecs->table('users') . " set `real_name`='$real_name',`card`='$card',`country`='$country',`province`='$province',`city`='$city',`district`='$district',`address`='$address',`status`='$status',open_quickpay='$open_quickpay',parent_id='$parent_id' where user_name = '" . $username . "'";
	$db->query($sql);
...
function user_list ()
{
	...
	if($result === false)
	{
		...
		// reg_time ".
		$sql = "SELECT user_id, user_name, email, mobile_phone, is_validated, validated, user_money, frozen_money, rank_points, pay_points,is_fenxiao, status, reg_time, froms".
		        /* 代码增加2014-12-23 by www.yshop100.com  _end  */
