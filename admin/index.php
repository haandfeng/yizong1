...
{
    clear_all_files();
    clearhtml_all();  //代码增加   By  www.yshop100.com
    echo 1;
    //sys_msg($_LANG['caches_cleared']);
}
//www.yshop100.com手机缓存修改start
...
elseif ($_REQUEST['act'] == 'main')
{
    ...
    /*后台管理起始页_修改_START_www.yshop100.com*/
	...
	$sql = 'SELECT  SUM(money_paid) AS sales FROM ' .$ecs->table('order_info'). ' WHERE `confirm_time` BETWEEN '.$today_start.' AND '.$today_end.'  AND supplier_id=0  '."AND order_status  IN ('1','5')   AND pay_status  IN ('2','1')  ";
    //file_put_contents('20191213.txt',"sql:".$sql."\r\n" , FILE_APPEND); 
	$today['money'] = $db->GetOne($sql);
    $today['formatted_money'] = price_format($today['money']);
    //今日订单数
    /* 代码修改_yshop100_20150729_STAR */
...
//    " WHERE supplier_id=0  " . order_query_sql('await_pay'));
    ...
    $order['new_repay'] = $db->getOne('SELECT COUNT(*) FROM '.$ecs->table('back_order').' WHERE status_back < 6 AND status_back != 3 AND back_type = 4 AND supplier_id=0');// 代码修改   By  www.yshop100.com End
    //退货申请
    // 代码修改   By  www.yshop100.com Start
...
//    " WHERE supplier_id=0 " . order_query_sql('finished');
    ...
    /*后台管理起始页_修改_END_www.yshop100.com*/
...
elseif ($_REQUEST['act'] == 'main_api')
{
    ...
    ' WHERE is_delete = 0 AND is_alone_sale = 1 AND is_real = 1');
        ...
        $t = new transport;
        // $api_comment = $t->request('http://api.yshop100.com/census.php', $apiget);
