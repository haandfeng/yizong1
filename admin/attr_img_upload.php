...
{
	$img_id_www_yshop100_com = empty($_REQUEST['img_id']) ? 0 : intval($_REQUEST['img_id']);
	$sql_www_yshop100_com = "update ". $GLOBALS['ecs']->table('goods_gallery') ." set is_attr_image='0' where goods_id='$goods_id' and goods_attr_id='$goods_attr_id' ";
	$db->query($sql_www_yshop100_com);
	$sql_www_yshop100_com = "update ". $GLOBALS['ecs']->table('goods_gallery') ." set is_attr_image='1' where img_id='$img_id_www_yshop100_com' ";
	$db->query($sql_www_yshop100_com);
	clear_cache_files();
}
