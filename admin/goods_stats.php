...
elseif ($_REQUEST['act'] == 'query')
{
    ...
    $result = get_sell_list($start_date, $end_date, $sel_shop, $supplier_id, $sel_fenxiao);
...
elseif ($_REQUEST['act'] == 'export')
{
    ...
    $sql = 'SELECT g.goods_id, g.goods_sn, g.goods_name, g.goods_attr, SUM(g.goods_number) sales_volume, SUM(g.goods_price*g.goods_number) sales_money, '
        ...
        . $GLOBALS['ecs']->table('order_goods') . ' g, ' . $GLOBALS['ecs']->table('order_info') . ' i LEFT JOIN ' . $GLOBALS['ecs']->table('users') . ' u ON i.user_id = u.user_id ' . $where
    ...
    $outputFileName = '商品销售排行_' . $start_date .'-'. $end_date . '.xls';
 ...
 * @return array
 */
function get_sell_list($start_date, $end_date, $sel_shop, $supplier_id, $sel_fenxiao)
{
    $result = get_filter();
    if($result === false)
    {
        ...
        // 按分销商查询
        if($filter['sel_fenxiao'] < 0)
        {
            // 平台方
            $where .= ' AND u.parent_id = 0';
        }elseif($filter['sel_fenxiao'] > 0){
        	$where .= ' AND u.parent_id = ' . $filter['sel_fenxiao'];
        }

        // 记录数
        $cnt = $GLOBALS['db']->getOne(
            'SELECT COUNT(*) FROM (SELECT DISTINCT goods_id FROM '. $GLOBALS['ecs']->table('order_goods')
            .' WHERE order_id IN (SELECT order_id FROM ' . $GLOBALS['ecs']->table('order_info') . ' i LEFT JOIN ' . $GLOBALS['ecs']->table('users') . ' u ON i.user_id = u.user_id ' . $where . ')) t'
        );
