...
if ($_REQUEST['act'] == 'list')
{
    ...
    foreach ($result['result'] as $k => $v) {
        $pay_ids = $result['result'][$k]['pay_ids'];
        $pay_ids_arr = explode(",", trim($pay_ids));

        if($pay_ids_arr){
            foreach ($pay_ids_arr as $pay_id) {
                $sql = "SELECT pay_name
                FROM " . $ecs->table('payment') . "
                WHERE pay_id = '$pay_id'
                AND enabled <> '0'";
                $row = $db->getOne($sql, TRUE);
                if($row){
                    if(isset($result['result'][$k]['pay_name'])){
                        $result['result'][$k]['pay_name'] .= ' ， ';
                    }
                    $result['result'][$k]['pay_name'] .= $row;
                }
            }
        }
    }
...
elseif (in_array($_REQUEST['act'], array('add', 'edit')))
{
    ...
    if ($_REQUEST['act'] == 'add')
    {
        ...
        $sql = "SELECT pay_id, pay_name FROM " . $ecs->table('payment') . "  WHERE enabled <> '0'";

        $suppliers['pay_list'] = $db->getAll($sql);
    ...
    elseif ($_REQUEST['act'] == 'edit')
    {
        ...
        if($pay_ids_arr && $pay_list){
            foreach ($pay_list as $k => $v) {
                if(in_array($v['pay_id'], $pay_ids_arr)){
                    $pay_list[$k]['is_pay'] = "1";
                }
            }
            $suppliers['pay_list'] = $pay_list;
        }
...
elseif (in_array($_REQUEST['act'], array('insert', 'update')))
{
    ...
    if ($_REQUEST['act'] == 'insert')
    {
        ...
        $suppliers = array('suppliers_name'   => trim($_POST['suppliers_name']),
                           ...
                           'is_exchange'   	  => trim($_POST['is_exchange']),
                           'parent_id'        => 0
                           );
        ...
        /* 更新支付方式 */
        $pay_ids_arr = $_POST['pay_ids'];
        foreach ($pay_ids_arr as $v) {
            if(isset($pay_ids)){
                $pay_ids .= ',';
            }
            $pay_ids .= $v;
        }
        $sql = "UPDATE " . $ecs->table('suppliers') . " SET pay_ids = '" . $pay_ids . "' WHERE suppliers_id = '". $suppliers['id'] ."'";
        $db->query($sql);
...
function suppliers_list()
{
    ...
    if ($result === false)
    {
        ...
        /* 查询 */
        $sql = "SELECT suppliers_id, suppliers_name, suppliers_desc, is_check, pay_ids 
