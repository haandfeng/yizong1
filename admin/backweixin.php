...
elseif ($_REQUEST['act'] == 'operate_refund')
{
	...
	/* 退回付款帐户 */
	else if ($_REQUEST['refund_type'] == '3')
	{ 
		$sql = "insert into " . $GLOBALS['ecs']->table('back_order_refund') . "(order_sn,back_id,order_id,order_amount,money,createtime,state)values";
		$sql = $sql."('".$order["order_sn"]."','".$back_id."','".$order['order_id']."','".$order['order_amount']."','".$refund_money_2."',NOW(),0)";
		$GLOBALS['db']->query($sql); 
		
		$sql = "select rid,order_sn,back_id,order_id,order_amount,money from " . $GLOBALS['ecs']->table('back_order_refund') . " where back_id='".$back_id."' and order_id='".$order['order_id']."' and state=0";
		$refund=$GLOBALS['db']->getRow($sql); 
		if($refund['rid'])
		{
			$sql = "update " . $GLOBALS['ecs']->table('back_order_refund') . " set state=1 where rid='".$refund['rid']."'";
			$GLOBALS['db']->query($sql); 
			
			include_once('../mobile/includes/modules/payment/weixin_cn.php');
			$pay_params = array(); 
			$pay_obj = new weixin_cn;
			$pay_params['order_sn']=$refund['order_sn'];
			$pay_params['order_amount']=$refund['order_amount'];
			$pay_params['out_refund_no']=$refund['rid'];
			$pay_params['refund_fee']=$refund['money'];
			$str = $pay_obj->jsApiRefund($pay_params); 
			file_put_contents('20211130.txt',"str::".$str."\r\n" , FILE_APPEND);
			$sql = "update " . $GLOBALS['ecs']->table('back_order_refund') . " set remark1='".str_replace("'","''",$str)."' where rid='".$refund['rid']."'";
			$GLOBALS['db']->query($sql); 
