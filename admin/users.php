...
function action_newuserscount()
{
	...
	$user_list = get_fenxiao_userscount($date_start,$date_end);
	$smarty->assign('user_list', $user_list); 
	$smarty->display('users_totol.htm');
}
...
function action_insert ()
{
	...
	$invite_code = empty($_POST['invite_code']) ? '0' : trim($_POST['invite_code']);//邀请码 不填写 默认为0 平台
    ...
    $invite_parent = $db->GetRow('select user_id from '. $ecs->table('users') .' where `invite_code`="'.$invite_code.'" limit 1' );
    $other['invite_parent'] = $invite_parent['user_id']?:0;
    created_invite_qrcode($user_id_arr['user_id']);
	...
	$sql = "update " . $ecs->table('users') . " set  mobile_phone='$mobile_phone' , `real_name`='$real_name',`card`='$card',`country`='$country',`province`='$province',`city`='$city',`district`='$district',`address`='$address',`status`='$status',`dl_status`='$dl_status' where user_name = '" . $username . "'";
	$db->query($sql);
...
function action_edit ()
{
	...
	$sql = "SELECT u.user_name, u.sex, u.birthday, u.pay_points, u.rank_points, u.user_rank , u.user_money, u.frozen_money, u.credit_line, u.parent_id, u2.user_name as parent_username, u.qq, u.msn, u.office_phone, u.home_phone, u.mobile_phone,u.open_quickpay " . " FROM " . $ecs->table('users') . " u LEFT JOIN " . $ecs->table('users') . " u2 ON u.parent_id = u2.user_id WHERE u.user_id='$_GET[id]'";
    ...
    u.office_phone, u.home_phone, u.mobile_phone,u.real_name,u.card,u.face_card,u.back_card,u.country,u.province,u.city,u.district,u.address,u.status,u.dl_status,u.open_quickpay  " . " FROM " . $ecs->table('users') . " u LEFT JOIN " . $ecs->table('users') . " u2 ON u.parent_id = u2.user_id WHERE u.user_id='$_GET[id]'";
	/* 代码增加2014-12-23 by www.yshop100.com _end */
	...
	if($row)
	{
		...
		$user['dl_status'] = $row['dl_status'];
		$user['open_quickpay'] = $row['open_quickpay'];
		/* 代码增加2014-12-23 by www.yshop100.com _end */
	}
	else
	{
	...
	$sql = "SELECT user_id, user_name FROM " . $ecs->table('users') . " WHERE is_fenxiao = '1' AND status = '1' ORDER BY isfenxiao_topuser DESC";
	$fenxiao_list = $db->getAll($sql);
	$smarty->assign('fenxiao_list', $fenxiao_list);
...
function action_update ()
{
	...
	$status = $_POST['status'];
	$dl_status = $_POST['dl_status'];
	$open_quickpay = $_POST['open_quickpay'];
	...
	$sql = "update " . $ecs->table('users') . " set `real_name`='$real_name',`card`='$card',`country`='$country',`province`='$province',`city`='$city',`district`='$district',`address`='$address',`status`='$status',`dl_status`='$dl_status',open_quickpay='$open_quickpay',parent_id='$parent_id' where user_id = '" . $user_id . "'";
	$db->query($sql);
	
	if($face_card != '')
	{
		$sql = "update " . $ecs->table('users') . " set `face_card` = '$face_card' where user_id = '" . $user_id . "'";
		$db->query($sql);
	}
	if($back_card != '')
	{
		$sql = "update " . $ecs->table('users') . " set `back_card` = '$back_card' where user_id = '" . $user_id . "'";
		$db->query($sql);
	}
	/* 代码增加2014-12-23 by www.yshop100.com _end */
	/* 更新用户扩展字段的数据 */
...
function action_batch_remove ()
{
	...
	if(isset($_POST['checkboxes']))
	{
		...
		//mantsui 增加删除weixin_user
		$sql = "SELECT uid FROM " . $ecs->table('weixin_user') . " WHERE ecuid " . db_create_in($_POST['checkboxes']) . "";
		$query = $db->query($sql);
		while ($row = $db->fetch_array($query)) {
			if($row['uid']>0){
				$sql_del = "DELETE FROM " . $ecs->table('weixin_user') . " WHERE uid = '".$row['uid']."'";
				$db->query($sql_del);
			}
		}exit;
			$sql = "DELETE FROM " . $ecs->table('weixin_user') . " WHERE uid '".db_create_in($_POST['checkboxes'])."'";
			$db->query($sql);
...
function action_remove ()
{
	...
	if($issupplier>0){
		...
		//删除weixin_user
		$sql = "SELECT uid FROM " . $ecs->table('weixin_user') . " WHERE ecuid = '" . $_GET['id'] . "'";
		$weixin_uid = $db->getOne($sql);
		if($weixin_uid>0){
			$sql = "DELETE FROM " . $ecs->table('weixin_user') . " WHERE uid = '".$weixin_uid."'";
			$db->query($sql);
		}
...
function action_aff_list ()
{
	...
	for($i = 1; $i <= $num; $i ++)
	{
		...
		if($count)
		{
			$sql = "SELECT user_id, user_name, '$i' AS level, email, is_validated, user_money, frozen_money, rank_points, pay_points, reg_time,invite_qrcode " . " FROM " . $GLOBALS['ecs']->table('users') . " WHERE user_id IN($up_uid)" . " ORDER by level, user_id";
	...
	for($i = 0; $i < $temp_count; $i ++)
	{
		...
		$user_list['user_list'][$i]['invite_qrcode'] =  "http://".$_SERVER['HTTP_HOST'].'/images/qrimage/'.$user_list['user_list'][$i]['invite_qrcode'];
	}
 ...
 * @return void
 */
function get_fenxiao_userscount($start,$end)
{
	if ($start=='')
	{
	...
	{  
		$today_start=strtotime($start);
		$today_end=strtotime($end."+1day")-1; 
	}
    $sql = "select 0 as user_id,'平台方' as user_name,0 as cc union select user_id,user_name,0 as cc from ". $GLOBALS['ecs']->table('users') .
	...
	for($i = 0; $i < $count; $i ++)
	{ 
        $sql = 'SELECT count(user_id) as ccount FROM ' . $GLOBALS['ecs']->table('users') . ' WHERE `reg_time` BETWEEN '.$today_start.' AND '.$today_end.' and parent_id ='.$res[$i]['user_id'];
...
function user_list ()
{
	...
	if($result === false)
	{
		...
		// reg_time ".
		$sql = "SELECT user_id, user_name, email, mobile_phone, is_validated, validated, user_money, frozen_money, rank_points, pay_points,is_fenxiao, status,sm_status, reg_time, froms".
		        /* 代码增加2014-12-23 by www.yshop100.com  _end  */
	...
	for($i = 0; $i < $count; $i ++)
	{
		...
		//$user_list[$i]['invite_qrcode'] = 'http://'.$_SERVER['HTTP_HOST'].'/images/qrimage/'.$user_list[$i]['invite_qrcode'];
		/*获取 */
        $sql = "SELECT qcode FROM " . $GLOBALS['ecs']->table('weixin_qcode') . " WHERE content =" .$user_list[$i]['user_id'];
        $row = $GLOBALS['db']->getRow($sql);
        $user_list[$i]['invite_qrcode'] = $row['qcode'];
 ...
 * @return string
 */
function created_invite_qrcode($uid)
{
    ...
    //更新 注册会员的 邀请码 及 二维码
    $sql = 'UPDATE ' . $GLOBALS['ecs']->table('users') . ' SET `invite_code` = "' . $invite_code_self .'",`invite_qrcode` = "' . $file .'" WHERE user_id = ' .$uid;
    $GLOBALS['db']->query($sql);
}
